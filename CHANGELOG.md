...
- Added 8 new built-in scales: Melodic Minor, Hungarian Minor, Marva (Indian), Arabian, Whole Tone, Blues, Pentatonic
  Minor, Hirajoshi.
- Added "play `ONCE`" clip launch mode, settable in `ROW VIEW` (Song view) by pressing the Select Encode while holding
  the mute pad. Causes the clip to play once when triggered and then mute without changing the section.

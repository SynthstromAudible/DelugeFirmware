...
public:
    ...
    bool render(SamplePlaybackGuide* guide, int32_t* oscBuffer, int numSamples, Sample* sample, int numChannels, int loopingType,
    		int32_t phaseIncrement, int32_t timeStretchRatio, int32_t amplitude, int32_t amplitudeIncrement, int bufferSize, int desiredInterpolationMode,
			int priorityRating);
    void beenUnassigned();
    bool shouldObeyMarkers() { return (!cache && !timeStretcher && !forAudioClip); }	// AudioClips don't obey markers because they "fudge" instead.
    																					// Or if fudging can't happen cos no pre-margin, then
    																					// AudioClip::doTickForward() manually forces restart.

    void readSamplesResampledPossiblyCaching(int32_t** oscBufferPos, int32_t** oscBufferRPos, int numSamples, Sample* sample, int jumpAmount, int numChannels, int numChannelsAfterCondensing,
    		int32_t phaseIncrement, int32_t* sourceAmplitudeNow, int32_t amplitudeIncrement, int bufferSize, int reduceMagnitudeBy = 1);
    ...
    uint32_t pendingSamplesLate; // This isn't used for AudioClips. And for samples in STRETCH mode, the exact number isn't relevant - it gets recalculated
    ...
    int32_t cacheLoopEndPointBytes; // 2147483647 means no looping. Will be set to sample end-point if looping there. Gets re-set to 2147483647 when note "released"
    int32_t cacheEndPointBytes; // Will sometimes be the whole length of the sample. Wherever the red marker is. Or a little further if it's the full length of the sample, to allow for timestretch / interpolation ring-out
    uint32_t cacheLoopLengthBytes;

private:
    bool weShouldBeTimeStretchingNow(Sample* sample, SamplePlaybackGuide* guide, int numSamples, int32_t phaseIncrement, int32_t timeStretchRatio, int playDirection, int priorityRating, int loopingType);

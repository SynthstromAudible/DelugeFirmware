...
public:
    ...
    virtual void detachFromOutput(ModelStackWithTimelineCounter* modelStack, bool shouldRememberDrumName, bool shouldDeleteEmptyNoteRowsAtEndOfList = false,
			...
			bool shouldBackUpExpressionParamsToo = true) = 0; // You're likely to want to call pickAnActiveClipIfPossible() after this
    ...
    virtual int claimOutput(ModelStackWithTimelineCounter* modelStack) = 0; // To be called after Song loaded, to link to the relevant Output object
    ...
    int32_t loopLength;
	int32_t originalLength; // Before linear recording of this Clip began, and this Clip started getting extended to multiples of this
...
protected:
    virtual void posReachedEnd(ModelStackWithTimelineCounter* modelStack); // May change the TimelineCounter in the modelStack if new Clip got created
    virtual bool cloneOutput(ModelStackWithTimelineCounter* modelStack) = 0; // Returns whether a new Output was in fact created

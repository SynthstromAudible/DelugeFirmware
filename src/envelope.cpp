...
int32_t Envelope::render(uint32_t numSamples, uint32_t attack, uint32_t decay, uint32_t sustain, uint32_t release, const uint16_t* releaseTable) {

considerEnvelopeStage:
	switch (state) {
	case ENVELOPE_STAGE_ATTACK:
		pos += attack * numSamples; // Increment the pos *before* taking a value, so we can skip the attack section entirely with a high posIncrease
		if (pos >= 8388608) {
			pos = 0;
	...
	case ENVELOPE_STAGE_DECAY:
		lastValue = sustain
				+ (uint32_t)multiply_32x32_rshift32(getDecay8(pos, 23),
				2147483647 - sustain
				) * 2;
	...
	case ENVELOPE_STAGE_FAST_RELEASE:
		...
		lastValue = multiply_32x32_rshift32((getSine(pos + (8388608 >> 1), 24) >> 1) + 1073741824, lastValuePreCurrentStage) << 1;
		break;

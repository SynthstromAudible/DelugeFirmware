...
int AudioFile::loadFile(AudioFileReader* reader, bool isAiff, bool makeWaveTableWorkAtAllCosts) {
	...
	if (type == AUDIO_FILE_TYPE_WAVETABLE && !makeWaveTableWorkAtAllCosts && isAiff) return ERROR_FILE_NOT_LOADABLE_AS_WAVETABLE;
    ...
    while (bytePos < reader->fileSize) {
    	...
    	thisChunk.length = (thisChunk.length + 1) & ~(uint32_t)1; // If chunk size is odd, skip the extra byte of padding at the end too. Weird RIFF file requirement.
...
doSetupWaveTable:
					if (byteDepth == 255) return ERROR_FILE_UNSUPPORTED; // If haven't found "fmt " tag yet, we don't know the bit depth or anything. Shouldn't happen.

					if (numChannels != 1) return ERROR_FILE_NOT_LOADABLE_AS_WAVETABLE_BECAUSE_STEREO; // Stereo files not useable as WaveTable, ever.
					...
					error = ((WaveTable*)this)->setup(NULL, waveTableCycleSize, audioDataStartPosBytes, audioDataLengthBytes, byteDepth, rawDataFormat, (WaveTableReader*)reader);
			...
			case 'lpms': {
				if (type == AUDIO_FILE_TYPE_SAMPLE) {
					...
					if ((midiNote || midiPitchFraction) && midiNote < 128) ((Sample*)this)->midiNoteFromFile = (float)midiPitchFraction / ((uint64_t)1 << 32) + midiNote;
        ...
        // ------ AIFF ------
        	...
        	case 'KRAM': {
                ...
                for (int m = 0; m < numMarkers; m++) {
                    ...
                    reader->byteIndexWithinCluster += stringLengthRoundedUpToBeEven; // Cluster boundaries will be checked at next read
                }
        		break;
        	}

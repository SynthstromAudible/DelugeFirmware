...
void MenuItemMidiCommand::drawPixelsForOled() {
	...
	if (!command->containsSomething()) {
		OLED::drawString("Command unassigned", 0, yPixel, OLED::oledMainImage[0], OLED_MAIN_WIDTH_PIXELS, TEXT_SPACING_X, TEXT_SIZE_Y_UPDATED);
	}
	else {
		...
		OLED::drawString(deviceString, 0, yPixel, OLED::oledMainImage[0], OLED_MAIN_WIDTH_PIXELS, TEXT_SPACING_X, TEXT_SIZE_Y_UPDATED);
		OLED::setupSideScroller(0, deviceString, TEXT_SPACING_X, OLED_MAIN_WIDTH_PIXELS, yPixel, yPixel + 8, TEXT_SPACING_X, TEXT_SPACING_Y, false);
		...
		if (command->channelOrZone == MIDI_CHANNEL_MPE_LOWER_ZONE) channelText = "MPE lower zone";
			...
			OLED::drawString(buffer, TEXT_SPACING_X * 8, yPixel, OLED::oledMainImage[0], OLED_MAIN_WIDTH_PIXELS, TEXT_SPACING_X, TEXT_SIZE_Y_UPDATED);
		}
		OLED::drawString(channelText, 0, yPixel, OLED::oledMainImage[0], OLED_MAIN_WIDTH_PIXELS, TEXT_SPACING_X, TEXT_SIZE_Y_UPDATED);

		yPixel += TEXT_SPACING_Y;

		OLED::drawString("Note", 0, yPixel, OLED::oledMainImage[0], OLED_MAIN_WIDTH_PIXELS, TEXT_SPACING_X, TEXT_SIZE_Y_UPDATED);
		char buffer[12];
		intToString(command->noteOrCC, buffer, 1);
		OLED::drawString(buffer, TEXT_SPACING_X * 5, yPixel, OLED::oledMainImage[0], OLED_MAIN_WIDTH_PIXELS, TEXT_SPACING_X, TEXT_SIZE_Y_UPDATED);
	}
}
#else

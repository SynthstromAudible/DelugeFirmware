...
void InstrumentClip::copyBasicsFrom(Clip* otherClip) {
    ...
    memcpy(backedUpInstrumentSubSlot, otherInstrumentClip->backedUpInstrumentSubSlot, sizeof(backedUpInstrumentSubSlot));
    for (int i = 0; i < 2; i++) {
...
int InstrumentClip::clone(ModelStackWithTimelineCounter* modelStack, bool shouldFlattenReversing) {
    ...
    InstrumentClip* newClip = new (clipMemory) InstrumentClip(); // Don't supply Song. yScroll will get set in copyBasicsFrom()
...
deleteClipAndGetOut:
    ...
    if (shouldFlattenReversing && newClip->sequenceDirectionMode == SEQUENCE_DIRECTION_REVERSE) newClip->sequenceDirectionMode = SEQUENCE_DIRECTION_FORWARD;
...
void InstrumentClip::increaseLengthWithRepeats(ModelStackWithTimelineCounter* modelStack, int32_t newLength, int independentNoteRowInstruction, bool completelyRenderOutIterationDependence, Action* action) {

	int numRepeatsRounded = completelyRenderOutIterationDependence ?
			0 :
			(uint32_t)(newLength + (loopLength >> 1)) / (uint32_t)loopLength;
	...
	for (int i = 0; i < noteRows.getNumElements(); i++) {
		...
		if (thisNoteRow->loopLengthIfIndependent) {
			...
			case INDEPENDENT_NOTEROW_LENGTH_INCREASE_ROUND_UP:
				newLengthHere = ((uint32_t)(newLength - 1) / (uint32_t)thisNoteRow->loopLengthIfIndependent + 1) * thisNoteRow->loopLengthIfIndependent;
				break;
			...
			numRepeatsRoundedHere = completelyRenderOutIterationDependence ?
					0 : (uint32_t)(newLengthHere + (thisNoteRow->loopLengthIfIndependent >> 1)) / (uint32_t)thisNoteRow->loopLengthIfIndependent;
		...
		if (newLengthHere > oldLengthHere) { // Or do nothing if length staying the same
			...
			thisNoteRow->generateRepeats(modelStackWithNoteRow, oldLengthHere, newLengthHere, numRepeatsRoundedHere, action);
		}
	...
	if (newLength > loopLength) {
		ModelStackWithThreeMainThings* modelStackWithParamManager = modelStack->addOtherTwoThingsButNoNoteRow(output->toModControllable(), &paramManager);
		paramManager.generateRepeats(modelStackWithParamManager, loopLength, newLength, pingponging);
	}

	if (pingponging) sequenceDirectionMode = SEQUENCE_DIRECTION_FORWARD; // Pingponging has been flattened out, and although there are arguments either way, I think removing that setting now is best.
...
void InstrumentClip::repeatOrChopToExactLength(ModelStackWithTimelineCounter* modelStack, int32_t newLength) {
    ...
    Clip::lengthChanged(modelStack, oldLength, NULL); // Call this on Clip::, not us InstrumentClip, because we've done our own version above of what that call would do.
...
void InstrumentClip::halveNoteRowsWithIndependentLength(ModelStackWithTimelineCounter* modelStack) {
	for (int i = 0; i < noteRows.getNumElements(); i++) {
		...
		if (noteRow->loopLengthIfIndependent) {
			noteRow->setLength(modelStackWithNoteRow, noteRow->loopLengthIfIndependent >> 1, NULL, modelStackWithNoteRow->getLastProcessedPos(), true);
		}
	}
}
...
void InstrumentClip::setPos(ModelStackWithTimelineCounter* modelStack, int32_t newPos, bool useActualPosForParamManagers) {
	...
	Clip::setPosForParamManagers(modelStack, useActualPosForParamManagers);	// Call on Clip:: only - below in this function, we're going to do the equivalent
	...
	for (int i = 0; i < noteRows.getNumElements(); i++) {
		...
		if (thisNoteRow->paramManager.mightContainAutomation()) {
			ModelStackWithThreeMainThings* modelStackWithThreeMainThings = modelStack->addNoteRowAndExtraStuff(i, thisNoteRow);
			thisNoteRow->paramManager.setPlayPos(posForParamManagers, modelStackWithThreeMainThings, modelStackWithThreeMainThings->isCurrentlyPlayingReversed());
		}
		...
		if (thisNoteRow->hasIndependentPlayPos()) {
			...
			thisNoteRow->currentlyPlayingReversedIfIndependent = (effectiveSequenceDirectionMode == SEQUENCE_DIRECTION_REVERSE			// Syncing pingponging with repeatCount is particularly important for when resuming after
					|| (effectiveSequenceDirectionMode == SEQUENCE_DIRECTION_PINGPONG && (thisNoteRow->repeatCountIfIndependent & 1)));	// recording a clone of this Clip from session to arranger.

			thisNoteRow->lastProcessedPosIfIndependent = newPos - thisNoteRow->repeatCountIfIndependent * effectiveLoopLength;
			if (thisNoteRow->currentlyPlayingReversedIfIndependent) {
				if (thisNoteRow->lastProcessedPosIfIndependent) thisNoteRow->lastProcessedPosIfIndependent = effectiveLoopLength - thisNoteRow->lastProcessedPosIfIndependent;
	...
	if (output->type == INSTRUMENT_TYPE_KIT) {
		...
		for (Drum* thisDrum = kit->firstDrum; thisDrum; thisDrum = thisDrum->next) {
			...
			if (velocity) {
				...
				int noteRowIndex;
				NoteRow* noteRow = getNoteRowForDrum(thisDrum, &noteRowIndex); // Remember, I'm planning to introduce a faster search/index for this
				if (noteRow) {
		...
		if (melodicInstrument->earlyNotes.getNumElements()) {
			...
			for (int i = 0; i < melodicInstrument->earlyNotes.getNumElements(); i++) {
				...
				ModelStackWithNoteRow* modelStackWithNoteRow = getOrCreateNoteRowForYNote(basicNote->note, modelStack, action, &scaleAltered);
				NoteRow* noteRow = modelStackWithNoteRow->getNoteRowAllowNull();
				if (noteRow) {

					noteRow->attemptNoteAdd(0, 1, basicNote->velocity, NUM_PROBABILITY_VALUES, modelStackWithNoteRow, action);
...
void InstrumentClip::setPosForParamManagers(ModelStackWithTimelineCounter* modelStack, bool useActualPos) {
	...
	for (int i = 0; i < noteRows.getNumElements(); i++) {
		...
		if (thisNoteRow->paramManager.mightContainAutomation()) {
			ModelStackWithThreeMainThings* modelStackWithThreeMainThings = modelStack->addNoteRowAndExtraStuff(i, thisNoteRow);
			thisNoteRow->paramManager.setPlayPos(pos, modelStackWithThreeMainThings, modelStackWithThreeMainThings->isCurrentlyPlayingReversed());
		}
	}
...
void InstrumentClip::reGetParameterAutomation(ModelStackWithTimelineCounter* modelStack) {
	...
	for (int i = 0; i < noteRows.getNumElements(); i++) {
		...
		if (thisNoteRow->paramManager.mightContainAutomation()) {
			ModelStackWithThreeMainThings* modelStackWithThreeMainThings = modelStack->addNoteRowAndExtraStuff(i, thisNoteRow);
			thisNoteRow->paramManager.setPlayPos(actualPos, modelStackWithThreeMainThings, modelStackWithThreeMainThings->isCurrentlyPlayingReversed());
		}
	}
}
...
int InstrumentClip::transferVoicesToOriginalClipFromThisClone(ModelStackWithTimelineCounter* modelStackOriginal, ModelStackWithTimelineCounter* modelStackClone) {
	...
	if (output->type == INSTRUMENT_TYPE_KIT) {
		...
		for (int i = 0; i < noteRows.getNumElements(); i++) {
			...
			clonedNoteRow->resumeOriginalNoteRowFromThisClone(modelStackWithNoteRowOriginal, modelStackWithNoteRowClone);
		}
	}
		...
		for (int i = 0; i < noteRows.getNumElements(); i++) {
			NoteRow* clonedNoteRow = noteRows.getElement(i);
			NoteRow* originalNoteRow = originalClip->getNoteRowForYNote(clonedNoteRow->y); // Might come back NULL cos it doesn't exist - that's ok

			ModelStackWithNoteRow* modelStackWithNoteRowClone = modelStackClone->addNoteRow(clonedNoteRow->y, clonedNoteRow);
			ModelStackWithNoteRow* modelStackWithNoteRowOriginal = modelStackOriginal->addNoteRow(clonedNoteRow->y, originalNoteRow); // May end up with NULL noteRow
...
// Returns error
int InstrumentClip::appendClip(ModelStackWithTimelineCounter* thisModelStack, ModelStackWithTimelineCounter* otherModelStack) {
	...
	if (output->type == INSTRUMENT_TYPE_KIT) {
		...
		for (int i = 0; i < noteRows.getNumElements(); i++) {
			...
			if (otherNoteRow->loopLengthIfIndependent) continue; // Skip NoteRows with independent length - they'll take care of themselves.
			...
			int error = thisNoteRow->appendNoteRow(thisModelStackWithNoteRow, otherModelStackWithNoteRow, loopLength, whichRepeatThisIs, otherInstrumentClip->loopLength);
		...
		for (int i = 0; i < otherInstrumentClip->noteRows.getNumElements(); i++) {
			...
			if (thisNoteRow) {
				ModelStackWithNoteRow* otherModelStackWithNoteRow = otherModelStack->addNoteRow(noteRowId, otherNoteRow);
...
void InstrumentClip::posReachedEnd(ModelStackWithTimelineCounter* thisModelStack) {
	...
	if (playbackHandler.recording == RECORDING_ARRANGEMENT && isArrangementOnlyClip()) {
		...
		ModelStackWithTimelineCounter* otherModelStack = setupModelStackWithSong(otherModelStackMemory, thisModelStack->song)->addTimelineCounter(beingRecordedFromClip);
...
bool InstrumentClip::wantsToBeginLinearRecording(Song* song) {
	...
	if (isPendingOverdub) return true; // Must take precedence - because we may have already placed some new notes at 0 if user hit key just now
...
void InstrumentClip::pingpongOccurred(ModelStackWithTimelineCounter* modelStack) {
	for (int i = 0; i < noteRows.getNumElements(); i++) {
		...
		if (thisNoteRow->paramManager.containsAnyParamCollectionsIncludingExpression() && !thisNoteRow->hasIndependentPlayPos()) {
			ModelStackWithThreeMainThings* modelStackWithThreeMainThings = modelStack->addNoteRowAndExtraStuff(i, thisNoteRow);
...
void InstrumentClip::processCurrentPos(ModelStackWithTimelineCounter* modelStack, uint32_t ticksSinceLast) {
    ...
    if (ticksTilNextNoteRowEvent <= 0) {
		...
		static PendingNoteOnList pendingNoteOnList; // Making this static, which it really should have always been, actually didn't help max stack usage at all somehow...
		...
		for (int i = 0; i < noteRows.getNumElements(); i++) {
			...
			int32_t noteRowTicksTilNextEvent = thisNoteRow->processCurrentPos(modelStackWithNoteRow, noteRowsNumTicksBehindClip, &pendingNoteOnList);
			if (noteRowTicksTilNextEvent < ticksTilNextNoteRowEvent) ticksTilNextNoteRowEvent = noteRowTicksTilNextEvent;
		}
		...
		for (int i = 0; i < pendingNoteOnList.count; i++) {
			...
			// Otherwise...
				...
				if (probability > NUM_PROBABILITY_VALUES) {
					...
					ModelStackWithNoteRow* modelStackWithNoteRow = modelStack->addNoteRow(pendingNoteOnList.pendingNoteOns[i].noteRowId, pendingNoteOnList.pendingNoteOns[i].noteRow);

					conditionPassed = (iterationWithinDivisor == ((uint32_t)modelStackWithNoteRow->getRepeatCount() % (uint32_t)divisor));
				}
				...
				// Or if it's an actual probability kind...
					...
					if (pendingNoteOnList.pendingNoteOns[i].probability & 128) {
						...
						if (lastProbabiltyPos[probability] == -1 || lastProbabiltyPos[probability] == lastProcessedPos) goto doNewProbability;
...
doNewProbability:
	...
	if (ticksTilNextNoteRowEvent < playbackHandler.swungTicksTilNextEvent) playbackHandler.swungTicksTilNextEvent = ticksTilNextNoteRowEvent;
}


void InstrumentClip::sendPendingNoteOn(ModelStackWithTimelineCounter* modelStack, PendingNoteOn* pendingNoteOn) {

	ModelStackWithNoteRow* modelStackWithNoteRow = modelStack->addNoteRow(pendingNoteOn->noteRowId, pendingNoteOn->noteRow);

	int16_t mpeValues[NUM_EXPRESSION_DIMENSIONS];
	pendingNoteOn->noteRow->getMPEValues(modelStackWithNoteRow, mpeValues);

	if (output->type == INSTRUMENT_TYPE_KIT) {
		ModelStackWithThreeMainThings* modelStackWithThreeMainThings = modelStackWithNoteRow->addOtherTwoThings(pendingNoteOn->noteRow->drum->toModControllable(), &pendingNoteOn->noteRow->paramManager);

		pendingNoteOn->noteRow->drum->noteOn(modelStackWithThreeMainThings, pendingNoteOn->velocity, (Kit*)output, mpeValues, MIDI_CHANNEL_NONE, pendingNoteOn->sampleSyncLength, pendingNoteOn->ticksLate);
	}
	else {
		ModelStackWithThreeMainThings* modelStackWithThreeMainThings = modelStackWithNoteRow->addOtherTwoThings(output->toModControllable(), &paramManager);
		((MelodicInstrument*)output)->sendNote(modelStackWithThreeMainThings, true, pendingNoteOn->noteRow->getNoteCode(), mpeValues, MIDI_CHANNEL_NONE, pendingNoteOn->velocity, pendingNoteOn->sampleSyncLength, pendingNoteOn->ticksLate);
	}
}
...
void InstrumentClip::toggleNoteRowMute(ModelStackWithNoteRow* modelStack) {
    ...
    if (action) {
    	...
    	if (consMemory) {
    		ConsequenceNoteRowMute* newConsequence = new (consMemory) ConsequenceNoteRowMute(this, modelStack->noteRowId);
    		action->addConsequence(newConsequence);
    	}
    }

    modelStack->getNoteRow()->toggleMute(modelStack, playbackHandler.isEitherClockActive() && modelStack->song->isClipActive(this));
}
...
ModelStackWithNoteRow* InstrumentClip::getNoteRowForDrumName(ModelStackWithTimelineCounter* modelStack, char const* name) {
	...
	for (i = 0; i < noteRows.getNumElements(); i++) {
		...
		if (thisNoteRow->drum && thisNoteRow->paramManager.containsAnyMainParamCollections() && thisNoteRow->drum->type == DRUM_TYPE_SOUND) {
...
ModelStackWithNoteRow* InstrumentClip::getOrCreateNoteRowForYNote(int yNote, ModelStackWithTimelineCounter* modelStack, Action* action, bool* scaleAltered) {
	...
	if (!modelStackWithNoteRow->getNoteRowAllowNull()) {
        ...
        if (thisNoteRow) {
            ...
            if (!modelStack->song->isYNoteAllowed(yNote, inScaleMode)) {
                ...
                int newI = thisNoteRow->notes.insertAtKey(0); // Total hack - make it look like the NoteRow has a Note, so it doesn't get discarded during setRootNote(). We set it back (and then will soon give it a real note) really soon
            	...
            	if (action) {
            		...
            		if (consMemory) {
            			ConsequenceScaleAddNote* newConsequence = new (consMemory) ConsequenceScaleAddNote((yNote + 120) % 12);
            			action->addConsequence(newConsequence);
            		}
...
void InstrumentClip::expectNoFurtherTicks(Song* song, bool actuallySoundChange) {
	...
	ModelStackWithTimelineCounter* modelStack = setupModelStackWithTimelineCounter(modelStackMemory, song, this); // TODO: make caller supply this

	stopAllNotesPlaying(modelStack, actuallySoundChange && !currentlyRecordingLinearly); // Stop all sound

	ModelStackWithThreeMainThings* modelStackWithThreeMainThings = modelStack
	    				->addOtherTwoThingsButNoNoteRow(output->toModControllable(), &paramManager);
    ...
    if (output->type == INSTRUMENT_TYPE_KIT) {
    	for (int i = 0; i < noteRows.getNumElements(); i++) {
    		...
    		if (thisNoteRow->drum && thisNoteRow->paramManager.containsAnyParamCollectionsIncludingExpression()) {
    			ModelStackWithThreeMainThings* modelStackWithThreeMainThingsForNoteRow = modelStack->addNoteRow(i, thisNoteRow)->addOtherTwoThings(thisNoteRow->drum->toModControllable(), &thisNoteRow->paramManager);
    	...
    	if (expressionParamsSummary->paramCollection) {
    		ModelStackWithParamCollection* modelStackWithParamCollection = modelStackWithThreeMainThings->addParamCollectionSummary(expressionParamsSummary);

    		((ExpressionParamSet*)modelStackWithParamCollection->paramCollection)->clearValues(modelStackWithParamCollection);
    	}
    }
#endif
...
bool InstrumentClip::renderAsSingleRow(ModelStackWithTimelineCounter* modelStack, TimelineView* editorScreen, int32_t xScroll, uint32_t xZoom, uint8_t* image, uint8_t occupancyMask[], bool addUndefinedArea,
	...
	Clip::renderAsSingleRow(modelStack, editorScreen, xScroll, xZoom, image, occupancyMask, addUndefinedArea, noteRowIndexStart, noteRowIndexEnd, xStart, xEnd, allowBlur, drawRepeats);
	...
	for (int i = noteRowIndexStart; i < noteRowIndexEnd; i++) {
		...
		thisNoteRow->renderRow(editorScreen, mainColour, tailColour, blurColour, image, occupancyMask, false, loopLength, rowAllowsNoteTails, displayWidth, xScroll, xZoom, xStart, xEnd, drawRepeats);
    }
    if (addUndefinedArea) drawUndefinedArea(xScroll, xZoom, loopLength, image, occupancyMask, displayWidth, editorScreen, currentSong->tripletsOn);
...
int InstrumentClip::guessRootNote(Song* song, int previousRoot) {
    ...
    seeWhatNotesWithinOctaveArePresent(notesPresent, 0, song, false); // Don't delete anything yet, since we're still going to make use of the noteRowsOnScreen!
...
int InstrumentClip::setNonAudioInstrument(Instrument* newInstrument, Song* song, ParamManager* newParamManager) {
	...
	if (newInstrument->type == INSTRUMENT_TYPE_MIDI_OUT) {
		char modelStackMemory[MODEL_STACK_MAX_SIZE];
		ModelStackWithModControllable* modelStack = setupModelStackWithModControllable(modelStackMemory, song, this, newInstrument->toModControllable());
...
int InstrumentClip::setInstrument(Instrument* newInstrument, Song* song, ParamManager* newParamManager, InstrumentClip* favourClipForCloningParamManager) {
    ...
    // Or if Synth or Kit...
		...
		return setAudioInstrument(newInstrument, song, false, newParamManager, favourClipForCloningParamManager); // Tell it not to setup patching - this will happen back here in changeInstrumentPreset() after all Drums matched up
    }
}
...
int InstrumentClip::changeInstrument(ModelStackWithTimelineCounter* modelStack, Instrument* newInstrument, ParamManagerForTimeline* newParamManager, int instrumentRemovalInstruction, InstrumentClip* favourClipForCloningParamManager, bool keepNoteRowsWithMIDIInput, bool giveMidiAssignmentsToNewInstrument) {
	...
	if (newInstrument->type == INSTRUMENT_TYPE_KIT) {
		...
		if (output->type != INSTRUMENT_TYPE_KIT) {
			...
			shouldBackUpExpressionParamsToo = true; // If switching from non-Kit to Kit, expression params won't get used, so store them with the backup in case the old MelodicInstrument gets used again later. Actually is this ideal?
		}
	}
    ...
    if (isActiveOnOutput() && playbackHandler.isEitherClockActive()) expectNoFurtherTicks(modelStack->song); // Still necessary? Probably.
	...
	if (newInstrument->type == INSTRUMENT_TYPE_SYNTH) {
		...
		paramManager.getPatchCableSet()->grabVelocityToLevelFromMIDIInput(&synth->midiInput); // Should happen before we call setupPatching().
	...
	if (playbackHandler.isEitherClockActive() && paramManager.mightContainAutomation() && modelStack->song->isClipActive(this)) {
	...
	if (newInstrument->type == INSTRUMENT_TYPE_KIT) {
		...
		assignDrumsToNoteRows(modelStack, true, numNoteRowsDeletedFromBottom); // If any unassigned Drums, give them to any NoteRows without a Drum - or create them a new NoteRow. Sets up patching
			...
			if (zone >= 0) {
				...
				if (newBendRanges[BEND_RANGE_FINGER_LEVEL]) {
					if (!hasAnyPitchExpressionAutomationOnNoteRows()) {
						...
						if (expressionParams) {
							expressionParams->bendRanges[BEND_RANGE_FINGER_LEVEL] = newBendRanges[BEND_RANGE_FINGER_LEVEL];
						}
					}
				}
...
void InstrumentClip::deleteEmptyNoteRowsAtEitherEnd(bool onlyIfNoDrum, ModelStackWithTimelineCounter* modelStack, bool mustKeepLastOne, bool keepOnesWithMIDIInput) {
	...
	for (int i = noteRows.getNumElements() - 1; i >= mustKeepLastOne; i--) {
		...
		if (!possiblyDeleteEmptyNoteRow(thisNoteRow, onlyIfNoDrum, modelStack->song, false, keepOnesWithMIDIInput)) break;
	...
	for (int i = 0; i < noteRows.getNumElements() - mustKeepLastOne; i++) {
		NoteRow* thisNoteRow = noteRows.getElement(i);
        if (!possiblyDeleteEmptyNoteRow(thisNoteRow, onlyIfNoDrum, modelStack->song, true, keepOnesWithMIDIInput)) break;
...
bool InstrumentClip::possiblyDeleteEmptyNoteRow(NoteRow* noteRow, bool onlyIfNoDrum, Song* song, bool onlyIfNonNumeric, bool keepIfHasMIDIInput) {
    ...
    if (drum) {
    	...
    	if (onlyIfNonNumeric && drum->type == DRUM_TYPE_SOUND && stringIsNumericChars(((SoundDrum*)drum)->name.get())) return false;
...
void InstrumentClip::assignDrumsToNoteRows(ModelStackWithTimelineCounter* modelStack, bool shouldGiveMIDICommandsToDrums, int numNoteRowsPreviouslyDeletedFromBottom) {
...
void InstrumentClip::unassignAllNoteRowsFromDrums(ModelStackWithTimelineCounter* modelStack, bool shouldRememberDrumNames, bool shouldRetainLinksToSounds, bool shouldGrabMidiCommands, bool shouldBackUpExpressionParamsToo) {
	for (int i = 0; i < noteRows.getNumElements(); i++) {
		NoteRow* thisNoteRow = noteRows.getElement(i);
    	if (thisNoteRow->drum) {
    		...
    		if (shouldRetainLinksToSounds) {
    			if (thisNoteRow->paramManager.containsAnyMainParamCollections()) {
    				modelStack->song->backUpParamManager((SoundDrum*)thisNoteRow->drum, this, &thisNoteRow->paramManager, shouldBackUpExpressionParamsToo);
    			}
    		}
...
int InstrumentClip::undoUnassignmentOfAllNoteRowsFromDrums(ModelStackWithTimelineCounter* modelStack) {
	for (int i = 0; i < noteRows.getNumElements(); i++) {
    	...
    	if (noteRow->drum && noteRow->drum->type == DRUM_TYPE_SOUND) {
    		...
    		bool success = modelStack->song->getBackedUpParamManagerPreferablyWithClip((SoundDrum*)noteRow->drum, this, &noteRow->paramManager);
...
void InstrumentClip::detachFromOutput(ModelStackWithTimelineCounter* modelStack, bool shouldRememberDrumNames, bool shouldDeleteEmptyNoteRowsAtEitherEnd, bool shouldRetainLinksToOutput, bool keepNoteRowsWithMIDIInput, bool shouldGrabMidiCommands, bool shouldBackUpExpressionParamsToo) {
	...
	if (output->type == INSTRUMENT_TYPE_MIDI_OUT) {
		if (paramManager.containsAnyMainParamCollections()) { // Wouldn't this always be? Or is there some case where we might be calling this just after it's been created, and no paramManager yet?
		...
		if (output->type == INSTRUMENT_TYPE_KIT) {
			...
			if (shouldDeleteEmptyNoteRowsAtEitherEnd) { // Only true when called from changeInstrument()
				deleteEmptyNoteRowsAtEitherEnd(false, modelStack, false, keepNoteRowsWithMIDIInput); // Might call audio routine (?). Will back up ParamManagers for any NoteRows deleted with Drums
				// That does not enforce keeping the last NoteRow. This is ok because we know if we're here that we're remaining a Kit
			}
			unassignAllNoteRowsFromDrums(modelStack, shouldRememberDrumNames, shouldRetainLinksToOutput, shouldGrabMidiCommands, shouldBackUpExpressionParamsToo);
		}

		modelStack->song->backUpParamManager((ModControllableAudio*)output->toModControllable(), this, &paramManager, shouldBackUpExpressionParamsToo);
	}
...
int InstrumentClip::undoDetachmentFromOutput(ModelStackWithTimelineCounter* modelStack) {
	...
	if (output->type == INSTRUMENT_TYPE_MIDI_OUT) {
		...
		ModelStackWithModControllable* modelStackWithModControllable = modelStack->addModControllableButNoNoteRow(output->toModControllable());
...
int InstrumentClip::setAudioInstrument(Instrument* newInstrument, Song* song, bool shouldSetupPatching, ParamManager* newParamManager, InstrumentClip* favourClipForCloningParamManager) {
...
someError:
    ...
    while (*(tagName = storageManager.readNextTagOrAttributeName())) {
        ...
        if (!strcmp(tagName, "inKeyMode")) {
        	...
        	output = song->getInstrumentFromPresetSlot(INSTRUMENT_TYPE_MIDI_OUT, instrumentPresetSlot, instrumentPresetSubSlot, NULL, NULL, false);
        	if (!output) {
        		output = storageManager.createNewNonAudioInstrument(INSTRUMENT_TYPE_MIDI_OUT, instrumentPresetSlot, instrumentPresetSubSlot);
...
{				void* instrumentMemory = generalMemoryAllocator.alloc(sizeof(SoundInstrument), NULL, false, true);
				...
				error = soundInstrument->dirPath.set("SYNTHS");			if (error) goto someError;	// Default, in case not included in file.
				output = soundInstrument;
}

loadInstrument:	error = output->readFromFile(song, this, readAutomationUpToPos);
        ...
        // For song files from before V2.0, where Instruments were stored within the Clip
            ...
            if (!output) {
            	...
            	error = kit->dirPath.set("KITS");			if (error) goto someError;	// Default, in case not included in file.
                output = kit;
            	goto loadInstrument;
            }
        }
    ...
    createNewParamManager:
        	...
        	// Slight hack to fix crash with late-2016-ish songs
        		...
        		ParamManager* otherParamManager = song->getBackedUpParamManagerPreferablyWithClip((ModControllableAudio*)output->toModControllable(), this);
...
doReadBendRange:
			...
			if (!paramManager.containsAnyMainParamCollections()) {
				...
				// If there wasn't one, that's because another Clip already took it. Clone it from that Clip.
					...
					if (!instrumentWasLoadedByReferenceFromClip && output->type == INSTRUMENT_TYPE_KIT) {
						...
						if (!instrumentWasLoadedByReferenceFromClip || !instrumentWasLoadedByReferenceFromClip->paramManager.containsAnyMainParamCollections()) {
							error = ERROR_FILE_CORRUPTED;
							goto someError;
						}
						error = paramManager.cloneParamCollectionsFrom(&instrumentWasLoadedByReferenceFromClip->paramManager, false); // No need to trim - param automation didn't exist back then
						if (error) goto someError;
					}
				}
			}
    ...
    if (storageManager.firmwareVersionOfFileBeingRead <= FIRMWARE_3P2P0_ALPHA && !paramManager.getExpressionParamSet()) {
    	ExpressionParamSet* expressionParams = paramManager.getOrCreateExpressionParamSet();
    	if (expressionParams) {
    ...
    case INSTRUMENT_TYPE_KIT:
    	...
    	if (dirPathHasBeenSpecified) {
    		...
    		error = backedUpInstrumentDirPath[instrumentTypeWhileLoading].set(getInstrumentFolder(instrumentTypeWhileLoading));		if (error) return error;
    	}
    	break;
...
expressionParam:
                		...
                		if (paramId < NUM_REAL_CC_NUMBERS) {
                			...
                			MIDIParam* midiParam = paramManager.getMIDIParamCollection()->params.getOrCreateParamFromCC(paramId, 0);
                	...
                	if (param) {
                		...
                		if (expressionParams) {
                			...
                			// Or if pitch bend, it'll no longer interpolate, so go place some new nodes. Actually even without this step, you can only just tell there's any problem.
                				...
                				param->makeInterpolationGoodAgain(loopLength, 22); // 22 is picked somewhat arbitrarily - see comment for function itself.
                			}
                		}
                	}
...
noteRowFailed: {}
    ...
    if (chosenNoteRow) {
    	if (chosenNoteRow->y < -256 || chosenNoteRow->y >= 256) goto useRootNote; // Can't use isScrollWithinRange, cos that relies on existing note positions, which are messed up
    }
    ...
    if (output->type == INSTRUMENT_TYPE_KIT) {
		...
		for (int i = 0; i < noteRows.getNumElements(); ) {
			...
			if ((!clipIsActive || noteRow->muted || noteRow->hasNoNotes())
					...
					&& (!noteRow->drum || noteRow->drum->type != DRUM_TYPE_SOUND || ((SoundDrum*)noteRow->drum)->skippingRendering)
					&& (!noteRow->drum || noteRow->drum->type != DRUM_TYPE_SOUND || (SoundDrum*)noteRow->drum != view.activeModControllableModelStack.modControllable)
			) {
				...
				if (clipIsActive && noteRow->drum) {
					...
					if (ALPHA_OR_BETA_VERSION && noteRow->drum->type == DRUM_TYPE_SOUND && ((SoundDrum*)noteRow->drum)->hasAnyVoices()) {
...
bool InstrumentClip::allowNoteTails(ModelStackWithNoteRow* modelStack) {
	...
	if (output->type == INSTRUMENT_TYPE_SYNTH) {
		ModelStackWithSoundFlags* modelStackWithSoundFlags = modelStack->addOtherTwoThings((SoundInstrument*)output, &paramManager)->addSoundFlags();
	...
	ModelStackWithSoundFlags* modelStackWithSoundFlags = modelStack->addOtherTwoThings(noteRow->drum->toModControllable(), &noteRow->paramManager)->addSoundFlags();
	return noteRow->drum->allowNoteTails(modelStackWithSoundFlags); // Needs to survive a NULL noteRow, even if this generally wouldn't happen (it might if auditioning a Drum via MIDI or arranger audition pad which doesn't have one)
}


// What does this do exactly, again?
void InstrumentClip::ensureInaccessibleParamPresetValuesWithoutKnobsAreZero(ModelStackWithTimelineCounter* modelStack, Sound* sound) {
	if (output) {
		if (output->type == INSTRUMENT_TYPE_SYNTH) {
			if ((SoundInstrument*)output == sound) {

				ModelStackWithThreeMainThings* modelStackWithThreeMainThings = modelStack->addOtherTwoThingsButNoNoteRow(sound, &paramManager);
			...
			for (int i = 0; i < noteRows.getNumElements(); i++) {
				...
				if (thisNoteRow->drum == soundDrum) {
					...
					ModelStackWithThreeMainThings* modelStackWithThreeMainThings = modelStack->addNoteRow(i, thisNoteRow)->addOtherTwoThings(sound, &thisNoteRow->paramManager);
...
void InstrumentClip::shiftOnlyOneNoteRowHorizontally(ModelStackWithNoteRow* modelStack, int shiftAmount) {
	...
	if (playbackHandler.isEitherClockActive() && modelStack->song->isClipActive(this)) {
		...
		if (noteRow->paramManager.mightContainAutomation()) {
    		...
    		noteRow->paramManager.setPlayPos(getLivePos(), modelStackWithThreeMainThings, modelStackWithThreeMainThings->isCurrentlyPlayingReversed());
		}
	}
}
...
void InstrumentClip::clearArea(ModelStackWithTimelineCounter* modelStack, int32_t startPos, int32_t endPos, Action* action) {
	for (int i = 0; i < noteRows.getNumElements(); i++) {
		NoteRow* thisNoteRow = noteRows.getElement(i);
		...
		thisNoteRow->clearArea(startPos, endPos - startPos, modelStackWithNoteRow, action, loopLength); // No cross-screen
	}
}
...
void InstrumentClip::compensateVolumeForResonance(ModelStackWithTimelineCounter* modelStack) {
	((Instrument*)output)->compensateInstrumentVolumeForResonance(modelStack->addOtherTwoThingsButNoNoteRow(output->toModControllable(), &paramManager));
	...
	if (output->type == INSTRUMENT_TYPE_KIT) {
		...
		for (int i = 0; i < noteRows.getNumElements(); i++) {
			...
			if (thisNoteRow->drum && thisNoteRow->paramManager.containsAnyMainParamCollections() && thisNoteRow->drum->type == DRUM_TYPE_SOUND) {
				SoundDrum* thisDrum = (SoundDrum*)thisNoteRow->drum;
				ModelStackWithThreeMainThings* modelStackWithThreeMainThings = modelStack->addNoteRow(i, thisNoteRow)->addOtherTwoThings(thisDrum, &thisNoteRow->paramManager);
...
Instrument* InstrumentClip::changeInstrumentType(ModelStackWithTimelineCounter* modelStack, int newInstrumentType) {
	...
	if (newInstrumentType == INSTRUMENT_TYPE_MIDI_OUT || newInstrumentType == INSTRUMENT_TYPE_CV) {
		newInstrument = modelStack->song->getNonAudioInstrumentToSwitchTo(newInstrumentType, availabilityRequirement, newSlot, newSubSlot, &instrumentAlreadyInSong);
	...
	// Synth / Kit
		...
		if (Browser::currentDir.isEmpty()) {
			...
			if (result.error) {
displayError:	numericDriver.displayError(result.error);
				return NULL;
			}
		}
		...
		if (!newInstrument) {
			...
			result.error = storageManager.loadInstrumentFromFile(modelStack->song, NULL, newInstrumentType, false, &newInstrument, &result.fileItem->filePointer, &newPresetName, &Browser::currentDir);
		}
	...
	if (shouldReplaceWholeInstrument) {
		...
		int error = changeInstrument(modelStack, newInstrument, NULL, INSTRUMENT_REMOVAL_DELETE_OR_HIBERNATE_IF_UNUSED, NULL, true);
...
int InstrumentClip::claimOutput(ModelStackWithTimelineCounter* modelStack) {
	...
	if (!output) { // Would only have an output already if file from before V2.0.0 I think? So, this block normally does apply.
		...
		output = modelStack->song->getInstrumentFromPresetSlot(
				instrumentType,
				backedUpInstrumentSlot[instrumentType],
    ...
    if (output->type == INSTRUMENT_TYPE_KIT) {
    	...
    	for (int i = 0; i < noteRows.getNumElements(); i++) {
        	...
        	// Otherwise, we know we've sneakily put an integer index in place of the pointer, so convert that back to an actual pointer now
        		...
        		ModelStackWithNoteRow* modelStackWithNoteRow; // Defined up here so we can do the jump to haveNoDrum, below.
        		...
        		if (!thisNoteRow->paramManager.containsAnyMainParamCollections() && thisNoteRow->drum->type == DRUM_TYPE_SOUND) {

        			modelStackWithNoteRow = modelStack->addNoteRow(i, thisNoteRow);

        			// Try grabbing the Drum's "backed up" one
        			success = modelStackWithNoteRow->song->getBackedUpParamManagerPreferablyWithClip((SoundDrum*)thisNoteRow->drum, this, &thisNoteRow->paramManager);
        			if (success) {
        			...
        			// If there wasn't one there, it means another Clip's NoteRow already claimed it
        				...
        				otherParamManager = modelStackWithNoteRow->song->findParamManagerForDrum(kit, thisNoteRow->drum, this);
        		    		...
        		    		for (int s = 0; s < NUM_SOURCES; s++) {
        		    			...
        		    			if (source->oscType == OSC_TYPE_SAMPLE) {
        		    				if (	sound->transpose
        		    						...
        		    						|| patchedParams->params[PARAM_LOCAL_OSC_A_PITCH_ADJUST + s].containsSomething(0)
        		    						) {
        	...
        	if (output->type == INSTRUMENT_TYPE_SYNTH) {
        		...
        		for (int s = 0; s < NUM_SOURCES; s++) {
					if (sound->sources[s].oscType != OSC_TYPE_SQUARE) {
						...
						ModelStackWithParamCollection* modelStackWithParamCollection = modelStackWithThreeMainThings
								->addParamCollection(patchedParams, patchedParamsSummary);

						patchedParams->deleteAutomationForParamBasicForSetup(modelStackWithParamCollection, PARAM_LOCAL_OSC_A_PHASE_WIDTH + s);
						patchedParams->params[PARAM_LOCAL_OSC_A_PHASE_WIDTH + s].setCurrentValueBasicForSetup(0);
						patchedCables->removeAllPatchingToParam(modelStackWithParamCollection, PARAM_LOCAL_OSC_A_PHASE_WIDTH + s);
					}
        		}
        	}
        }
...
void InstrumentClip::finishLinearRecording(ModelStackWithTimelineCounter* modelStack, Clip* nextPendingLoop, int buttonLatencyForTempolessRecord) {
	...
	for (int i = 0; i < noteRows.getNumElements(); i++) {
		...
		while (thisNoteRow->notes.getNumElements()) { // There's most likely only one offender, but you never really know
			...
			if (lastNote->pos >= loopLength) {
				...
				if (newInstrumentClip) {
					...
					if (output->type == INSTRUMENT_TYPE_KIT) {
						...
						modelStackWithNoteRow = newInstrumentClip->getOrCreateNoteRowForYNote(thisNoteRow->y, modelStack);
					}

					NoteRow* newNoteRow = modelStackWithNoteRow->getNoteRowAllowNull();
					if (newNoteRow) {
						newNoteRow->attemptNoteAdd(0, lastNote->length, lastNote->velocity, lastNote->probability, modelStackWithNoteRow, NULL); // I'm guessing I deliberately didn't send the Action in here, cos didn't want to make this Note on the new InstrumentClip undoable?
						newNoteRow->skipNextNote = true;
					}
				}

				// Delete the Note
				thisNoteRow->deleteNoteByIndex(thisNoteRow->notes.getNumElements() - 1, NULL, getNoteRowId(thisNoteRow, i), this);
			}
			...
			// Or if Note not past end-point...
				...
				if (mayStillLengthen && ((Instrument*)output)->isNoteRowStillAuditioningAsLinearRecordingEnded(thisNoteRow)) {

					if (!action) action = actionLogger.getNewAction(ACTION_RECORD, true);
					int noteRowId = getNoteRowId(thisNoteRow, i);

					if (action) action->recordNoteArrayChangeIfNotAlreadySnapshotted(this, noteRowId, &thisNoteRow->notes, false, true); // This has probably already been done
	...
	if (getRootUI()) getRootUI()->clipNeedsReRendering(this); // Notes might have been lengthened - we'd better render it.
}
...
ramError:
	...
	newInstrumentClip->setupForRecordingAsAutoOverdub(this, modelStack->song, newOverdubNature); // Hopefully fine - I've moved this to after setInstrument in March 2021, so we can override the new affectEntire default value set there.

	char modelStackMemoryNewClip[MODEL_STACK_MAX_SIZE];
	ModelStackWithTimelineCounter* modelStackNewClip = setupModelStackWithTimelineCounter(modelStackMemoryNewClip, modelStack->song, newInstrumentClip);

	newInstrumentClip->setupAsNewKitClipIfNecessary(modelStackNewClip);

	// If Kit, copy NoteRow colours
	if (output->type == INSTRUMENT_TYPE_KIT && noteRows.getNumElements() == newInstrumentClip->noteRows.getNumElements()) {
		for (int i = 0; i < noteRows.getNumElements(); i++) {
...
void InstrumentClip::quantizeLengthForArrangementRecording(ModelStackWithTimelineCounter* modelStack, int32_t lengthSoFar, uint32_t timeRemainder, int32_t suggestedLength, int32_t alternativeLongerLength) {
...
void InstrumentClip::getActiveModControllable(ModelStackWithTimelineCounter* modelStack) {
	if (output->type == INSTRUMENT_TYPE_KIT && !affectEntire && getRootUI() != &sessionView && getRootUI() != &arrangerView) {
		Kit* kit = (Kit*)output;
...
returnNull:
			...
			modelStack->addNoteRow(noteRowIndex, noteRow)->addOtherTwoThings((SoundDrum*)kit->selectedDrum, &noteRow->paramManager);
		}
	}
...
ModelStackWithNoteRow* InstrumentClip::duplicateModelStackForClipBeingRecordedFrom(ModelStackWithNoteRow* modelStack, char* otherModelStackMemory) {
...
void InstrumentClip::recordNoteOn(ModelStackWithNoteRow* modelStack, int velocity, bool forcePos0, int16_t const* mpeValuesOrNull, int fromMIDIChannel) {
    ...
    if (forcePos0) {
		...
		if (FlashStorage::recordQuantizeLevel) {
			...
			uint16_t quantizeInterval = baseThing << (8 + modelStack->song->insideWorldTickMagnitude + modelStack->song->insideWorldTickMagnitudeOffsetFromBPM - FlashStorage::recordQuantizeLevel);
			...
			if (quantizedLater != reversed) { // If need to quantize forwards (to later in time)...
				...
				if (quantizedPos >= effectiveLength) {
					...
					if (playbackHandler.recording == RECORDING_ARRANGEMENT && isArrangementOnlyClip()) {
						...
						if (noteRow->loopLengthIfIndependent) {
							if (output->type == INSTRUMENT_TYPE_KIT && noteRows.getNumElements() != ((InstrumentClip*)beingRecordedFromClip)->noteRows.getNumElements()) {
								error = ERROR_UNSPECIFIED;
							}

							else {
								char otherModelStackMemory[MODEL_STACK_MAX_SIZE];
								ModelStackWithNoteRow* otherModelStackWithNoteRow = duplicateModelStackForClipBeingRecordedFrom(modelStack, otherModelStackMemory);

								NoteRow* otherNoteRow = otherModelStackWithNoteRow->getNoteRowAllowNull();
								if (otherNoteRow) { // It "should" always have it...

									int whichRepeatThisIs = (uint32_t)noteRow->loopLengthIfIndependent / (uint32_t)otherNoteRow->loopLengthIfIndependent;
									noteRow->appendNoteRow(modelStack, otherModelStackWithNoteRow, noteRow->loopLengthIfIndependent, whichRepeatThisIs, otherNoteRow->loopLengthIfIndependent);
						...
						// Otherwise, just extend the whole Clip.
							...
							ModelStackWithTimelineCounter* thisModelStack = (ModelStackWithTimelineCounter*) thisModelStackMemory;

							char otherModelStackMemory[MODEL_STACK_MAX_SIZE];
							ModelStackWithTimelineCounter* otherModelStack = setupModelStackWithSong(otherModelStackMemory, modelStack->song)->addTimelineCounter(beingRecordedFromClip);
...
doNormal:				// Wrap it back to the start.
	...
	Action* action = actionLogger.getNewAction(ACTION_RECORD, true);
	if (action) action->recordNoteArrayChangeIfNotAlreadySnapshotted(this, modelStack->noteRowId, &noteRow->notes, false, true);
		// moveToFrontIfAlreadySnapshotted = true because we need to make the Consequence closer to the front than any previous Clip-lengthening that took place.
	...
	if (reversed) {
		...
		distanceToNextNote = noteRow->attemptNoteAdd(quantizedPos, 1, velocity, NUM_PROBABILITY_VALUES, modelStack, NULL); // Don't supply Action, cos we've done our own thing, above
	}
...
doHomogenize:
#if ALPHA_OR_BETA_VERSION
			param->nodes.testSequentiality("E442"); // drbourbon got, when check was inside homogenizeRegion(). Now trying to work out where that came from. March 2022.
#endif

			param->homogenizeRegion(modelStackWithAutoParam, quantizedPos, distanceToNextNote, value, reversed, reversed,
					effectiveLength, reversed, posAtWhichClipWillCut);
		}

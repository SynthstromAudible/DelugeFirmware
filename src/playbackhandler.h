...
public:
    ...
    int64_t nextTimerTickScheduled;	// *Yes* valid while (internal clock) playback being set up. Will be zero during that time
    ...
    unsigned long timeVeryFirstInputTick; // Very first tick received in playback. Only used for tempo magnitude matching
    ...
    void aftertouchReceived(MIDIDevice* fromDevice, int channel, int value, int noteCode, bool* doingMidiThru); // noteCode -1 means channel-wide
    ...
    inline bool isEitherClockActive() {

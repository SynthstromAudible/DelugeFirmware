...
private:
	...
	void getColBarPositions(int xDisplay, WaveformRenderData* data, int32_t* min24, int32_t* max24, int32_t valueCentrePoint, int32_t valueSpan);
	void drawColBar(int xDisplay, int32_t min24, int32_t max24, uint8_t thisImage[][displayWidth + sideBarWidth][3], int brightness = 128, uint8_t rgb[] = NULL);
	void renderOneColForCollapseAnimationInterpolation(int xDisplayOutput, int32_t min24, int32_t max24, int singleSquareBrightness, int progress, uint8_t thisImage[][displayWidth + sideBarWidth][3], uint8_t rgb[]);
};

...
public:
    ...
    virtual MenuItem* patchingSourceShortcutPress(int s, bool previousPressStillActive = false) { return NULL; } // NULL means do nothing. 0xFFFFFFFF means go up a level
    ...
    virtual bool learnNoteOn(MIDIDevice* fromDevice, int channel, int noteCode) { return false; }  // Returns whether it was used, I think?
	...
	char const* getTitle();//char* buffer);	// Supplied buffer size must be MENU_ITEM_TITLE_BUFFER_SIZE. Actual max num chars for OLED display is 14.

...
int32_t oscSyncRenderingBuffer[SSI_TX_BUFFER_NUM_SAMPLES + 4] __attribute__ ((aligned (CACHE_LINE_SIZE))); // Hopefully I could make this use the spareRenderingBuffer instead...
...
bool Voice::noteOn(ModelStackWithVoice* modelStack, int newNoteCodeBeforeArpeggiation, int newNoteCodeAfterArpeggiation, uint8_t velocity, uint32_t newSampleSyncLength, int32_t ticksLate, uint32_t samplesLate, bool resetEnvelopes, int newFromMIDIChannel, const int16_t* mpeValues) {
    ...
    sourceValues[PATCH_SOURCE_VELOCITY] = (velocity == 128) ?
    		2147483647 :
    		((int32_t)velocity - 64) * 33554432;
    ...
    // Porta
    if (sound->polyphonic != POLYPHONY_LEGATO && paramManager->getUnpatchedParamSet()->getValue(PARAM_UNPATCHED_PATCHINGCONFIG_PORTA) != -2147483648 && sound->lastNoteCode != -2147483648) {
    	setupPorta(sound);
    }
			...
			if (sound->getSynthMode() != SYNTH_MODE_FM
				...
				if (sound->sources[s].oscType == OSC_TYPE_SAMPLE) {
					...
					if (sound->sources[s].repeatMode == SAMPLE_REPEAT_STRETCH) {
						...
						guides[s].sequenceSyncStartedAtTick = playbackHandler.lastSwungTickActioned - ticksLate;	// No harm setting this even if it's not valid or needed.
    ...
    for (int s = 0; s < NUM_SOURCES; s++) {
		...
		for (int u = 0; u < sound->numUnison; u++) {
			...
			if (unisonParts[u].sources[s].active) {
				bool success = unisonParts[u].sources[s].noteOn(this, source, &guides[s], samplesLate, sound->oscRetriggerPhase[s], resetEnvelopes, sound->synthMode);
	...
	if (sound->getSynthMode() == SYNTH_MODE_FM) {
		...
		for (int u = 0; u < sound->numUnison; u++) {
			for (int m = 0; m < numModulators; m++) {
				if (sound->modulatorRetriggerPhase[m] != 0xFFFFFFFF) unisonParts[u].modulatorPhase[m] = initialPhase + sound->modulatorRetriggerPhase[m];
...
void Voice::changeNoteCode(ModelStackWithVoice* modelStack, int newNoteCodeBeforeArpeggiation, int newNoteCodeAfterArpeggiation, int newInputMIDIChannel, const int16_t* newMPEValues) {
	inputCharacteristics[MIDI_CHARACTERISTIC_NOTE]		= newNoteCodeBeforeArpeggiation;
...
void Voice::setupPorta(Sound* patchingConfig) {
	portaEnvelopePos = 0;
	int semitoneAdjustment = patchingConfig->lastNoteCode - noteCodeAfterArpeggiation;
			...
			if (sound->sources[s].oscType == OSC_TYPE_SAMPLE && guides[s].loopEndPlaybackAtByte) {
				for (int u = 0; u < sound->numUnison; u++) {
					if (unisonParts[u].sources[s].active) {
						...
						bool success = unisonParts[u].sources[s].voiceSample->noteOffWhenLoopEndPointExists(this, &guides[s]);
...
bool Voice::sampleZoneChanged(ModelStackWithVoice* modelStack, int s, int markerType) {
	...
	if (!holder) return true; // If no holder, that means this Source/Sample is not currently playing, e.g. because its volume was set to 0.
	...
	for (int u = 0; u < sound->numUnison; u++) {
		...
		if (voiceUnisonPartSource->active) {
			bool stillActive = voiceUnisonPartSource->voiceSample->sampleZoneChanged(&guides[s], sample, markerType, loopingType, getPriorityRating());
			if (!stillActive) {
	...
	if (!anyStillActive && !paramManager->getPatchedParamSet()->params[PARAM_LOCAL_NOISE_VOLUME].containsSomething(-2147483648)) {
...
bool Voice::render(ModelStackWithVoice* modelStack, int32_t* soundBuffer, int numSamples, bool soundRenderingInStereo, bool applyingPanAtVoiceLevel, uint32_t sourcesChanged, FilterSetConfig* filterSetConfig, int32_t externalPitchAdjust) {
    ...
    for (int e = 0; e < numEnvelopes; e++) {
        if (e == 0 || (paramManager->getPatchCableSet()->sourcesPatchedToAnything[GLOBALITY_LOCAL] & (1 << (PATCH_SOURCE_ENVELOPE_0 + e)))) {
            ...
            sourceValues[PATCH_SOURCE_ENVELOPE_0 + e] = envelopes[e].render(
            		numSamples,
					paramFinalValues[PARAM_LOCAL_ENV_0_ATTACK + e],
					paramFinalValues[PARAM_LOCAL_ENV_0_DECAY + e],
					paramFinalValues[PARAM_LOCAL_ENV_0_SUSTAIN + e],
					release,
					decayTableSmall8);
    ...
    bool unassignVoiceAfter = (envelopes[0].state == ENVELOPE_STAGE_OFF);//(envelopes[0].state >= ENVELOPE_STAGE_DECAY && localSourceValues[PATCH_SOURCE_ENVELOPE_0 - FIRST_LOCAL_SOURCE] == -2147483648);
    // Local LFO
    if (paramManager->getPatchCableSet()->sourcesPatchedToAnything[GLOBALITY_LOCAL] & (1 << PATCH_SOURCE_LFO_LOCAL)) {
        ...
        sourceValues[PATCH_SOURCE_LFO_LOCAL] = lfo.render(numSamples, sound->lfoLocalWaveType, paramFinalValues[PARAM_LOCAL_LFO_LOCAL_FREQ]);
	...
	if (portaEnvelopePos < 8388608) {
		...
		int32_t pitchAdjustmentHere = 16777216 + (multiply_32x32_rshift32_rounded(envValue, portaEnvelopeMaxAmplitude) << 1);
    ...
    if (	sound->getSynthMode() != SYNTH_MODE_FM
    		&& envelopes[0].state < ENVELOPE_STAGE_RELEASE
    		&& hasReleaseStage()
    	...
    	for (int s = 0; s < NUM_SOURCES; s++) {
    		...
    		if (	source->oscType != OSC_TYPE_SAMPLE
    				|| source->repeatMode != SAMPLE_REPEAT_ONCE // Don't do it for anything else. STRETCH is too hard to calculate
    	...
    	if (whichSourcesNeedAttention) {
        	...
        	for (int s = 0; s < NUM_SOURCES; s++) {
        		...
        		if (whichSourcesNeedAttention & (1 << s)) {
            		...
            		if (!adjustPitch(&actualSampleReadRate, paramFinalValues[PARAM_LOCAL_OSC_A_PITCH_ADJUST + s])) continue;
    ...
    int32_t overallOscAmplitude = lshiftAndSaturate(multiply_32x32_rshift32(paramFinalValues[PARAM_LOCAL_VOLUME], (sourceValues[PATCH_SOURCE_ENVELOPE_0] >> 1) + 1073741824), 2);

    int32_t filterGain; // This is the gain which gets applied to compensate for any change in gain that the filter is going to cause


	// Prepare the filters
    if (sound->hasFilters()) {

    	filterGain = filterSetConfig->init(paramFinalValues[PARAM_LOCAL_LPF_FREQ],
    			paramFinalValues[PARAM_LOCAL_LPF_RESONANCE],
	...
	if (synthMode != SYNTH_MODE_RINGMOD) {
		...
		if (synthMode == SYNTH_MODE_FM) {
			...
			overallOscAmplitude = multiply_32x32_rshift32_rounded(overallOscAmplitude, sound->volumeNeutralValueForUnison) << 3;
		...
		// Or if subtractive, we don't do that, because we do want to apply the overall amplitude *after* the filter
            ...
            if (sound->hasFilters()) {
            	sourceAmplitudes[0] = multiply_32x32_rshift32_rounded(paramFinalValues[PARAM_LOCAL_OSC_A_VOLUME], filterGain);
            	sourceAmplitudes[1] = multiply_32x32_rshift32_rounded(paramFinalValues[PARAM_LOCAL_OSC_B_VOLUME], filterGain);
            }
            else {
		...
		bool shouldAvoidIncrementing = doneFirstRender ?
        ...
        if (synthMode == SYNTH_MODE_FM) {
            for (int m = 0; m < numModulators; m++) {
                modulatorsActive[m] = (paramFinalValues[PARAM_LOCAL_MODULATOR_0_VOLUME + m] != 0 || modulatorAmplitudeLastTime[m] != 0);

                if (modulatorsActive[m]) {
                	modulatorAmplitudeIncrements[m] = (int32_t)(paramFinalValues[PARAM_LOCAL_MODULATOR_0_VOLUME + m] - modulatorAmplitudeLastTime[m]) / numSamples;
                }
            }
        }
	}
	...
	if (synthMode != SYNTH_MODE_FM) {
        ...
        for (int s = 0; s < NUM_SOURCES; s++) {
        	sourceWaveIndexIncrements[s] = (int32_t)(paramFinalValues[PARAM_LOCAL_OSC_A_WAVE_INDEX + s] - sourceWaveIndexesLastTime[s]) / numSamples;
        }
	}
...
decidedWhichBufferRenderingInto:
	...
	if (renderingStraightIntoSoundBuffer) {
		...
		if (synthMode == SYNTH_MODE_SUBTRACTIVE) {
			for (int s = 0; s < NUM_SOURCES; s++) {
				sourceAmplitudeIncrements[s] = (multiply_32x32_rshift32(sourceAmplitudeIncrements[s], overallOscAmplitudeLastTime)
						+ multiply_32x32_rshift32(overallOscillatorAmplitudeIncrement, sourceAmplitudesNow[s])) << 1;

				sourceAmplitudesNow[s] = multiply_32x32_rshift32(sourceAmplitudesNow[s], overallOscAmplitudeLastTime) << 1;
			}
		}
	}
	...
	// Or if rendering to local Voice buffer, We need to do some other setting up - like wiping the buffer clean first
	    ...
	    if (!applyingPanAtVoiceLevel) {
			...
			// Set up panning
			doPanning = (AudioEngine::renderInStereo && shouldDoPanning(paramFinalValues[PARAM_LOCAL_PAN], &amplitudeL, &amplitudeR));
	    }
	}
    ...
    if (synthMode == SYNTH_MODE_SUBTRACTIVE) {
    	...
    	for (int s = 0; s < NUM_SOURCES; s++) {
    		...
    		if (!sound->sources[s].renderInStereo((SampleHolder*)guides[s].audioFileHolder)) {
    			renderBasicSource(sound, paramManager, s, oscBuffer, numSamples, sourceAmplitudesNow[s], &unisonPartBecameInactive, overallPitchAdjust, (s == 1) && doingOscSync, oscSyncPos, oscSyncPhaseIncrement, sourceAmplitudeIncrements[s], getPhaseIncrements, getOutAfterGettingPhaseIncrements, sourceWaveIndexIncrements[s]);
    			anythingInOscBuffer = true;
    		}
    	...
    	if (sourcesToRenderInStereo) {
        	...
        	for (int s = 0; s < NUM_SOURCES; s++) {
        		if (sourcesToRenderInStereo & (1 << s)) {
        			renderBasicSource(sound, paramManager, s, oscBuffer, numSamples, sourceAmplitudesNow[s], &unisonPartBecameInactive, overallPitchAdjust, false, 0, 0, sourceAmplitudeIncrements[s], NULL, false, sourceWaveIndexIncrements[s]);
        		}
        	}
        	...
        	if (!renderingStraightIntoSoundBuffer) {
				...
				if (!sound->clippingAmount) {
					...
					do {
						...
						if (doPanning) outputSample->addPannedStereo(outputSampleL, outputSampleR, amplitudeL, amplitudeR);
    	...
    	if (unisonPartBecameInactive && areAllUnisonPartsInactive(modelStack)) {
    		...
    		// Otherwise, must do a fast-release to avoid a click
    			...
    			if (envelopes[0].state < ENVELOPE_STAGE_FAST_RELEASE) envelopes[0].unconditionalRelease(ENVELOPE_STAGE_FAST_RELEASE);
    		}
    	}
...
cantBeDoingOscSyncForFirstOsc:
				    ...
				    int32_t pulseWidth = (uint32_t)lshiftAndSaturate(paramFinalValues[PARAM_LOCAL_OSC_A_PHASE_WIDTH + s], 1);

				    int oscType = sound->sources[s].oscType;

					renderOsc(s, oscType, 0, spareRenderingBuffer[s + 1], spareRenderingBuffer[s + 1] + numSamples, numSamples, phaseIncrements[s], pulseWidth, &unisonParts[u].sources[s].oscPos, false, 0, doingOscSyncThisOscillator, oscSyncPos[u], phaseIncrements[0], sound->oscRetriggerPhase[s], sourceWaveIndexIncrements[s]);
			...
			// Or if FM
                ...
                for (int m = 0; m < numModulators; m++) {
                	...
                	if (phaseIncrementModulator[m] == 0xFFFFFFFF) modulatorsActive[m] = false; // If frequency marked as too high
                }
                ...
                for (int m = 0; m < numModulators; m++) {
                	if (modulatorsActive[m]) {
                		if (!adjustPitch(&phaseIncrementModulator[m], paramFinalValues[PARAM_LOCAL_MODULATOR_0_PITCH_ADJUST + m])) {
                			modulatorsActive[m] = false;
                		}
                    }
                }
                ...
                if (modulatorsActive[1]) {
                	...
                	// Render mod1
					renderSineWaveWithFeedback(spareRenderingBuffer[1], numSamples, &unisonParts[u].modulatorPhase[1], modulatorAmplitudeLastTime[1], phaseIncrementModulator[1], paramFinalValues[PARAM_LOCAL_MODULATOR_1_FEEDBACK], &unisonParts[u].modulatorFeedback[1], false, modulatorAmplitudeIncrements[1]);

					// If mod1 is modulating mod0...
					if (sound->modulator1ToModulator0) {
						// .. render modulator0, receiving the FM from mod1
						renderFMWithFeedback(spareRenderingBuffer[1], numSamples, NULL, &unisonParts[u].modulatorPhase[0], modulatorAmplitudeLastTime[0], phaseIncrementModulator[0], paramFinalValues[PARAM_LOCAL_MODULATOR_0_FEEDBACK], &unisonParts[u].modulatorFeedback[0], modulatorAmplitudeIncrements[0]);
					}

					// Otherwise, so long as modulator0 is in fact active, render it separately and add it
					else if (modulatorsActive[0]) {
						renderSineWaveWithFeedback(spareRenderingBuffer[1], numSamples, &unisonParts[u].modulatorPhase[0], modulatorAmplitudeLastTime[0], phaseIncrementModulator[0], paramFinalValues[PARAM_LOCAL_MODULATOR_0_FEEDBACK], &unisonParts[u].modulatorFeedback[0], true, modulatorAmplitudeIncrements[0]);
					}
                }
                else {
                	if (modulatorsActive[0]) {
                		renderSineWaveWithFeedback(spareRenderingBuffer[1], numSamples, &unisonParts[u].modulatorPhase[0], modulatorAmplitudeLastTime[0], phaseIncrementModulator[0], paramFinalValues[PARAM_LOCAL_MODULATOR_0_FEEDBACK], &unisonParts[u].modulatorFeedback[0], false, modulatorAmplitudeIncrements[0]);
                	}
                	else {
                    	noModulatorsActive:
                        for (int s = 0; s < NUM_SOURCES; s++) {
                        	if (sourceAmplitudes[s]) {
                       			renderSineWaveWithFeedback(oscBuffer, numSamples, &unisonParts[u].sources[s].oscPos, sourceAmplitudesNow[s], phaseIncrements[s], paramFinalValues[PARAM_LOCAL_CARRIER_0_FEEDBACK + s], &unisonParts[u].sources[s].carrierFeedback, true, sourceAmplitudeIncrements[s]);
                        	}
                        }
                ...
                for (int s = 0; s < NUM_SOURCES; s++) {
                	if (sourceAmplitudes[s]) {
               			renderFMWithFeedbackAdd(oscBuffer, numSamples, spareRenderingBuffer[1], &unisonParts[u].sources[s].oscPos, sourceAmplitudesNow[s], phaseIncrements[s], paramFinalValues[PARAM_LOCAL_CARRIER_0_FEEDBACK + s], &unisonParts[u].sources[s].carrierFeedback, sourceAmplitudeIncrements[s]);
                	}
                }
...
bool Voice::areAllUnisonPartsInactive(ModelStackWithVoice* modelStack) {
	...
	if (!modelStack->paramManager->getPatchedParamSet()->params[PARAM_LOCAL_NOISE_VOLUME].containsSomething(-2147483648)) {
...
#define fmVectorLoopComponent(i) { \
	...
	int16x4_t strength2 = vreinterpret_s16_u16(vshr_n_u16(vshrn_n_u32(finalPhase, (32 - 16 - SINE_TABLE_SIZE_MAGNITUDE)), 1));
...
void Voice::renderSineWaveWithFeedback(int32_t* bufferStart, int numSamples, uint32_t* phase, int32_t amplitude, uint32_t phaseIncrement, int32_t feedbackAmount, int32_t* lastFeedbackValue, bool add, int32_t amplitudeIncrement) {
...
void Voice::renderFMWithFeedback(int32_t* bufferStart, int numSamples, int32_t* fmBuffer, uint32_t* phase, int32_t amplitude, uint32_t phaseIncrement, int32_t feedbackAmount, int32_t* lastFeedbackValue, int32_t amplitudeIncrement) {
...
void Voice::renderFMWithFeedbackAdd(int32_t* bufferStart, int numSamples, int32_t* fmBuffer, uint32_t* phase, int32_t amplitude, uint32_t phaseIncrement, int32_t feedbackAmount, int32_t* lastFeedbackValue, int32_t amplitudeIncrement) {
...
pitchTooHigh:
		...
		if (patchingConfig->sources[s].oscType == OSC_TYPE_SAMPLE) {
			...
			if (!(getNoise() >> 19)) {
				...
				if (r < 128) {
					...
					if ((guides[s].audioFileHolder->transpose || guides[s].audioFileHolder->cents) && getRandom255() < 128) {
	    	...
	    	if (phaseIncrement != 16777216) {
				...
				if (!voiceSample->doneFirstRenderYet
						&& !tryToStartMidNote
					...
					if (loopingType) {
						...
						uint64_t phaseIncrementTimesTimeStretchRatio = ((uint64_t)(uint32_t)phaseIncrement * (uint32_t)timeStretchRatio) >> 24;
						int loopLengthCached = ((uint64_t)(uint32_t)loopLength << 24) / phaseIncrementTimesTimeStretchRatio;
					...
					for (int c = 0; c < paramManager->getPatchCableSet()->numUsablePatchCables; c++) {
						...
						if (cable->destinationParamDescriptor.isSetToParamWithNoSource(PARAM_LOCAL_PITCH_ADJUST) || cable->destinationParamDescriptor.isSetToParamWithNoSource(PARAM_LOCAL_OSC_A_PITCH_ADJUST + s)) {

							// And if it's an envelope or LFO or random...
							if (cable->from == PATCH_SOURCE_ENVELOPE_0 || cable->from == PATCH_SOURCE_ENVELOPE_1 || cable->from == PATCH_SOURCE_LFO_GLOBAL || cable->from == PATCH_SOURCE_LFO_LOCAL || cable->from == PATCH_SOURCE_RANDOM) {
								goto dontUseCache;
							}
			...
			// No pitch shifting
				...
				if (patchingConfig->sources[s].oscType != OSC_TYPE_INPUT_STEREO || (!AudioEngine::lineInPluggedIn && !AudioEngine::micPluggedIn)) {
					...
					do {
						...
						*(oscBufferPos++) += multiply_32x32_rshift32(inputReadPos[channelOffset], sourceAmplitudeNow) << 4;
		...
		// Or regular wave
			...
			int32_t* oscBufferEnd = oscBuffer + numSamples; // TODO: we don't really want to be calculating this so early do we?
...
void renderPDWave(const int16_t* table, const int16_t* secondTable, int numBitsInTableSize, int numBitsInSecondTableSize, int32_t amplitude, int32_t* thisSample, int32_t* bufferEnd, int numSamplesRemaining, uint32_t phaseIncrementNow, uint32_t* thisPhase,
		bool applyAmplitude, bool doOscSync, uint32_t resetterPhase, uint32_t resetterPhaseIncrement, uint32_t resetterHalfPhaseIncrement, uint32_t resetterLower,
		int32_t resetterDivideByPhaseIncrement, uint32_t pulseWidth, uint32_t phaseToAdd, uint32_t retriggerPhase, uint32_t horizontalOffsetThing, int32_t amplitudeIncrement,
	...
	do {
		...
		*thisPhase += phaseIncrementEachHalf[whichHalfBefore];
		...
		int32_t value = waveValueFunction(eachTable[whichHalfAfter], eachTableSize[whichHalfAfter], *thisPhase, pulseWidth, phaseToAdd);
...
const int16_t* analogSquareTables[] = {analogSquare_1722, analogSquare_1217, analogSquare_861, analogSquare_609, analogSquare_431, analogSquare_305, analogSquare_215,
		analogSquare_153, analogSquare_109, analogSquare_76, analogSquare_53, analogSquare_39, analogSquare_27, analogSquare_19, analogSquare_13, analogSquare_9,
...
const int16_t* analogSawTables[] = {mysterySynthASaw_1722, mysterySynthASaw_1217, mysterySynthASaw_861, mysterySynthASaw_609, mysterySynthASaw_431, mysterySynthASaw_305,
		mysterySynthASaw_215, mysterySynthASaw_153, mysterySynthBSaw_109, mysterySynthBSaw_76, mysterySynthBSaw_53, mysterySynthBSaw_39, mysterySynthBSaw_27,
		mysterySynthBSaw_19, mysterySynthBSaw_13, mysterySynthBSaw_9, mysterySynthBSaw_7, mysterySynthBSaw_5, mysterySynthBSaw_3, mysterySynthBSaw_1};


__attribute__((optimize("unroll-loops")))
void Voice::renderOsc(int s, int type, int32_t amplitude, int32_t* bufferStart, int32_t* bufferEnd, int numSamples, uint32_t phaseIncrement, uint32_t pulseWidth, uint32_t* startPhase, bool applyAmplitude, int32_t amplitudeIncrement, bool doOscSync, uint32_t resetterPhase, uint32_t resetterPhaseIncrement, uint32_t retriggerPhase, int32_t waveIndexIncrement) {
	...
	*startPhase += phaseIncrement * numSamples;
	...
	if (type == OSC_TYPE_SINE) {
		...
		if (type == OSC_TYPE_SQUARE) {
			...
			if (doPulseWave) {
				phaseIncrementForCalculations = phaseIncrement * 0.6; // Mildly band limit the square waves before they get ringmodded to create the pulse wave. *0.5 would be no band limiting
			}
		}
	...
	if (type != OSC_TYPE_SQUARE) {
		...
		if (doPulseWave) {
			...
			if (type == OSC_TYPE_ANALOG_SQUARE) {
				...
				if ((uint32_t)(resetterPhase) >= (uint32_t)-(resetterPhaseIncrement >> 1)) {
					resetterPhaseToMultiply -= ((uint32_t)1 << 31); // Count the last little bit of the cycle as actually a negative-number bit of the next one.
				}

				phase = (uint32_t)multiply_32x32_rshift32_rounded((pulseWidthAbsolute >> 1) + 1073741824, resetterPhaseToMultiply) << 3;
				phaseIncrement = ((uint32_t)multiply_32x32_rshift32_rounded((pulseWidthAbsolute >> 1) + 1073741824, (uint32_t)phaseIncrement >> 1) << 3);
			}
...
doOscSyncSetup:
		...
		resetterDivideByPhaseIncrement = // You should >> 47 if multiplying by this.
				(uint32_t)2147483648u / (uint16_t)((resetterPhaseIncrement + 65535) >> 16); // Round resetterPhaseIncrement up first, so resetterDivideByPhaseIncrement gets a tiny bit smaller, so things multiplied by it don't get a bit too big and overflow.
	}
...
doSine:
		...
		phase = waveTable->render(wavetableRenderingBuffer, numSamples, phaseIncrement, phase, doOscSync, resetterPhase, resetterPhaseIncrement, resetterDivideByPhaseIncrement, retriggerPhase, waveIndex, waveIndexIncrement);
			...
			if (doOscSync) {
				...
				do {
					...
					if (resetterPhaseNow < resetterPhaseIncrement) {
						phaseNow = (multiply_32x32_rshift32(multiply_32x32_rshift32(resetterPhaseNow, phaseIncrement), resetterDivideByPhaseIncrement) << 17) + 1 + retriggerPhase;
					}
...
doSaw:
			...
			if (tableNumber < AudioEngine::cpuDireness + 6) {
				...
				if (!doOscSync) {
					if (applyAmplitude) {
						renderCrudeSawWaveWithAmplitude(bufferStart, bufferEnd, phase, phaseIncrement, amplitude, amplitudeIncrement, numSamples);
					}
					else {
					...
					do {
						...
						if (applyAmplitude) {
							amplitudeNow += amplitudeIncrement;
							*thisSample = multiply_accumulate_32x32_rshift32_rounded(*thisSample, (int32_t)phaseNow, amplitudeNow); // Using multiply_accumulate saves like 10% here!
						}
			...
			if (tableNumber < AudioEngine::cpuDireness + 6) {
				...
				if (!doOscSync) {
					...
					if (applyAmplitude) {
						...
						do {
							phaseNow += phaseIncrement;
							amplitudeNow += amplitudeIncrement;
							*thisSample = multiply_accumulate_32x32_rshift32_rounded(*thisSample, getSquare(phaseNow, pulseWidth), amplitudeNow); // Using multiply_accumulate saves like 20% here, WTF!!!!
							++thisSample;
						} while (thisSample != bufferEnd);
					}
					...
					do {
						...
						if (applyAmplitude) {
							amplitudeNow += amplitudeIncrement;
							*thisSample = multiply_accumulate_32x32_rshift32_rounded(*thisSample, getSquare(phaseNow, pulseWidth), amplitudeNow);
						}
				...
				if (doPulseWave) {
					...
					if (doOscSync) {
						...
						RENDER_OSC_SYNC(STORE_VECTOR_WAVE_FOR_ONE_SYNC, waveRenderingFunctionPulse, 0, startRenderingASyncForPulseWave);
						phase <<= 1;
						goto doNeedToApplyAmplitude;
					}
					else {
						renderPulseWave(table, tableSizeMagnitude, amplitude, bufferStart, bufferEnd, phaseIncrement, phase, applyAmplitude, phaseToAdd, amplitudeIncrement);
						return;
					}
				}
			}
...
callRenderWave:
		if (doOscSync) {
			int32_t* bufferStartThisSync = applyAmplitude ? oscSyncRenderingBuffer : bufferStart;
			int numSamplesThisOscSyncSession = numSamples;
			RENDER_OSC_SYNC(STORE_VECTOR_WAVE_FOR_ONE_SYNC, waveRenderingFunctionGeneral, 0, startRenderingASyncForWave);
			goto doNeedToApplyAmplitude;
		}
		else {
			renderWave(table, tableSizeMagnitude, amplitude, bufferStart, bufferEnd, phaseIncrement, phase, applyAmplitude, phaseToAdd, amplitudeIncrement);
			return;
		}
	}

...
Voice staticVoices[NUM_VOICES_STATIC]; // Hmm, I forgot this was still being used. It's not a great way of doing things... wait does this still actually get used? No?
...
void init() {
	...
	ModelStackWithParamCollection* modelStackWithParamCollection = modelStack
...
void routine() {
    ...
    bool anythingInGateOutputBufferNow = cvEngine.gateOutputPending || cvEngine.clockOutputPending; // Not asapGateOutputPending (RUN)
...
startAgain:
        ...
        if (playbackHandler.swungTickScheduled && (int32_t)(playbackHandler.scheduledSwungTickTime - timeNextTick) < 0) {
        ...
        if (timeTilNextTick <= 0) {
        	if (nextTickType == TICK_TYPE_TIMER) {
        		...
        		playbackHandler.scheduleSwungTick(); // If that swung tick action just did a song swap, this will have already been called, but fortunately this doesn't break anything
        	}

        	// Those could have outputted clock or other MIDI / gate
        	if (midiEngine.anythingInOutputBuffer() || cvEngine.clockOutputPending || cvEngine.gateOutputPending) { // Not asapGateOutputPending. That probably actually couldn't have been
	...
	bool reverbOn = ((uint32_t)(audioSampleTimer - timeThereWasLastSomeReverb) < 44100 * 12); // Stop reverb after 12 seconds of inactivity

	if (reverbOn) {
		// Patch that to reverb volume
		int32_t positivePatchedValue = multiply_32x32_rshift32(compressorOutput, reverbCompressorVolumeInEffect) + 536870912;
    ...
    if (getCurrentUI() == &sampleBrowser
    		|| getCurrentUI() == &contextMenuFileBrowserKit
    		|| getCurrentUI() == &contextMenuFileBrowserSynth
			|| getCurrentUI() == &slicer) {

    	char modelStackMemory[MODEL_STACK_MAX_SIZE];
    	ModelStackWithThreeMainThings* modelStack = setupModelStackWithThreeMainThingsButNoNoteRow(modelStackMemory, currentSong, sampleForPreview, NULL, paramManagerForSamplePreview);
    ...
    if (currentSong) {
    	...
    	currentSong->globalEffectable.setupFilterSetConfig(&filterSetConfig, &masterVolumeAdjustmentL, &currentSong->paramManager);
    	currentSong->globalEffectable.processFilters(renderingBuffer, numSamples, &filterSetConfig);
    	currentSong->globalEffectable.processSRRAndBitcrushing(renderingBuffer, numSamples, &masterVolumeAdjustmentL, &currentSong->paramManager);
        ...
        int32_t pan = currentSong->paramManager.getUnpatchedParamSet()->getValue(PARAM_UNPATCHED_GLOBALEFFECTABLE_PAN) >> 1;
	...
	// Monitoring setup
	doMonitoring = false;
	if (audioRecorder.recordingSource == AUDIO_INPUT_CHANNEL_STEREO || audioRecorder.recordingSource == AUDIO_INPUT_CHANNEL_LEFT) {
...
*/
	...
	bool anyGateOutputPending = cvEngine.gateOutputPending || cvEngine.clockOutputPending || cvEngine.asapGateOutputPending;

    if ((midiEngine.anythingInOutputBuffer() || anyGateOutputPending)
		&& !isTimerEnabled(TIMER_MIDI_GATE_OUTPUT)) {
		...
		if (anyGateOutputPending) {
			...
			int32_t gateMinDelayInSamples = ((uint32_t)cvEngine.minGateOffTime * 289014) >> 16; // That's *4.41 (derived from 44100)
			int32_t samplesTilAllowedToSend = cvEngine.mostRecentSwitchOffTimeOfPendingNoteOn + gateMinDelayInSamples - audioSampleTimer;
		...
		*TGRA[TIMER_MIDI_GATE_OUTPUT] = ((uint32_t)samplesTilMIDIOrGate * 766245) >> 16; // Set delay time. This is samplesTilMIDIOrGate * 515616 / 44100.
...
bool doSomeOutputting() {
    ...
    while (renderingBufferOutputPosNow != renderingBufferOutputEnd) {
		...
		int64_t lAdjustedBig = (int64_t)renderingBufferOutputPosNow->l * (int64_t)masterVolumeAdjustmentL + (int64_t)getNoise();
		int64_t rAdjustedBig = (int64_t)renderingBufferOutputPosNow->r * (int64_t)masterVolumeAdjustmentR + (int64_t)getNoise();
		...
		if (doMonitoring) {
			...
			if (monitoringAction == ACTION_SUBTRACT_RIGHT_CHANNEL) {
				int32_t value = (inputReadPos[0] >> (AUDIO_OUTPUT_GAIN_DOUBLINGS + 1)) - (inputReadPos[1] >> (AUDIO_OUTPUT_GAIN_DOUBLINGS));
				lAdjusted += value;
				rAdjusted += value;
			}
    ...
    if (numSamplesOutputted) {
		...
		for (SampleRecorder* recorder = firstRecorder; recorder; recorder = recorder->next) {
			...
			// Recording from an input source
				...
				if (recorder->sourcePos >= getRxBufferEnd()) recorder->sourcePos -= SSI_RX_BUFFER_NUM_SAMPLES << NUM_MONO_INPUT_CHANNELS_MAGNITUDE;
			}
#endif
		}
    }
...
void updateReverbParams() {
	...
	if (reverbCompressorVolume < 0) {
		...
		if (soundWithMostReverb) {
			...
			if (whichCable != 255) {
				reverbCompressorVolumeInEffect = patchCableSet->getModifiedPatchCableAmount(whichCable, PARAM_GLOBAL_VOLUME_POST_REVERB_SEND);
			}
			...
			reverbCompressorVolumeInEffect = globalEffectableWithMostReverb->getSidechainVolumeAmountAsPatchCableDepth(paramManagerWithMostReverb);

compressorFound:
			reverbCompressorShapeInEffect = paramManagerWithMostReverb->getUnpatchedParamSet()->getValue(PARAM_UNPATCHED_COMPRESSOR_SHAPE);
...
void previewSample(String* path, FilePointer* filePointer, bool shouldActuallySound) {
	...
	if (shouldActuallySound) {
		...
		bypassCulling = true; // Needed - Dec 2021. I think it's during SampleBrowser::selectEncoderAction() that we may have gone a while without an audio routine call.
	}
}
...
Voice* solicitVoice(Sound* forSound) {
	...
	if (numSamplesLastTime >= 100 && activeVoices.getNumElements()) {
		...
		numSamplesLastTime -= 10; // Stop this triggering for lots of new voices. We just don't know how they'll weigh up to the ones being culled
...
SampleRecorder* getNewRecorder(int numChannels, int folderID, int mode, bool keepFirstReasons, bool writeLoopPoints, int buttonPressLatency) {
	int error;
	...
	if (mode >= AUDIO_INPUT_CHANNEL_FIRST_INTERNAL_OPTION) {
		renderInStereo = true; // Quickly set it to true. It'll keep getting reset to true now every time inputRoutine() is called
	}

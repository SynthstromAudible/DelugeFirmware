...
void MelodicInstrument::offerReceivedNote(ModelStackWithTimelineCounter* modelStack, MIDIDevice* fromDevice, bool on, int midiChannel, int note, int velocity, bool shouldRecordNotes, bool* doingMidiThru) {
...
yupItsForUs:
		...
		ModelStackWithNoteRow* modelStackWithNoteRow = instrumentClip ?
				instrumentClip->getNoteRowForYNote(note, modelStack) :
				modelStack->addNoteRow(0, NULL);
		...
		if (on) {
			...
			instrumentClipView.reportMPEInitialValuesForNoteEditing(modelStackWithNoteRow, mpeValues); // Hmm, should we really be going in here even when it's not MPE input?
...
recordingEarly:
							earlyNotes.insertElementIfNonePresent(note, velocity, instrumentClip->allowNoteTails(modelStackWithNoteRow));
...
doRecord:					instrumentClip->possiblyCloneForArrangementRecording(modelStack); // Will have to re-get modelStackWithNoteRow after this call
...
/*
				...
				*/
				beginAuditioningForNote(
						modelStack->toWithSong(), // Safe, cos we won't reference this again
...
void MelodicInstrument::offerReceivedPitchBend(ModelStackWithTimelineCounter* modelStackWithTimelineCounter, MIDIDevice* fromDevice, uint8_t channel, uint8_t data1, uint8_t data2, bool* doingMidiThru) {
...
mpeX:
				...
				int32_t value32 = (int32_t)value16 << 16; // Unlike for whole-Instrument pitch bend, this per-note kind is a modulation *source*, not the "preset" value for the parameter!
				polyphonicExpressionEventPossiblyToRecord(modelStackWithTimelineCounter, value32, 0, channel, MIDI_CHARACTERISTIC_CHANNEL);
			}
		}
		else if (midiInput.channelOrZone == MIDI_CHANNEL_MPE_UPPER_ZONE) {
...
void MelodicInstrument::offerReceivedCC(ModelStackWithTimelineCounter* modelStackWithTimelineCounter, MIDIDevice* fromDevice, uint8_t channel, uint8_t ccNumber, uint8_t value, bool* doingMidiThru) {
...
mpeY:
				if (ccNumber == 74) { // All other CCs are not supposed to be used for Member Channels, for anything.
					int32_t value32 = (value - 64) << 25;
					polyphonicExpressionEventPossiblyToRecord(modelStackWithTimelineCounter, value32, 1, channel, MIDI_CHARACTERISTIC_CHANNEL);
				}
			}
		}
		else if (midiInput.channelOrZone == MIDI_CHANNEL_MPE_UPPER_ZONE) {
...
void MelodicInstrument::offerReceivedAftertouch(ModelStackWithTimelineCounter* modelStackWithTimelineCounter, MIDIDevice* fromDevice, int channel, int value, int noteCode, bool* doingMidiThru) {
...
forMasterChannel:
			...
			if (noteCode != -1) {
				polyphonicExpressionEventPossiblyToRecord(modelStackWithTimelineCounter, valueBig, 2, noteCode, MIDI_CHARACTERISTIC_NOTE);
		...
		// Or if MPE enabled...
			...
			if (noteCode == -1) {
				if (midiInput.channelOrZone == MIDI_CHANNEL_MPE_LOWER_ZONE) {
					if (channel <= fromDevice->ports[MIDI_DIRECTION_INPUT_TO_DELUGE].mpeLowerZoneLastMemberChannel) {
						if (channel == 0) goto forMasterChannel;
processPolyphonicZ:		polyphonicExpressionEventPossiblyToRecord(modelStackWithTimelineCounter, valueBig, 2, channel, MIDI_CHARACTERISTIC_CHANNEL);
					}
				}
				else if (midiInput.channelOrZone == MIDI_CHANNEL_MPE_UPPER_ZONE) {
					if (channel >= fromDevice->ports[MIDI_DIRECTION_INPUT_TO_DELUGE].mpeUpperZoneLastMemberChannel) {
...
void MelodicInstrument::offerBendRangeUpdate(ModelStack* modelStack, MIDIDevice* device, int channelOrZone, int whichBendRange, int bendSemitones) {
...
void MelodicInstrument::stopAnyAuditioning(ModelStack* modelStack) {
	...
	ModelStackWithThreeMainThings* modelStackWithThreeMainThings = modelStack->addTimelineCounter(activeClip)->addOtherTwoThingsButNoNoteRow(toModControllable(), getParamManager(modelStack->song));
	...
	earlyNotes.empty(); // This is fine, though in a perfect world we'd prefer to just mark the notes as no longer active
...
void MelodicInstrument::beginAuditioningForNote(ModelStack* modelStack, int note, int velocity, int16_t const* mpeValues, int fromMIDIChannel, uint32_t sampleSyncLength) {
	...
	ParamManager* paramManager = getParamManager(modelStackWithNoteRow->song);
	ModelStackWithThreeMainThings* modelStackWithThreeMainThings = modelStackWithNoteRow->addOtherTwoThings(toModControllable(), paramManager);
...
ModelStackWithAutoParam* MelodicInstrument::getParamToControlFromInputMIDIChannel(int cc, ModelStackWithThreeMainThings* modelStack) {
...
void MelodicInstrument::processParamFromInputMIDIChannel(int cc, int32_t newValue, ModelStackWithTimelineCounter* modelStack) {
	...
	ModelStackWithAutoParam* modelStackWithParam = getParamToControlFromInputMIDIChannel(cc, modelStackWithThreeMainThings);

	if (modelStackWithParam->autoParam) {
		modelStackWithParam->autoParam->setValuePossiblyForRegion(newValue, modelStackWithParam, modPos, modLength, false); // Don't delete nodes in linear run, cos this might need to be outputted as MIDI again
	}
}
...
void MelodicInstrument::polyphonicExpressionEventPossiblyToRecord(ModelStackWithTimelineCounter* modelStack, int32_t newValue, int whichExpressionDimension, int channelOrNoteNumber, int whichCharacteristic) {
	...
	if (modelStack->timelineCounterIsSet()) { // && playbackHandler.isEitherClockActive() && playbackHandler.recording) {
		...
		for (int n = 0; n < arpeggiator.notes.getNumElements(); n++) {
			...
			if (arpNote->inputCharacteristics[whichCharacteristic] == channelOrNoteNumber) {	// If we're actually identifying by MIDI_CHARACTERISTIC_NOTE, we could do a much faster search,
				...
				ModelStackWithNoteRow* modelStackWithNoteRow = ((InstrumentClip*)modelStack->getTimelineCounter())->getNoteRowForYNote(arpNote->inputCharacteristics[MIDI_CHARACTERISTIC_NOTE], modelStack); // No need to create - it should already exist if they're recording a note here.
				NoteRow* noteRow = modelStackWithNoteRow->getNoteRowAllowNull();
				if (noteRow) {
					bool success = noteRow->recordPolyphonicExpressionEvent(modelStackWithNoteRow, newValue, whichExpressionDimension, false);
					if (success) continue;
				}

				// If still here, that didn't work, so just send it without recording.
				polyphonicExpressionEventOnChannelOrNote(newValue, whichExpressionDimension, arpNote->inputCharacteristics[MIDI_CHARACTERISTIC_NOTE], MIDI_CHARACTERISTIC_NOTE);
			}
		}
	}
	...
	// Or if not recording, just sound the change ourselves here (as opposed to the AutoParam doing it).
		...
		polyphonicExpressionEventOnChannelOrNote(newValue, whichExpressionDimension, channelOrNoteNumber, whichCharacteristic);
	}

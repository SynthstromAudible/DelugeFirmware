...
#define RECORDER_STATUS_ABORTED 4 // Means RAM error only. SD errors are noted separately and won't affect operation, as long as RAM lasts
...
public:
	...
	Cluster* currentRecordCluster; // Note! If this is NULL, that means that currentRecordClusterIndex refers to a cluster that never got created (cos some error or max file size reached)
	...
	String filePathCreated; // When this gets set, we add the Sample to the master list. This is stored here in addition to in the Sample,
	...
	bool haveAddedSampleToArray; // Need to keep track of this, so we know whether to remove it. Well I guess we could just look and see if it's there... but this is nice.

...
Kit::Kit() :
	Instrument(INSTRUMENT_TYPE_KIT),
	drumsWithRenderingActive(sizeof(Drum*))
{
    firstDrum = NULL;
    selectedDrum = NULL;
}
...
Drum* Kit::getPrevDrum(Drum* fromDrum) {
    ...
    while (thisDrum->next != fromDrum) thisDrum = thisDrum->next;
    return thisDrum;
}
...
bool Kit::writeDataToFile(Clip* clipForSavingOutputOnly, Song* song) {
    ...
    if (clipToTakeDrumOrderFrom) {
		...
		for (int i = 0; i < ((InstrumentClip*)clipToTakeDrumOrderFrom)->noteRows.getNumElements(); i++) {
			...
			if (thisNoteRow->drum) {
				...
				writeDrumToFile(drum, paramManagerForDrum, (clipForSavingOutputOnly == NULL), &selectedDrumIndex, &drumIndex, song);
    ...
    while (true) {
    	...
    	if (clipForSavingOutputOnly) {
    		...
    		paramManagerForDrum = &noteRow->paramManager; // Of course there won't be one if it's a NonAudioDrum, but that's fine
    	}
    	...
    	// Or if saving song...
   		...
   		writeDrumToFile(thisDrum, paramManagerForDrum, clipForSavingOutputOnly == NULL, &selectedDrumIndex, &drumIndex, song);
...
void Kit::writeDrumToFile(Drum* thisDrum, ParamManager* paramManagerForDrum, bool savingSong, int* selectedDrumIndex, int* drumIndex, Song* song) {
...
int Kit::readDrumFromFile(Song* song, Clip* clip, int drumType, int32_t readAutomationUpToPos) {
    ...
    int error = newDrum->readFromFile(song, clip, readAutomationUpToPos); // Will create and "back up" a new ParamManager if anything to read into it
    if (error) {
...
void Kit::addDrum(Drum* newDrum) {
	Drum** prevPointer = &firstDrum;
    while (*prevPointer != NULL) prevPointer = &((*prevPointer)->next);
    *prevPointer = newDrum;
...
SoundDrum* Kit::getDrumFromName(char const* name, bool onlyIfNoNoteRow) {
    for (Drum* thisDrum = firstDrum; thisDrum; thisDrum = thisDrum->next) {
        ...
        if (thisDrum->type == DRUM_TYPE_SOUND && ((SoundDrum*)thisDrum)->name.equalsCaseIrrespective(name)) return (SoundDrum*)thisDrum;
    }
...
void Kit::renderGlobalEffectableForClip(ModelStackWithTimelineCounter* modelStack, StereoSample* globalEffectableBuffer, int32_t* bufferToTransferTo, int numSamples, int32_t* reverbBuffer, int32_t reverbAmountAdjust,
	...
	for (int d = drumsWithRenderingActive.getNumElements() - 1; d >= 0; d--) {
		...
		ModelStackWithThreeMainThings* modelStackWithThreeMainThings = modelStack->addNoteRow(noteRowIndex, thisNoteRow)->addOtherTwoThings(soundDrum, drumParamManager);

		soundDrum->render(modelStackWithThreeMainThings, globalEffectableBuffer, numSamples, reverbBuffer, sideChainHitPending, reverbAmountAdjust, shouldLimitDelayFeedback, pitchAdjust); // According to our volume, we tell Drums to send less reverb
	}
	...
	if (playbackHandler.isEitherClockActive() && !playbackHandler.ticksLeftInCountIn && isClipActive) {
		...
		for (int i = 0; i < noteRows->getNumElements(); i++) {
			...
			if (thisNoteRow->drum && thisNoteRow->drum->type == DRUM_TYPE_SOUND) { // Just don't bother ticking other ones for now - their MPE doesn't need to interpolate.

				ParamCollectionSummary* patchedParamsSummary = &thisNoteRow->paramManager.summaries[1]; // No time to call the proper function and do error checking, sorry.
...
yesTickParamManager:
					ModelStackWithThreeMainThings* modelStackWithThreeMainThings = modelStack->addNoteRow(i, thisNoteRow)->addOtherTwoThings((SoundDrum*)thisNoteRow->drum, &thisNoteRow->paramManager);
				...
				ParamCollectionSummary* unpatchedParamsSummary = &thisNoteRow->paramManager.summaries[0]; // No time to call the proper function and do error checking, sorry.
				...
				ParamCollectionSummary* patchCablesSummary = &thisNoteRow->paramManager.summaries[2]; // No time to call the proper function and do error checking, sorry.
				...
				ParamCollectionSummary* expressionParamsSummary = &thisNoteRow->paramManager.summaries[3]; // No time to call the proper function and do error checking, sorry.
...
void Kit::renderOutput(ModelStack* modelStack, StereoSample *outputBuffer, StereoSample *outputBufferEnd, int numSamples, int32_t* reverbBuffer, int32_t reverbAmountAdjust, int32_t sideChainHitPending, bool shouldLimitDelayFeedback, bool isClipActive) {
	...
	GlobalEffectableForClip::renderOutput(modelStackWithTimelineCounter, paramManager, outputBuffer, numSamples, reverbBuffer, reverbAmountAdjust, sideChainHitPending, shouldLimitDelayFeedback, isClipActive, INSTRUMENT_TYPE_KIT, 8);
}


void Kit::offerReceivedCCToLearnedParams(MIDIDevice* fromDevice, uint8_t channel, uint8_t ccNumber, uint8_t value, ModelStackWithTimelineCounter* modelStack) {

	// Do it for this whole Kit
	ModControllableAudio::offerReceivedCCToLearnedParams(fromDevice, channel, ccNumber, value, modelStack); // NOTE: this call may change modelStack->timelineCounter etc!

    // Now do it for each NoteRow / Drum
    if (modelStack->timelineCounterIsSet()) { // This is always actually true currently for calls to this function, but let's make this safe and future proof.
		InstrumentClip* clip = (InstrumentClip*)modelStack->getTimelineCounter(); // May have been changed by call above!
		for (int i = 0; i < clip->noteRows.getNumElements(); i++) {
			...
			if (thisDrum && thisDrum->type == DRUM_TYPE_SOUND) {
				((SoundDrum*)thisDrum)->offerReceivedCCToLearnedParams(fromDevice, channel, ccNumber, value, modelStack, i);
			}
		}
    }
}

bool Kit::offerReceivedPitchBendToLearnedParams(MIDIDevice* fromDevice, uint8_t channel, uint8_t data1, uint8_t data2, ModelStackWithTimelineCounter* modelStack) {

	bool messageUsed;

	// Do it for this whole Kit
	messageUsed = ModControllableAudio::offerReceivedPitchBendToLearnedParams(fromDevice, channel, data1, data2, modelStack); // NOTE: this call may change modelStack->timelineCounter etc!
    ...
    if (modelStack->timelineCounterIsSet()) { // This is always actually true currently for calls to this function, but let's make this safe and future proof.
		...
		for (int i = 0; i < clip->noteRows.getNumElements(); i++) {
			...
			Drum* thisDrum = thisNoteRow->drum;
			if (thisDrum && thisDrum->type == DRUM_TYPE_SOUND) {
				if (((SoundDrum*)thisDrum)->offerReceivedPitchBendToLearnedParams(fromDevice, channel, data1, data2, modelStack)) {
					messageUsed = true;
				}
			}
		}
...
bool Kit::setActiveClip(ModelStackWithTimelineCounter* modelStack, int maySendMIDIPGMs) {
	...
	if (clipChanged) {
		...
		for (int i = 0; i < noteRows->getNumElements(); i++) {
			...
			if (thisNoteRow->drum) { // In a perfect world we'd do this for every Drum, even any without NoteRows in new Clip, but meh this'll be fine
...
void Kit::compensateInstrumentVolumeForResonance(ParamManagerForTimeline* paramManager, Song* song) {
    ...
    if (storageManager.firmwareVersionOfFileBeingRead < FIRMWARE_1P2P0 && !paramManager->resonanceBackwardsCompatibilityProcessed) {
		...
		int32_t compensation = interpolateTableSigned(unpatchedParams->getValue(PARAM_UNPATCHED_GLOBALEFFECTABLE_LPF_RES) + 2147483648, 32, oldResonanceCompensation, 3);
...
int32_t Kit::doTickForwardForArp(ModelStack* modelStack, int32_t currentPos) {
	...
	for (int i = 0; i < ((InstrumentClip*)activeClip)->noteRows.getNumElements(); i++) {
    	...
    	if (thisNoteRow->drum && thisNoteRow->drum->type == DRUM_TYPE_SOUND) { // For now, only SoundDrums have Arps, but that's actually a kinda pointless restriction...
    		...
    		int32_t currentPosThisRow = shouldUseIndependentPlayPos ? thisNoteRow->lastProcessedPosIfIndependent : currentPos;
...
void Kit::offerReceivedNote(ModelStackWithTimelineCounter* modelStack, MIDIDevice* fromDevice, bool on, int channel, int note, int velocity, bool shouldRecordNotes, bool* doingMidiThru) {
	...
	bool shouldRecordNoteOn = shouldRecordNotes && instrumentClip && currentSong->isClipActive(instrumentClip);	// Even if this comes out as false here, there are some
	...
	for (Drum* thisDrum = firstDrum; thisDrum; thisDrum = thisDrum->next) {
		...
		if (thisDrum->midiInput.equalsNoteOrCCAllowMPE(fromDevice, channelOrZone, note)) {
			...
			if (doingMidiThru && thisDrum->type == DRUM_TYPE_MIDI
					&& ((MIDIDrum*)thisDrum)->channel == channel
...
goingToRecordNoteOnEarly:
				...
				else if (currentPlaybackMode == &session && session.launchEventAtSwungTickCount && !instrumentClip->getCurrentlyRecordingLinearly()) {
					int ticksTilLaunch = session.launchEventAtSwungTickCount - playbackHandler.getActualSwungTickCount();
			...
			if (on) {
				...
				if (thisDrum->midiInput.isForMPEZone()) {
					for (int i = 0; i < NUM_EXPRESSION_DIMENSIONS; i++) {
						thisDrum->lastExpressionInputsReceived[BEND_RANGE_FINGER_LEVEL][i] = fromDevice->defaultInputMPEValuesPerMIDIChannel[channel][i] >> 8;
					}
				}
				...
				if (!thisNoteRow || !thisNoteRow->soundingStatus) {
					...
					if (thisNoteRow && thisDrum->type == DRUM_TYPE_SOUND && !thisNoteRow->paramManager.containsAnyMainParamCollections()) numericDriver.freezeWithError("E326"); // Trying to catch an E313 that Vinz got
			...
			// Note-off
				...
				if (thisNoteRow) {
					if (shouldRecordNotes && thisDrum->auditioned
						&& ((playbackHandler.recording == RECORDING_ARRANGEMENT && instrumentClip->isArrangementOnlyClip()) || currentSong->isClipActive(instrumentClip))) {
				...
				endAuditioningForDrum(modelStackWithNoteRow, thisDrum, velocity); // Do this even if not marked as auditioned, to avoid stuck notes in cases like if two note-ons were sent
			}
		}
...
void Kit::offerReceivedPitchBend(ModelStackWithTimelineCounter* modelStackWithTimelineCounter, MIDIDevice* fromDevice, uint8_t channel, uint8_t data1, uint8_t data2, bool* doingMidiThru) {
	...
	for (Drum* thisDrum = firstDrum; thisDrum; thisDrum = thisDrum->next) {
		if (thisDrum->midiInput.equalsChannelAllowMPE(fromDevice, channel)) {
			...
			if (thisDrum->midiInput.isForMPEZone()) { // If Drum has MPE input.
				if (channel == thisDrum->midiInput.getMasterChannel()) { // Message coming in on master channel - that's "main"/zone-level, too.
					goto yesThisDrum;
				}
				if (channel == thisDrum->lastMIDIChannelAuditioned) { // Or if per-finger level, check the member channel of the message matches the one sounding on the Drum right now.
					level = BEND_RANGE_FINGER_LEVEL;
					goto yesThisDrum;
				}
			}
			else { // Or, if Drum does not have MPE input, then this is a channel-level message.
yesThisDrum:	int16_t value16 = (((uint32_t)data1 | ((uint32_t)data2 << 7)) - 8192) << 2;
...
void Kit::offerReceivedCC(ModelStackWithTimelineCounter* modelStackWithTimelineCounter, MIDIDevice* fromDevice, uint8_t channel, uint8_t ccNumber, uint8_t value, bool* doingMidiThru) {
	...
	for (Drum* thisDrum = firstDrum; thisDrum; thisDrum = thisDrum->next) {
		if (thisDrum->midiInput.equalsChannelAllowMPE(fromDevice, channel)) {
			...
			if (thisDrum->midiInput.isForMPEZone()) { // If Drum has MPE input.
				...
				if (channel == thisDrum->lastMIDIChannelAuditioned) { // Or if per-finger level, check the member channel of the message matches the one sounding on the Drum right now.
					level = BEND_RANGE_FINGER_LEVEL;
yesThisDrum:		int16_t value16 = (value - 64) << 9;
...
void Kit::offerReceivedAftertouch(ModelStackWithTimelineCounter* modelStackWithTimelineCounter, MIDIDevice* fromDevice, int channel, int value, int noteCode, bool* doingMidiThru) {
...
yesThisDrum:		int32_t value15 = value << 8;
		...
		// Or a polyphonic aftertouch message - these aren't allowed for MPE except on the "master" channel.
			...
			if (thisDrum->midiInput.equalsNoteOrCCAllowMPEMasterChannels(fromDevice, channel, noteCode) && channel == thisDrum->lastMIDIChannelAuditioned) {
				level = BEND_RANGE_FINGER_LEVEL;
				goto yesThisDrum;
			}
		}
...
void Kit::offerBendRangeUpdate(ModelStack* modelStack, MIDIDevice* device, int channelOrZone, int whichBendRange, int bendSemitones) {

	if (whichBendRange == BEND_RANGE_MAIN) return;	// This is not used in Kits for Drums. Drums use their BEND_RANGE_FINGER_LEVEL for both kinds of bend.
...
void Kit::stopAnyAuditioning(ModelStack* modelStack) {
	...
	for (Drum* thisDrum = firstDrum; thisDrum; thisDrum = thisDrum->next) {
		if (thisDrum->auditioned) {
			ModelStackWithNoteRow* modelStackWithNoteRow = activeClip ?
...
void Kit::beginAuditioningforDrum(ModelStackWithNoteRow* modelStack, Drum* drum, int velocity, int16_t const* mpeValues, int fromMIDIChannel) {

	ParamManager* paramManagerForDrum = NULL;

	if (modelStack->getNoteRowAllowNull()) {
		paramManagerForDrum = &modelStack->getNoteRow()->paramManager;
		if (!paramManagerForDrum->containsAnyMainParamCollections() && drum->type == DRUM_TYPE_SOUND) numericDriver.freezeWithError("E313"); // Vinz got this!
	}
	else {
		if (drum->type == DRUM_TYPE_SOUND) {
			...
			if (!paramManagerForDrum) numericDriver.freezeWithError("E314"); // Ron got this, June 2020, while "dragging" a row vertically in arranger
		}
	}

	ModelStackWithThreeMainThings* modelStackWithThreeMainThings = modelStack->addOtherTwoThings(drum->toModControllable(), paramManagerForDrum);
...
void Kit::endAuditioningForDrum(ModelStackWithNoteRow* modelStack, Drum* drum, int velocity) {
	...
	if (drum->type == DRUM_TYPE_SOUND) {
		...
		paramManagerForDrum = modelStack->song->getBackedUpParamManagerPreferablyWithClip((SoundDrum*)drum, NULL);
		if (!paramManagerForDrum) numericDriver.freezeWithError("E312"); // Should make ALPHA_OR_BETA_VERSION after V3.0.0 release
	}

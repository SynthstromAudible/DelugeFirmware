...
#define DMA_SCIM_TX_CONFIG (0b00000000001000000000000000101000 | DMA_AM_FOR_SCIM) // LVL is 0 for SCIM despite the datasheet saying it should be 1 - a misprint, I believe
...
int uartFlush(int item) {
	...
	if (num < 0) {
		...
		if (numLeft) {
			...
			newConfig |= DMAC_CHCFG_0S_REN | DMAC_CHCFG_0S_RSW | DMAC_CHCFG_0S_DEM; // Set to switch to next "register" after first transaction, and also mask interrupt
		}
	}
	...
	uartItems[item].txBufferReadPosAfterTransfer = (uartItems[item].txBufferReadPosAfterTransfer + fullNum) & (txBufferSizes[item] - 1);
...
void uartFlushIfNotSending(int item) {
	...
	if (!uartItems[item].txSending) {
		...
		if (sentAny) {
			...
			DMACn(txDmaChannels[item]).CHCTRL_n |= DMAC_CHCTRL_0S_CLRTC | DMAC_CHCTRL_0S_SETEN; // ---- Enable DMA Transfer and clear TC bit ----
		}
	}
...
uint8_t uartGetChar(int item, char_t* readData) {
	...
	char_t const* currentWritePos = (char_t*)DMACnNonVolatile(rxDmaChannels[item]).CRDA_n; // We deliberately don't go (volatile uint32_t*) here, for speed
...
uint32_t* uartGetCharWithTiming(int timingCaptureItem, char_t* readData) {
	...
	*readData = *(rxBufferReadAddr[item] + UNCACHED_MIRROR_OFFSET);
	...
	uint32_t* timer = (uint32_t*)((uint32_t)&timingCaptureBuffers[timingCaptureItem][readPos & (timingCaptureBufferSizes[timingCaptureItem] - 1)] + UNCACHED_MIRROR_OFFSET);
...
void tx_interrupt(int item) {
	...
	if (item != UART_ITEM_MIDI || uartItems[item].shouldDoConsecutiveTransferAfter) {
		...
		if (sentAny) {
			DMACn(txDmaChannels[item]).CHCTRL_n = DMAC_CHCTRL_0S_CLRTC | DMAC_CHCTRL_0S_SETEN | DMAC_CHCTRL_0S_CLREND; // ---- Enable DMA Transfer and clear TC bit ----
			return;
		}
	}

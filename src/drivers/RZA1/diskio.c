...
static void setDataLength(BYTE newLength) {
	...
	if (currentDataLength == 32) {
		...
		RSPIx.SPBFCR = 0b00110010; // Transmit trigger number set to 0 bytes (only trigger when it's completely empty). You'd think setting it to 4 bytes would be faster, but somehow that doesn't work with DMA.
...
)
{
	...
	do {
		...
		if (doneAny) {
			...
			while (0 == RZA_IO_RegRead_8( &(RSPIx.SPSR),
...
static void mmcDMAInterrupt(uint32_t int_sense) {
	...
	DMACn(receive_dma_channel).CHCTRL_n |= (DMAC0_CHCTRL_n_CLREND
			| DMAC0_CHCTRL_n_CLRTC);
...
void setupMMCDMA(void) {
	...
	R_INTC_RegistIntFunc(INTC_ID_DMAINT0 + receive_dma_channel,
		&mmcDMAInterrupt);
...
)
{
	...
	Timer1 = 100;
	while (true) {							/* Wait for data packet in timeout of 100ms */
		token = xchg_spi(0xFF);
...
)
{
	...
	RSPIx.SPDR.UINT8[0] = 0xFF; // Extra one we're not going to add to the count, because we want to read it back after separately after reading back the counted ones
...
void spiDMAReadSectors(BYTE* buff, UINT* count) {
	...
	Timer1 = 1000;
	transferStatus = DMA_STATUS_WAIT_DATA_TOKEN;
	DMACn(send_dma_channel).CHITVL_n = 8192; // It'll take ages to respond to this first one, so don't bother quizzing it often
	spiDMARx(dmaTokenBuffer + 2, 1);
...
DRESULT disk_read_without_streaming_first (
	BYTE pdrv,		/* Physical drive nmuber (0) */
	BYTE *buff,		/* Pointer to the data buffer to store read data */
	LBA_t sector,	/* Start sector number (LBA) */
	UINT count		/* Sector count (1..128) */

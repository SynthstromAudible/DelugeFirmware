...
void usb_hstd_hcd_task(usb_vp_int_t stacd)
{
	...
	switch(msginfo)
	{
		...
		case USB_MSG_HCD_PCUTINT: // Not sure if this can actually happen - no other reference to this macro
			...
			uartPrintln("going into usb_hstd_interrupt_handler() from the routine task - that's kinda bad I think!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
...
void usb_send_start_rohan(usb_utr_t *ptr, uint16_t pipe, uint8_t const* data, int size)
{
    ...
    usb_cstd_set_nak_fast_rohan(pipe); // Ok somehow even this wasn't necessary!! Rohan. (Trying re-enabling now to fix that freeze I keep getting in usb_cstd_chg_curpipe_rohan_fast()...)

   // g_usb_data_cnt[pipe]      = length;                   /* Set data count */
    //g_p_usb_data[pipe]        = (uint8_t*) pp->p_tranadr; /* Set data pointer */
    g_usb_hstd_ignore_cnt[USB_CFG_USE_USBIP][pipe]    = (uint16_t) 0;             /* Ignore count clear */ // Only the original host function - not peripheral - had this.

    hw_usb_clear_status_bemp(NULL, pipe);        /* BEMP Status Clear */ // Seems not necessary. Rohan. (Trying re-enabling now to fix that freeze I keep getting in usb_cstd_chg_curpipe_rohan_fast()...)
    hw_usb_clear_sts_brdy(NULL, pipe);           /* BRDY Status Clear */ // Seems not necessary. Rohan. (Trying re-enabling now to fix that freeze I keep getting in usb_cstd_chg_curpipe_rohan_fast()...)

	//usb_hstd_buf2fifo(ptr, pipe, useport);  /* Buffer to FIFO data write */
    {
        hw_usb_clear_brdyenb(NULL, pipe); // Seems not necessary. Rohan. (Trying re-enabling now to fix that freeze I keep getting in usb_cstd_chg_curpipe_rohan_fast()...)
		...
		hw_usb_set_bempenb(USB_NULL,pipe); // It'd be cool if we could only need to set this up once, on pipe setup... but that didn't seem to work.


		/* Enable Not Ready Interrupt */
		hw_usb_set_nrdyenb(USB_NULL, pipe); // Seems not necessary. Rohan. (Trying re-enabling now to fix that freeze I keep getting in usb_cstd_chg_curpipe_rohan_fast()...)
    }
...
void usb_hstd_fifo_to_buf(usb_utr_t *ptr, uint16_t pipe, uint16_t useport)
{
    ...
    end_flag = usb_pstd_read_data(pipe, useport); // Modified by Rohan to call the PSTD function, since they're the same now

...
inline void PatchCableSet::freeDestinationMemory(bool destructing) {
	for (int g = 0; g < 2; g++) {
		if (destinations[g]) {
			...
			if (!destructing) destinations[g] = NULL;
		}
	}
}

PatchCableSet::PatchCableSet(ParamCollectionSummary* summary) :
		ParamCollection(sizeof(PatchCableSet), summary)
{
...
bool PatchCableSet::isSourcePatchedToSomething(int s) {
	uint32_t sourcesPatchedToSomething = sourcesPatchedToAnything[GLOBALITY_LOCAL] | sourcesPatchedToAnything[GLOBALITY_GLOBAL];
	return sourcesPatchedToSomething & (1 << s);
}
...
bool PatchCableSet::isSourcePatchedToSomethingManuallyCheckCables(int s) {
	for (int c = 0; c < numPatchCables; c++) {
		if (patchCables[c].from == s) return true;
	}
...
Destination* PatchCableSet::getDestinationForParam(int p) {
	int globality = (p < FIRST_GLOBAL_PARAM) ? GLOBALITY_LOCAL : GLOBALITY_GLOBAL;

	// Special case - no Destinations at all
	if (!destinations[globality]) return NULL;
	...
	for (Destination* destination = &destinations[globality][0]; destination->sources; destination++) {
		if (destination->destinationParamDescriptor == destinationParamDescriptor) return destination;
	}
	return NULL;
}
...
void PatchCableSet::setupPatching(ModelStackWithParamCollection const* modelStack) {
	...
	for (int g = 0; g < 2; g++) {
		destinations[g] = (Destination*)generalMemoryAllocator.alloc(sizeof(Destination) * (MAX_NUM_PATCH_CABLES + 1), NULL, false, true);
    ...
    for (numUsablePatchCables = 0; numUsablePatchCables < numPotentiallyUsablePatchCables; numUsablePatchCables++) {
        ...
        if (!patchCableIsUsable(numUsablePatchCables, modelStack)) {
        	while (true) {
        		...
        		if (numPotentiallyUsablePatchCables == numUsablePatchCables) goto numUsablePatchCablesWorkedOut;
...
goAgainWithoutIncrement:
		...
		if (!ourDescriptor->isJustAParam()) {
			...
			int destinationCableIndex = getPatchCableIndex(ourDescriptor->getBottomLevelSource(), ourDescriptor->getDestination(), NULL, false);

			// If doesn't exist, make this range-adjust*ing* cable "unusable".
			if (destinationCableIndex == 255) {
				numUsablePatchCables--;
				if (numUsablePatchCables == c) break;
    ...
    for (int c = 0; c < numUsablePatchCables; ) {
		...
		uint32_t sources = 1 << patchCables[c].from;
        ...
        for (int c2 = c + 1; c2 < numUsablePatchCables; c2++) {
        	...
        	if (patchCables[c2].destinationParamDescriptor == destinationParamDescriptor) {
        		...
        		sources |= (1 << patchCables[c2].from);

        		c++; // Always keep c pointing to the first one we haven't investigated yet

        		if (c2 != c) swapCables(c2, c); // If this wasn't the "first next" cable, do a swap
        	}
        }
    ...
    for (int globality = 0; globality < 2; globality++) {
    	...
    	// Otherwise...
    		...
    		if (numDestinations[globality] < MAX_NUM_PATCH_CABLES) {
    			generalMemoryAllocator.shortenRight(destinations[globality], sizeof(Destination) * (numDestinations[globality] + 1));
    		}
			...
			// Sort the Destinations by their DestinationDescriptor. That's just the Destinations that we're sorting - not the actual cables,
			// which are just grouped by param but we don't care about their order otherwise.
			if (numDestinations[globality] >= 2) {
				QuickSorter quickSorter(sizeof(Destination), 32, destinations[globality]);	// Pretty sure 32 is right? Was previously 16, which I guess worked because we
			...
			for ( ; destination->destinationParamDescriptor.data < (uint32_t)0xFFFFFF00; destination++) {
				...
				int c = getPatchCableIndex(destination->destinationParamDescriptor.getBottomLevelSource(), cableDestination);
				if (c != 255) {
					patchCables[c].rangeAdjustmentPointer = &rangeFinalValues[i];
				}

				// Ensure that any changes to the range/depth (because of sources patched to the range/depth, i.e. the cable we're looking at right now)
				// also cause the cable whose range/depth we're adjusting (and so also the param that that goes to) to recompute.
				...
				destination->sources |= 1 << destination->destinationParamDescriptor.getBottomLevelSource(); // TODO: if we ever wanted to allow another level of range-adjustment, this would probably need expanding.
    ...
    // Also, as we've just re-arranged patched cables, we need to check again whether any have interpolation active, or are even automated.
	modelStack->summary->resetAutomationRecord(NUM_UINTS_TO_REP_PATCH_CABLES - 1);
	modelStack->summary->resetInterpolationRecord(NUM_UINTS_TO_REP_PATCH_CABLES - 1);
...
bool PatchCableSet::doesDestinationDescriptorHaveAnyCables(ParamDescriptor destinationParamDescriptor) {
	for (int c = 0; c < numPatchCables; c++) {
        if (patchCables[c].destinationParamDescriptor == destinationParamDescriptor) return true;
    }
    return false;
}


bool PatchCableSet::isSourcePatchedToDestinationDescriptorVolumeInspecific(int s, ParamDescriptor destinationParamDescriptor) {
	if (destinationParamDescriptor.getJustTheParam() == PARAM_GLOBAL_VOLUME_POST_FX) {

		if (getPatchCableIndex(s, destinationParamDescriptor) != 255) return true;

		destinationParamDescriptor.changeParam(PARAM_GLOBAL_VOLUME_POST_REVERB_SEND);

		if (getPatchCableIndex(s, destinationParamDescriptor) != 255) return true;

		destinationParamDescriptor.changeParam(PARAM_LOCAL_VOLUME);
...
bool PatchCableSet::isAnySourcePatchedToParamVolumeInspecific(ParamDescriptor destinationParamDescriptor) {
	...
	if (destinationParamDescriptor.getJustTheParam() == PARAM_GLOBAL_VOLUME_POST_FX) {
		...
		if (doesDestinationDescriptorHaveAnyCables(destinationParamDescriptor)) return true;
...
uint8_t PatchCableSet::getPatchCableIndex(uint8_t from, ParamDescriptor destinationParamDescriptor, ModelStackWithParamCollection const* modelStack, bool createIfNotFound) {
	int c;
...
claimPatchCable:
		...
		if (modelStack) setupPatching(modelStack);
...
void PatchCableSet::deletePatchCable(ModelStackWithParamCollection const* modelStack, uint8_t c) {
	if (c >= numPatchCables) return; // Could probably happen. (Still?)
	patchCables[c].param.deleteAutomationBasicForSetup(); // No need to record what's happening - we kind of know that there's no automation left anyway...
...
bool PatchCableSet::patchCableIsUsable(uint8_t c, ModelStackWithThreeMainThings const* modelStack) {
    ...
    if (ourDescriptor->isNull()) return false; // When would this ever be the case?
	...
	int s = ourDescriptor->getTopLevelSource();

	// Or, if not a range-adjusting cable, do the normal thing.
	if (s == 0xFF) {
		s = patchCables[c].from;
	}
    return (((Sound*)modelStack->modControllable)->maySourcePatchToParam(s, p, modelStack->paramManager) == PATCH_CABLE_ACCEPTANCE_ALLOWED);
}
...
int32_t PatchCableSet::getModifiedPatchCableAmount(int c, int p) {
    ...
    case PARAM_GLOBAL_DELAY_RATE:
        ...
        if (p == PARAM_LOCAL_PITCH_ADJUST) {
            // If patching to master pitch, adjust range so that, on max range, the velocity-editing steps correspond with whole semitones
        	if (patchCables[c].from == PATCH_SOURCE_VELOCITY) {
        		//output = (output / 3 << 1);
        		output = multiply_32x32_rshift32_rounded(output, 1431655765) << 1;
        	}
        	else output = multiply_32x32_rshift32_rounded(output, 1518500250) << 1; // Divides by square root of 2. Gives us 3 octaves of shifting rather than 4
        }
...
#define FOR_EACH_FLAGGED_PARAM(whichParams) \
	for (int i = NUM_UINTS_TO_REP_PATCH_CABLES - 1; i >= 0; i--) { \
...
void PatchCableSet::generateRepeats(ModelStackWithParamCollection* modelStack, uint32_t oldLength, uint32_t newLength, bool shouldPingpong) {
...
void PatchCableSet::appendParamCollection(ModelStackWithParamCollection* modelStack, ModelStackWithParamCollection* otherModelStack, int32_t oldLength, int32_t reverseThisRepeatWithLength, bool pingpongingGenerally) {
	PatchCableSet* otherPatchCableSet = (PatchCableSet*)otherModelStack->paramCollection;

	FOR_EACH_FLAGGED_PARAM(otherModelStack->summary->whichParamsAreAutomated) // Iterate through the *other* ParamManager's stuff

		int s = otherPatchCableSet->patchCables[c].from;
		int i = getPatchCableIndex(s, otherPatchCableSet->patchCables[c].destinationParamDescriptor);

		if (i != 255) {
			patchCables[i].param.appendParam(&otherPatchCableSet->patchCables[c].param, oldLength, reverseThisRepeatWithLength, pingpongingGenerally);
		}
...
void PatchCableSet::trimToLength(uint32_t newLength, ModelStackWithParamCollection* modelStack, Action* action, bool maySetupPatching) {
	...
	if (maySetupPatching && anyStoppedBeingAutomated) {
		// In case the absence of automation here, presumably in conjunction with a value of 0, means this Cable is now inconsequential and can be deleted.
		setupPatching(modelStack);
	}
...
void PatchCableSet::shiftHorizontally(ModelStackWithParamCollection* modelStack, int32_t amount, int32_t effectiveLength) {
	FOR_EACH_FLAGGED_PARAM(modelStack->summary->whichParamsAreAutomated)
...
AutoParam* PatchCableSet::getParam(ModelStackWithParamCollection const* modelStack, int s, ParamDescriptor destinationParamDescriptor, bool allowCreation) {
...
void PatchCableSet::processCurrentPos(ModelStackWithParamCollection* modelStack, int ticksSkipped, bool reversed, bool didPingpong, bool mayInterpolate) {

	ticksTilNextEvent -= ticksSkipped;

	if (ticksTilNextEvent <= 0) {

		modelStack->summary->resetInterpolationRecord(NUM_UINTS_TO_REP_PATCH_CABLES - 1); // We'll repopulate this.
...
void PatchCableSet::beenCloned(bool copyAutomation, int32_t reverseDirectionWithLength) {
	...
	for ( ; c < MAX_NUM_PATCH_CABLES; c++) {
		patchCables[c].param.init();
	}

	// These pointers to allocated memory need that memory cloned.
	// If we knew we'd be calling setupPatching() again for this new clone, we wouldn't need to do this - we could just set these pointers to NULL. But this doesn't always happen.
	...
	// Allocate new memory - max size we might need
	for (int g = 0; g < 2; g++) {
		if (!destinations[g]) continue;

		newDestinations[g] = (Destination*)generalMemoryAllocator.alloc(sizeof(Destination) * (MAX_NUM_PATCH_CABLES + 1), NULL, false, true); // TODO: this is more than we'll soon realise we need - we should really shorten it again afterwards.
		...
		while (true) {
			*to = *from;
			if (!from->sources) break; // Stop *after* copying the terminating "0".
...
void PatchCableSet::readPatchCablesFromFile(int32_t readAutomationUpToPos) {
    ...
    int rangeAdjustableCableS = 255; // These are for loading in old-format presets, back when only one "range adjustable cable" was allowed.
    int rangeAdjustableCableP = 255;

    char const* tagName;
    while (*(tagName = storageManager.readNextTagOrAttributeName()) && numPatchCables < MAX_NUM_PATCH_CABLES) {
        if (!strcmp(tagName, "patchCable")) {

        	int numCablesAtStartOfThing = numPatchCables;

        	int source = PATCH_SOURCE_NONE;
            ParamDescriptor destinationParamDescriptor;
            ...
            while (*(tagName = storageManager.readNextTagOrAttributeName())) {
                if (!strcmp(tagName, "source")) {
                	...
                	destinationParamDescriptor.setToHaveParamOnly(((Sound*)NULL)->Sound::stringToParam(storageManager.readTagOrAttributeValue()));
                }
                    ...
                    rangeAdjustable = storageManager.readTagOrAttributeValueInt();
                }
                else if (!strcmp(tagName, "depthControlledBy")) {
                    while (*(tagName = storageManager.readNextTagOrAttributeName()) && numPatchCables < MAX_NUM_PATCH_CABLES - 1) {
                    	if (!strcmp(tagName, "patchCable")) {
                        	int rangeSource = PATCH_SOURCE_NONE;
                        	AutoParam tempRangeParam;
                            while (*(tagName = storageManager.readNextTagOrAttributeName())) {
                            	if (!strcmp(tagName, "source")) {
                            		rangeSource = stringToSource(storageManager.readTagOrAttributeValue());
                            	}
                            	else if (!strcmp(tagName, "amount")) {
                            		tempRangeParam.readFromFile(readAutomationUpToPos);
                            	}
                            	storageManager.exitTag();
                            }

                            if (rangeSource != PATCH_SOURCE_NONE && tempRangeParam.containsSomething(0)) {
...
doneWithThisRangeCable:
            ...
            if (source != PATCH_SOURCE_NONE && !destinationParamDescriptor.isNull() && tempParam.containsSomething(0)) {
                ...
                for (int c = 0; c < numCablesAtStartOfThing; c++) {
                    if (patchCables[c].from == source && patchCables[c].destinationParamDescriptor == destinationParamDescriptor) {
                        goto abandonThisCable;
                    }
                }
...
abandonThisCable:
    ...
    if (rangeAdjustableCableP != 255) {
    	for (int c = 0; c < numPatchCables; c++) {
    		if (patchCables[c].destinationParamDescriptor.isSetToParamWithNoSource(PARAM_PLACEHOLDER_RANGE)) {
    			patchCables[c].destinationParamDescriptor.setToHaveParamAndSource(rangeAdjustableCableP, rangeAdjustableCableS);
    		}
    	}
    }
}
...
void PatchCableSet::writePatchCablesToFile(bool writeAutomation) {
	if (!numPatchCables) return;

    // Patch cables
    storageManager.writeOpeningTag("patchCables");
    for (int c = 0; c < numPatchCables; c++) { // I have a feeling this should actually only do up to "numUsablePatchCables"... otherwise we end up with like FM-related cables written to file for a subtractive preset... etc.
    	if (!patchCables[c].destinationParamDescriptor.isJustAParam()) continue; // If it's a depth-controlling cable, we'll deal with that separately, below.

        storageManager.writeOpeningTagBeginning("patchCable");
        storageManager.writeAttribute("source", sourceToString(patchCables[c].from));
        storageManager.writeAttribute("destination", ((Sound*)NULL)->Sound::paramToString(patchCables[c].destinationParamDescriptor.getJustTheParam()));

        storageManager.write("\n");
        storageManager.printIndents();
        storageManager.write("amount=\"");
    	patchCables[c].param.writeToFile(writeAutomation);
        storageManager.write("\"");
        ...
        for (int d = 0; d < numPatchCables; d++) {
        	if (patchCables[d].destinationParamDescriptor == paramDescriptor) {
        		if (!anyDepthControllingCablesFound) {
        			...
        			storageManager.writeOpeningTag("depthControlledBy");
        		}

                storageManager.writeOpeningTagBeginning("patchCable");
                storageManager.writeAttribute("source", sourceToString(patchCables[d].from));

                storageManager.write("\n");
                storageManager.printIndents();
                storageManager.write("amount=\"");
            	patchCables[d].param.writeToFile(writeAutomation);
                storageManager.write("\"");
        ...
        if (anyDepthControllingCablesFound) {
			storageManager.writeClosingTag("depthControlledBy");
        	storageManager.writeClosingTag("patchCable");
        }
        else {
        	storageManager.closeTag();
        }
...
int PatchCableSet::getParamId(ParamDescriptor destinationParamDescriptor, int s) {
	destinationParamDescriptor.addSource(s);
	return destinationParamDescriptor.data;
}

void PatchCableSet::dissectParamId(uint32_t paramId, ParamDescriptor* destinationParamDescriptor, int* s) {
	ParamDescriptor paramDescriptor;
...
void PatchCableSet::notifyParamModifiedInSomeWay(ModelStackWithAutoParam const* modelStack, int32_t oldValue, bool automationChanged, bool automatedBefore, bool automatedNow) {

	ParamCollection::notifyParamModifiedInSomeWay(modelStack, oldValue, automationChanged, automatedBefore, automatedNow);
	...
	if (!modelStack->timelineCounterIsSet() || ((Clip*)modelStack->getTimelineCounter())->isActiveOnOutput()) {
		...
		if (!modelStack->autoParam->containsSomething(0)) {
			...
			if (c != 255) {
				...
				// could overwrite further-down fields of the ModelStack. Although I think in this case it actually doesn't - but best to be safe.
		...
		if (currentValueChanged) {
			int p = destinationParamDescriptor.getJustTheParam(); // Yes also do it if we've altered the "range" of a cable to p.... Although, would the call below actually cause all of that to get recalculated?
			((Sound*)modelStack->modControllable)->recalculatePatchingToParam(p, (ParamManagerForTimeline*)modelStack->paramManager);
		}
	}
...
void PatchCableSet::nudgeNonInterpolatingNodesAtPos(int32_t pos, int offset, int32_t lengthBeforeLoop, Action* action, ModelStackWithParamCollection* modelStack) {
...
PatchCable* PatchCableSet::getPatchCableFromVelocityToLevel() {
	ParamDescriptor paramDescriptor;
	paramDescriptor.setToHaveParamOnly(PARAM_LOCAL_VOLUME);

	int i = getPatchCableIndex(PATCH_SOURCE_VELOCITY, paramDescriptor, NULL, true); // Uh... ok this could create a cable but won't setupPatching() for it... was that intentional?

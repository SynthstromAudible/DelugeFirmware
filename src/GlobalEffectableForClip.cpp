...
void GlobalEffectableForClip::renderOutput(ModelStackWithTimelineCounter* modelStack, ParamManager* paramManagerForClip,
	...
	int32_t pitchAdjust = getFinalParameterValueExp(16777216, unpatchedParams->getValue(PARAM_UNPATCHED_GLOBALEFFECTABLE_PITCH_ADJUST) >> 3);
	...
	int32_t reverbSendAmount = getFinalParameterValueVolume(reverbAmountAdjust, cableToLinearParamShortcut(unpatchedParams->getValue(PARAM_UNPATCHED_GLOBALEFFECTABLE_REVERB_SEND_AMOUNT)));
    ...
    if (sidechainVolumeParam != -2147483648) {
		...
		int32_t compressorOutput = compressor.render(numSamples, unpatchedParams->getValue(PARAM_UNPATCHED_COMPRESSOR_SHAPE));

		int32_t positivePatchedValue = multiply_32x32_rshift32(compressorOutput, getSidechainVolumeAmountAsPatchCableDepth(paramManagerForClip)) + 536870912;
		postReverbVolume = (positivePatchedValue >> 15) * (positivePatchedValue >> 16); // This is tied to getParamNeutralValue(PARAM_GLOBAL_VOLUME_POST_REVERB_SEND) returning 134217728
    }
	...
	bool canRenderStraightIntoSongBuffer =
			...
			&& !delayWorkingState.doDelay
			&& (!pan || !AudioEngine::renderInStereo)
			&& !clippingAmount
			&& !hasBassAdjusted(paramManagerForClip)
			&& !hasTrebleAdjusted(paramManagerForClip)
			&& !reverbSendAmount
			&& !isBitcrushingEnabled(paramManagerForClip)
			&& !isSRREnabled(paramManagerForClip)
	...
	if (canRenderStraightIntoSongBuffer) {
		...
		if (willRenderAsOneChannelOnlyWhichWillNeedCopying()) {
			...
			renderGlobalEffectableForClip(modelStack, globalEffectableBuffer, (int32_t*)outputBuffer, numSamples, reverbBuffer, reverbAmountAdjustForDrums, sideChainHitPending, shouldLimitDelayFeedback, isClipActive, pitchAdjust, postFXAndReverbVolumeStart, postFXAndReverbVolumeEnd);
		}

		// Or if it's a stereo sample, it can render directly into the song buffer
		else {
			renderGlobalEffectableForClip(modelStack, outputBuffer, NULL, numSamples, reverbBuffer, reverbAmountAdjustForDrums, sideChainHitPending, shouldLimitDelayFeedback, isClipActive, pitchAdjust, postFXAndReverbVolumeStart, postFXAndReverbVolumeEnd);
		}
	}
...
doNormal:
		...
		renderGlobalEffectableForClip(modelStack, globalEffectableBuffer, NULL, numSamples, reverbBuffer, reverbAmountAdjustForDrums, sideChainHitPending, shouldLimitDelayFeedback, isClipActive, pitchAdjust, 134217728, 134217728);
		...
		processFXForGlobalEffectable(globalEffectableBuffer, numSamples, &volumePostFX, paramManagerForClip, &delayWorkingState, analogDelaySaturationAmount);
		...
		processReverbSendAndVolume(globalEffectableBuffer, numSamples, reverbBuffer, volumePostFX, postReverbVolumeLastTime, reverbSendAmount, pan, true, postReverbSendVolumeIncrement);
	...
	if (playbackHandler.isEitherClockActive() && !playbackHandler.ticksLeftInCountIn && isClipActive) {
		if (	paramManagerForClip->getUnpatchedParamSetSummary()->whichParamsAreInterpolating[0]
			...
			ModelStackWithThreeMainThings* modelStackWithThreeMainThings = modelStack->addOtherTwoThingsButNoNoteRow(this, paramManagerForClip);
...
int32_t GlobalEffectableForClip::getSidechainVolumeAmountAsPatchCableDepth(ParamManager* paramManager) {
    int32_t sidechainVolumeParam = paramManager->getUnpatchedParamSet()->getValue(PARAM_UNPATCHED_GLOBALEFFECTABLE_SIDECHAIN_VOLUME);
...
bool GlobalEffectableForClip::modEncoderButtonAction(uint8_t whichModEncoder, bool on, ModelStackWithThreeMainThings* modelStack) {
...
void GlobalEffectableForClip::getThingWithMostReverb(Clip* activeClip, Sound** soundWithMostReverb, ParamManager** paramManagerWithMostReverb, GlobalEffectableForClip** globalEffectableWithMostReverb, int32_t* highestReverbAmountFound) {
	...
	if (activeClip) {
		...
		if (!unpatchedParams->params[PARAM_UNPATCHED_GLOBALEFFECTABLE_REVERB_SEND_AMOUNT].isAutomated()
				&& unpatchedParams->params[PARAM_UNPATCHED_GLOBALEFFECTABLE_REVERB_SEND_AMOUNT].containsSomething(-2147483648)) {

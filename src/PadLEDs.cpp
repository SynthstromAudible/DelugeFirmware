...
bool shouldNotRenderDuringTimerRoutine() {
	return (renderingLock || currentUIMode == UI_MODE_EXPLODE_ANIMATION || currentUIMode == UI_MODE_ANIMATION_FADE ||
			currentUIMode == UI_MODE_HORIZONTAL_ZOOM || currentUIMode == UI_MODE_HORIZONTAL_SCROLL || currentUIMode == UI_MODE_INSTRUMENT_CLIP_EXPANDING || currentUIMode == UI_MODE_INSTRUMENT_CLIP_COLLAPSING
...
const uint8_t flashColours[3][3] = {{130, 120, 130},
...
void sendRGBForOnePadFast(int x, int y, const uint8_t* colourSource) {
	...
	if ((greyoutRows || greyoutCols) && ((greyoutRows & (1 << y)) || (greyoutCols & (1 << (displayWidth + sideBarWidth - 1 - x))))) {
...
void setupInstrumentClipCollapseAnimation(bool collapsingOutOfClipMinder) {
	...
	if (collapsingOutOfClipMinder) {
		view.getClipMuteSquareColour(currentSong->currentClip, clipMuteSquareColour); // This shouldn't have to be done every time
	}
}
...
void renderInstrumentClipCollapseAnimation(int xStart, int xEndOverall, int progress) {
	...
	for (int i = 0; i < numAnimatedRows; i++) {
		int newRowPosition = (int)animatedRowGoingFrom[i] * 65536 + ((int)animatedRowGoingTo[i] - animatedRowGoingFrom[i]) * (65536 - progress);
		newRowPosition1Array[i] = newRowPosition >> 16;
		intensity2Array[i] = newRowPosition; // & 65535;
		intensity1Array[i] = 65535 - intensity2Array[i];
	}

	int greyStart = instrumentClipView.getSquareFromPos(clipLength - 1, NULL, currentSong->xScroll[NAVIGATION_CLIP]) + 1;
	...
	for (int col = xStart; col < xEndOverall; col++) {
		...
		for (int i = 0; i < numAnimatedRows; i++) {
			...
			if (newRowPosition1Array[i] >= 0 && newRowPosition1Array[i] < displayHeight) {
				drawSquare(squareColours, intensity1, PadLEDs::image[newRowPosition1Array[i]][col], &occupancyMask[newRowPosition1Array[i]][col], occupancyMaskStore[i][col]);
			}

			if (newRowPosition1Array[i] >= -1 && newRowPosition1Array[i] < displayHeight - 1) {
				drawSquare(squareColours, intensity2, PadLEDs::image[newRowPosition1Array[i] + 1][col], &occupancyMask[newRowPosition1Array[i] + 1][col], occupancyMaskStore[i][col]);
			}
		}
	}
...
void renderAudioClipCollapseAnimation(int progress) {
	...
	int endSquareDisplay = divide_round_negative(clipLength - currentSong->xScroll[NAVIGATION_CLIP] - 1, currentSong->xZoom[NAVIGATION_CLIP]); // Rounds it well down, so we get the "final square" kinda...
	int greyStart = endSquareDisplay + 1;
	int xEnd = getMin(displayWidth, greyStart);

	for (int col = 0; col < xEnd; col++) {
		waveformRenderer.renderOneColForCollapseAnimation(col, col, sampleMaxPeakFromZero, progress, PadLEDs::image, &waveformRenderData, audioClipColour, sampleReversed, sampleValueCentrePoint, sampleValueSpan);
	}
	...
	if (xEnd < displayWidth) {
		...
		int greyTop = waveformRenderer.collapseAnimationToWhichRow + 1 + (((displayHeight - waveformRenderer.collapseAnimationToWhichRow) * progress + 32768) >> 16);
		int greyBottom = waveformRenderer.collapseAnimationToWhichRow - (((waveformRenderer.collapseAnimationToWhichRow) * progress + 32768) >> 16);
...
void renderAudioClipExplodeAnimation(int explodedness, bool shouldSendOut) {
	...
	for (int xDestSquareRightEdge = 0; xDestSquareRightEdge <= displayWidth; xDestSquareRightEdge++) {
		...
		int xSourceLeftEdge = xSourceRightEdge; // What was the last square's right edge is now the current square's left edge
		...
		waveformRenderer.renderOneColForCollapseAnimationZoomedOut(xSourceLeftEdgeLimited, xSourceRightEdgeLimited, xDest, sampleMaxPeakFromZero, explodedness, PadLEDs::image, &waveformRenderData, audioClipColour, sampleReversed, sampleValueCentrePoint, sampleValueSpan);

		if (xSourceRightEdge >= displayWidth) break; // If we got to the right edge of everything we want to draw onscreen
	}
...
void renderExplodeAnimation(int explodedness, bool shouldSendOut) {
	...
	for (int xSource = 0; xSource < displayWidth; xSource++) {
		...
		int xOriginBig = explodeAnimationXStartBig + (((int64_t)explodeAnimationXWidthBig * xSourceBig) >> (displayWidthMagnitude + 16));
		//xOriginBig = getMin(xOriginBig, explodeAnimationXStartBig + explodeAnimationXWidthBig - 65536);

		xOriginBig &= ~(uint32_t)65535; // Make sure each pixel's "origin-point" is right on an exact square - rounded to the left. That'll match what we'll see in the arranger
			...
			if (occupancyMaskStore[ySource + 1][xSource]) { // If there's actually anything in this source square...
				...
				for (int xOffset = 0; xOffset < 2; xOffset++) {
					...
					for (int yOffset = 0; yOffset < 2; yOffset++) {
						...
						drawSquare(imageStore[ySource + 1][xSource], intensityNow, PadLEDs::image[yNow][xNow], &occupancyMask[yNow][xNow], occupancyMaskStore[ySource + 1][xSource]);
					}
				}
			}
		}
	...
	if (shouldSendOut) {
		sendOutMainPadColours();
		uiTimerManager.setTimer(TIMER_MATRIX_DRIVER, 35); // Nice small number of milliseconds here. This animation is prone to looking jerky
	}
}
...
void timerRoutine() {
	...
	if (isUIModeActive(UI_MODE_HORIZONTAL_ZOOM)) {
    	...
    	if (progress >= 65536) { // If finished transitioning...
    		...
    		if (animationDirection == 1 && currentSong->currentClip->type == CLIP_TYPE_INSTRUMENT && ((InstrumentClip*)currentSong->currentClip)->onKeyboardScreen) {
    		...
    		// Otherwise, there's stuff we want to fade in / to
				...
				if (animationDirection == 1) {
					...
					if (arrangerView.doingAutoScrollNow) goto stopFade; // If we suddenly just started doing an auto-scroll, there's no time to fade
				}
...
void renderAudioClipExpandOrCollapse() {
    ...
    if (isUIModeActive(UI_MODE_AUDIO_CLIP_EXPANDING)) {
		...
		if (progress >= 65536) {
			...
			sessionView.renderRow(modelStack,
					waveformRenderer.collapseAnimationToWhichRow,
					imageStore[waveformRenderer.collapseAnimationToWhichRow],
					occupancyMaskStore[waveformRenderer.collapseAnimationToWhichRow],
					true);
...
void renderZoom() {
    ...
    uint32_t inImageTimesBiggerThanNormal = interpolateTable(fine, 10, expTableSmall); // This could be changed to run on a bigger number of bits in input
    inImageTimesBiggerThanNormal = increaseMagnitude(inImageTimesBiggerThanNormal, coarse - 14);

    renderZoomWithProgress(inImageTimesBiggerThanNormal, sineValue, &imageStore[0][0][0], &imageStore[displayHeight][0][0],
    		0, 0,
...
void renderZoomWithProgress(int inImageTimesBiggerThanNative, uint32_t inImageFadeAmount, uint8_t* __restrict__ innerImage, uint8_t* __restrict__ outerImage,
    ...
    uint32_t inImageTimesSmallerThanNative = 4294967295u / inImageTimesBiggerThanNative; // How many squares of the zoomed-in image fit into each square of our output image, at current zoom level
    uint32_t outImageTimesSmallerThanNative = 4294967295u / outImageTimesBiggerThanNative; // How many squares of the zoomed-out image fit into each square of our output image, at current zoom level
    ...
    for (int yDisplay = 0; yDisplay < displayHeight; yDisplay++) {
        if (transitionTakingPlaceOnRow[yDisplay]) {
			...
			if (zoomPinSquare[yDisplay] != lastZoomPinSquareDone) {
				...
				int32_t inImagePos0Onscreen = zoomPinSquare[yDisplay] - (zoomPinSquareInner[yDisplay] >> 8) * (inImageTimesBiggerThanNative >> 8); // Beware rounding inaccuracy
				int32_t inImageLeftEdgeOnscreen = inImagePos0Onscreen + inImageTimesBiggerThanNative * innerImageLeftEdge;
				int32_t inImageRightEdgeOnscreen = inImagePos0Onscreen + inImageTimesBiggerThanNative * innerImageRightEdge;
				...
				for (int xDisplay = 0; xDisplay < displayWidth; xDisplay++) {
					...
					int32_t inImageOverlap = getMin(outputSquareRightEdge, inImageRightEdgeOnscreen) - getMax(outputSquareLeftEdge, inImageLeftEdgeOnscreen);
					...
					int32_t outputSquareLeftEdgePositionRelativeToPinSquare = zoomPinSquare[yDisplay] - outputSquareLeftEdge;

					int32_t outputSquareLeftEdgePositionOnInImageRelativeToPinSquare = ((int64_t)outputSquareLeftEdgePositionRelativeToPinSquare * inImageTimesSmallerThanNative) >> 16;
					int32_t outputSquareLeftEdgePositionOnOutImageRelativeToPinSquare = ((int64_t)outputSquareLeftEdgePositionRelativeToPinSquare * outImageTimesSmallerThanNative) >> 16;
					...
					outputSquareStartOnOutImage[xDisplay] = zoomPinSquareOuter[yDisplay] - outputSquareLeftEdgePositionOnOutImageRelativeToPinSquare;
					outputSquareStartOnInImage[xDisplay] = zoomPinSquareInner[yDisplay] - outputSquareLeftEdgePositionOnInImageRelativeToPinSquare;

					outputSquareEndOnInImage[xDisplay] = outputSquareStartOnInImage[xDisplay] + inImageTimesSmallerThanNative;
					outputSquareEndOnOutImage[xDisplay] = outputSquareStartOnOutImage[xDisplay] + outImageTimesSmallerThanNative;
				}
			}
			...
			for (int xDisplay = 0; xDisplay < displayWidth; xDisplay++) {
				...
				if (inImageFadePerCol[xDisplay]) {
					...
					renderZoomedSquare(outputSquareStartOnInImage[xDisplay], outputSquareEndOnInImage[xDisplay], inImageTimesBiggerThanNative,
							inImageFadePerCol[xDisplay], outValue, innerImage, innerImageRightEdge, &drawingAnything);
				}

				{
					renderZoomedSquare(outputSquareStartOnOutImage[xDisplay], outputSquareEndOnOutImage[xDisplay], outImageTimesBiggerThanNative,
							65535 - inImageFadePerCol[xDisplay],
							outValue, outerImage, outerImageRightEdge, &drawingAnything);
				}
...
void renderZoomedSquare(int32_t outputSquareStartOnSourceImage, int32_t outputSquareEndOnSourceImage, uint32_t sourceImageTimesBiggerThanNormal,
	...
	for (int32_t sourceSquareLeftEdge = getMax((int32_t)0, outImageStartSquareLeftEdge); true; sourceSquareLeftEdge += 65536) {
		...
		*drawingAnything = true;
		...
		uint32_t intensity = getMin(sourceSquareRightEdge, outputSquareEndOnSourceImage) - getMax(sourceSquareLeftEdge, outputSquareStartOnSourceImage); // Will end up at max 65536

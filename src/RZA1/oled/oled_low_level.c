...
// (nothing currently on the bus). Returns true if a CV word was sent. Sends the word then bumps
// cvTransfersSent (the pull-based replacement for the old cvSent() upcall) so any gate waiting on
// this CV is released on schedule.
bool sendPriorityCVIfPending()
{

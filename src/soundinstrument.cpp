...
bool SoundInstrument::writeDataToFile(Clip* clipForSavingOutputOnly, Song* song) {
	...
	Sound::writeToFile(clipForSavingOutputOnly == NULL, paramManager, clipForSavingOutputOnly ? &((InstrumentClip*)clipForSavingOutputOnly)->arpSettings : NULL);
...
void SoundInstrument::renderOutput(ModelStack* modelStack, StereoSample *startPos, StereoSample *endPos, int numSamples, int32_t* reverbBuffer, int32_t reverbAmountAdjust, int32_t sideChainHitPending, bool shouldLimitDelayFeedback, bool isClipActive) {

	ModelStackWithThreeMainThings* modelStackWithThreeMainThings = modelStack->addTimelineCounter(activeClip)->addOtherTwoThingsButNoNoteRow(this, getParamManager(modelStack->song));

	if (!skippingRendering) {
		Sound::render(modelStackWithThreeMainThings, startPos, numSamples, reverbBuffer, sideChainHitPending, reverbAmountAdjust, shouldLimitDelayFeedback);
	}

	if (playbackHandler.isEitherClockActive() && !playbackHandler.ticksLeftInCountIn && isClipActive) {

		ParamCollectionSummary* patchedParamsSummary = &modelStackWithThreeMainThings->paramManager->summaries[1]; // No time to call the proper function and do error checking, sorry.
...
yesTickParamManagerForClip:
			modelStackWithThreeMainThings->paramManager->toForTimeline()->tickSamples(numSamples, modelStackWithThreeMainThings);
		}
		else {

			// Try other options too.

			ParamCollectionSummary* unpatchedParamsSummary = &modelStackWithThreeMainThings->paramManager->summaries[0]; // No time to call the proper function and do error checking, sorry.
			...
			ParamCollectionSummary* patchCablesSummary = &modelStackWithThreeMainThings->paramManager->summaries[2]; // No time to call the proper function and do error checking, sorry.
			...
			ParamCollectionSummary* expressionParamsSummary = &modelStackWithThreeMainThings->paramManager->summaries[3]; // No time to call the proper function and do error checking, sorry.
		...
		for (int i = 0; i < ((InstrumentClip*)activeClip)->noteRows.getNumElements(); i++) {
			...
			ParamCollectionSummary* expressionParamsSummary = &thisNoteRow->paramManager.summaries[0]; // No time to call the proper function and do error checking, sorry.
			if (expressionParamsSummary->whichParamsAreInterpolating[0]
#if NUM_EXPRESSION_DIMENSIONS > 32
					 || expressionParamsSummary->whichParamsAreInterpolating[1]
#endif
...
void SoundInstrument::setupPatching(ModelStackWithTimelineCounter* modelStack) {
	...
	if (clip) {
		...
		ModelStackWithThreeMainThings* modelStackWithThreeMainThings = modelStack->addOtherTwoThingsButNoNoteRow(this, paramManager);

		ensureInaccessibleParamPresetValuesWithoutKnobsAreZero(modelStackWithThreeMainThings);
	}
	else {
		paramManager = (ParamManagerForTimeline*)modelStack->song->getBackedUpParamManagerPreferablyWithClip(this, NULL);
		ensureInaccessibleParamPresetValuesWithoutKnobsAreZeroWithMinimalDetails(paramManager);
	}

	ModelStackWithParamCollection* modelStackWithParamCollection = paramManager->getPatchCableSet(modelStack->addOtherTwoThingsButNoNoteRow(this, paramManager));
...
void SoundInstrument::setupWithoutActiveClip(ModelStack* modelStack) {
	...
	ParamManager* paramManager = modelStackWithTimelineCounter->song->getBackedUpParamManagerPreferablyWithClip(this, NULL);
...
void SoundInstrument::sendNote(ModelStackWithThreeMainThings* modelStack, bool isOn, int noteCode, int16_t const* mpeValues, int fromMIDIChannel, uint8_t velocity, uint32_t sampleSyncLength, int32_t ticksLate, uint32_t samplesLate) {

	if (!inValidState) return;

    if (isOn) {
        noteOn(modelStack, &arpeggiator, noteCode, mpeValues, sampleSyncLength, ticksLate, samplesLate, velocity, fromMIDIChannel);
    }
    else {
    	...
    	if (instruction.noteCodeOffPostArp != ARP_NOTE_NONE) {
    		...
    		if (!modelStack->paramManager) numericDriver.freezeWithError("E402"); // Previously we were allowed to receive a NULL paramManager, then would just crudely do an unassignAllVoices(). But I'm pretty sure this doesn't exist anymore?
    #endif

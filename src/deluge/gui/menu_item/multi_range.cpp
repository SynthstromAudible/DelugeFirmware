...
//
// NOTE can be upto 4 characters, eg. C#-2, but usually 2-3. We use this
// "usually" to get away with a bit less padding and fit more on the line.
//
// 7-segment layout:
//
//   NOT-
...
//   CT
//
// For 7seg we only display the low note of the range: others are available
// singly by horizontal scrolling or under-the-screen buttons. Since sharps are
// marked by dots the note takes upto 3 letters (hence "NOT" instead of "NOTE").
...
void MultiRange::getText(char* buffer, int32_t* getLeftLength, int32_t* getRightLength, bool mayShowJustOne) {
	// Ensure we start from an empty buffer
	*buffer = 0;
	// Lower end
	if (display->haveOLED() || soundEditor.editingColumn <= 1) {
		if (this->getValue() == 0) {
	...
	// Upper end
	if (display->haveOLED() || soundEditor.editingColumn == 2) {
		if (this->getValue() == soundEditor.currentSource->ranges.getNumElements() - 1) {
			char* p = bufferPos;
	...
	// Sample
		...
		SampleHolderForVoice* sampleHolder = static_cast<SampleHolderForVoice*>(holder);
		// NOTE value
		if (display->haveOLED() || soundEditor.editingColumn == 3) {
			// transpose = 60 - midiNote <=> note = 60 - tranpose;
			noteCodeToString(60 - sampleHolder->transpose, bufferPos, getRightLength, true);
			bufferPos = buffer + strlen(buffer);
		}
		// Pad
		if (display->haveOLED()) {
		...
		// CENTS offset
		if (display->haveOLED() || soundEditor.editingColumn == 4) {
			int32_t cents = sampleHolder->getCents();
			if (display->haveOLED() && cents >= 0) {
				// non-negative numbers on OLED get a plus-prefix
				*(bufferPos++) = '+';
			}
...
ActionResult MultiRange::buttonAction(deluge::hid::Button b, bool on, bool inCard) {
	(void)inCard;
	bool ok = on; // XXX && display->haveOLED();

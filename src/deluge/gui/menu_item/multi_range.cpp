...
void MultiRange::beginSession(MenuItem* navigatedBackwardFrom) {
	...
	this->set_value(soundEditor.currentSource->defaultRangeI);
	soundEditor.currentSource->getOrCreateFirstRange(); // TODO: deal with error
	soundEditor.setCurrentMultiRange(this->get_value());

#if HAVE_OLED
	soundEditor.menuCurrentScroll = this->get_value() - 1;
	if (soundEditor.menuCurrentScroll > this->get_value() - kOLEDMenuNumOptionsVisible + 1) {
...
void MultiRange::selectEncoderAction(int32_t offset) {
	...
	if (soundEditor.editingRangeEdge != RangeEdit::OFF) {
		...
		if (soundEditor.editingRangeEdge == RangeEdit::LEFT) {
			...
			::MultiRange* lowerRange = soundEditor.currentSource->ranges.getElement(this->get_value() - 1);

			// Raising
			if (offset >= 0) {
				int32_t maximum;
				if (this->get_value() < soundEditor.currentSource->ranges.getNumElements() - 1) {
					::MultiRange* currentRange = soundEditor.currentSource->ranges.getElement(this->get_value());
			...
			// Lowering
				...
				int32_t minimum;
				if (this->get_value() >= 2) {
					::MultiRange* lowerLowerRange = soundEditor.currentSource->ranges.getElement(this->get_value() - 2);
		...
		// Editing right
			...
			if (offset >= 0) {
				int32_t maximum;
				if (this->get_value() < soundEditor.currentSource->ranges.getNumElements() - 2) {
					::MultiRange* higherRange = soundEditor.currentSource->ranges.getElement(this->get_value() + 1);
	...
	// Or, normal mode
		...
		if (Buttons::isShiftButtonPressed()) {
			...
			if (this->get_value() == 0) {
				currentRangeBottom = soundEditor.currentSource->ranges.getElement(this->get_value())->topNote - 1;
				if (currentRangeBottom > 0) {
					currentRangeBottom = 0;
				}
			}
			else {
				currentRangeBottom = soundEditor.currentSource->ranges.getElement(this->get_value() - 1)->topNote + 1;
			}
			...
			if (this->get_value() == soundEditor.currentSource->ranges.getNumElements() - 1) {
				...
				currentRangeTop = soundEditor.currentSource->ranges.getElement(this->get_value())->topNote;
			}
			...
			if (offset >= 0) {
				...
				::MultiRange* oldRange = soundEditor.currentSource->ranges.getElement(this->get_value());
		...
		// Or the normal thing of just flicking through existing ranges
			...
			soundEditor.currentSource->defaultRangeI = this->get_value();
...
void MultiRange::deletePress() {
	...
	soundEditor.currentSound->deleteMultiRange(soundEditor.currentSourceIndex,
	                                           this->get_value()); // Unassigns all Voices
...
void MultiRange::getText(char* buffer, int32_t* getLeftLength, int32_t* getRightLength, bool mayShowJustOne) {
	...
	if (this->get_value() == 0) {
		...
		int32_t note = soundEditor.currentSource->ranges.getElement(this->get_value() - 1)->topNote + 1;
	...
	if (this->get_value() == soundEditor.currentSource->ranges.getNumElements() - 1) {
		...
		*(bufferPos++) = 0;
		...
		int32_t note = soundEditor.currentSource->ranges.getElement(this->get_value())->topNote;

		if (mayShowJustOne && this->get_value() > 0
		    && note == soundEditor.currentSource->ranges.getElement(this->get_value() - 1)->topNote + 1) {
			return;
		}
...
bool MultiRange::mayEditRangeEdge(RangeEdit whichEdge) {
	...
	return (this->get_value() != soundEditor.currentSource->ranges.getNumElements() - 1);
}
...
void MultiRange::drawPixelsForOled() {
	...
	int32_t actualCurrentRange = this->get_value();

	this->set_value(soundEditor.menuCurrentScroll);
	size_t idx = 0;
	for (idx = 0; idx < kOLEDMenuNumOptionsVisible; idx++) {
		if (this->get_value() >= soundEditor.currentSource->ranges.getNumElements()) {
			break;
		}
	...
	this->set_value(actualCurrentRange);

	int32_t selectedOption = -1;
	if (soundEditor.editingRangeEdge == RangeEdit::OFF) {
		selectedOption = this->get_value() - soundEditor.menuCurrentScroll;
	}
	...
	if (soundEditor.editingRangeEdge != RangeEdit::OFF) {
		...
		baseY += (this->get_value() - soundEditor.menuCurrentScroll) * kTextSpacingY;

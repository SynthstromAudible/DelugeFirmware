...
void PatchCableStrength::renderOLED() {
	...
	// Draw patch sources and destination
	{
		...
		char const* sourceName = isJustAParam ? getSourceDisplayNameForOLED(source) : sourceToStringShort(source);
		image.drawString(sourceName, leftPadding, y, kTextSpacingX, kTextSizeYUpdated);
		y += ySpacing;
	...
	// Draw the number
	{
		...
		const int32_t numDecimals = getNonZeroDecimalPlacesCount();

		// We hide the fractional part if it's zero and the cursor is not on the fractional part
		const int32_t hiddenDigitsCount = [&] {
			if (soundEditor.numberEditPos > 1) {
				return numDecimals == 2 ? 0 : numDecimals == 1 ? 1 : 2;
			}
			if (soundEditor.numberEditPos == 1) {
				return numDecimals == 2 ? 0 : 1;
			}
			return 0;
		}();

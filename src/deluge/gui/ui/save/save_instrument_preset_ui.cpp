...
bool SaveInstrumentPresetUI::performSave(StorageManager &writer, bool mayOverwrite) {
	if (display->have7SEG()) {
...
fail:
	...
	error = writer.createXMLFile(filePath.get(), mayOverwrite, false);
	...
	instrumentToSave->writeToFile(writer, getCurrentClip(), currentSong);

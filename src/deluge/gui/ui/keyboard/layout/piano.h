...
private:
	 ...
	 */
	inline uint16_t intervalFromCoords(int32_t x, int32_t y) { return ( y == 0 || (y % 2 == 0))?(pianoIntervals[1][(x + getState().piano.noteOffset) % 7]):(pianoIntervals[0][(x + getState().piano.noteOffset) % 7]); }
	 ...
	 */
	inline uint16_t noteFromCoords(int32_t x, int32_t y) { return (getState().piano.scrollOffset)*12 + intervalFromCoords(x, y) + 12*(y / 2) + 12*((x+getState().piano.noteOffset) / 7) - 1; }

...
int LoadSongUI::buttonAction(hid::Button b, bool on, bool inCardRoutine) {
	...
	if ((b == hid::button::load) || (b == hid::button::selectEnc)) {
		if (on) {
			if (!currentUIMode) {
				if (inCardRoutine) {
...
void LoadSongUI::performLoad() {
	...
	if (!playbackHandler.isEitherClockActive()) {
		...
		playbackHandler.songSwapShouldPreserveTempo = Buttons::isButtonPressed(hid::button::tempoEnc);
	}
...
int LoadSongUI::verticalEncoderAction(int offset, bool inCardRoutine) {
	if (!currentUIMode && !Buttons::isButtonPressed(hid::button::yEnc) && !Buttons::isShiftButtonPressed()
	    && offset < 0) {
		if (inCardRoutine) {
			return ACTION_RESULT_REMIND_ME_OUTSIDE_CARD_ROUTINE;
		}

			...
			if (timeTil < 0) {
				...
				case TIMER_SEND_MIDI_FEEDBACK_FOR_AUTOMATION:
					...
					if (playbackHandler.isEitherClockActive()
						...
						if ((AudioEngine::audioSampleTimer - midiFollowView.timeAutomationFeedbackLastSent)
						    >= sendRate) {
							view.sendMidiFollowFeedback(nullptr, kNoSelection, true);
							midiFollowView.timeAutomationFeedbackLastSent = AudioEngine::audioSampleTimer;
						}
					}
					...
					else if (midiFollowView.timeAutomationFeedbackLastSent != 0) {
						view.sendMidiFollowFeedback(nullptr, kNoSelection, true);
						midiFollowView.timeAutomationFeedbackLastSent = 0;
					}
					break;

...
void PerformanceSessionView::goToArrangementEditor() {
...
void PerformanceSessionView::clipPressEnded() {
...
void PerformanceSessionView::sectionPadAction(uint8_t y, bool on) {
...
void PerformanceSessionView::editNumRepeatsTilLaunch(int32_t offset) {
...
ActionResult PerformanceSessionView::verticalScrollOneSquare(int32_t direction) {
...
void PerformanceSessionView::drawStatusSquare(uint8_t yDisplay, uint8_t thisImage[][3]) {
...
void PerformanceSessionView::drawSectionSquare(uint8_t yDisplay, uint8_t thisImage[][3]) {
...
Clip* PerformanceSessionView::createNewInstrumentClip(int32_t yDisplay) {
	actionLogger.deleteAllLogs();
...
void PerformanceSessionView::replaceAudioClipWithInstrumentClip(Clip* clip, InstrumentType instrumentType) {
...
void PerformanceSessionView::replaceInstrumentClipWithAudioClip(Clip* clip) {
	int32_t clipIndex = currentSong->sessionClips.getIndexForClip(clip);
...
void PerformanceSessionView::removeClip(Clip* clip) {
...
Clip* PerformanceSessionView::getClipOnScreen(int32_t yDisplay) {
	if (currentSong->sessionLayout == SessionLayoutType::SessionLayoutTypeGrid) {
...
void PerformanceSessionView::redrawClipsOnScreen(bool doRender) {
	if (doRender) {
...
void PerformanceSessionView::cloneClip(uint8_t yDisplayFrom, uint8_t yDisplayTo) {
...
bool PerformanceSessionView::calculateZoomPinSquares(uint32_t oldScroll, uint32_t newScroll, uint32_t newZoom, uint32_t oldZoom) {
...
int32_t PerformanceSessionView::getClipPlaceOnScreen(Clip* clip) {
...
bool PerformanceSessionView::setupScroll(uint32_t oldScroll) {
	if (currentSong->sessionLayout == SessionLayoutType::SessionLayoutTypeGrid) {
		return false;
	}
...
uint32_t PerformanceSessionView::getClipLocalScroll(Clip* clip, uint32_t overviewScroll, uint32_t xZoom) {
	return std::min((clip->loopLength - 1) / (xZoom * kDisplayWidth) * xZoom * kDisplayWidth, overviewScroll);
}

void PerformanceSessionView::flashPlayRoutine() {
...
void PerformanceSessionView::noteRowChanged(InstrumentClip* instrumentClip, NoteRow* noteRow) {
...
uint32_t PerformanceSessionView::getGreyedOutRowsNotRepresentingOutput(Output* output) {
...
void PerformanceSessionView::transitionToViewForClip(Clip* clip) {
...
void PerformanceSessionView::transitionToSessionView() {
	if (currentSong->sessionLayout == SessionLayoutType::SessionLayoutTypeGrid) {
...
void PerformanceSessionView::finishedTransitioningHere() {
...
void PerformanceSessionView::playbackEnded() {
	if (currentSong->sessionLayout == SessionLayoutType::SessionLayoutTypeGrid) {
		requestRendering(this, 0xFFFFFFFF, 0xFFFFFFFF);
		return;
	}
...
void PerformanceSessionView::clipNeedsReRendering(Clip* clip) {
	if (currentSong->sessionLayout == SessionLayoutType::SessionLayoutTypeGrid) {
		requestRendering(this, 0xFFFFFFFF, 0xFFFFFFFF);
		return;
	}
...
void PerformanceSessionView::sampleNeedsReRendering(Sample* sample) {
	if (currentSong->sessionLayout == SessionLayoutType::SessionLayoutTypeGrid) {
		requestRendering(this, 0xFFFFFFFF, 0xFFFFFFFF);
		return;
	}
...
void PerformanceSessionView::midiLearnFlash() {
	if (currentSong->sessionLayout == SessionLayoutType::SessionLayoutTypeGrid) {
		requestRendering(this, 0xFFFFFFFF, 0xFFFFFFFF);
		return;
	}
...
Clip* PerformanceSessionView::getClipForLayout() {
	switch (currentSong->sessionLayout) {
	case SessionLayoutType::SessionLayoutTypeGrid: {
		return gridClipFromCoords(gridFirstPressedX, gridFirstPressedY);
		break;
...
void PerformanceSessionView::selectLayout(int8_t offset) {
	gridSetDefaultMode();
	gridResetPresses();
...
bool PerformanceSessionView::gridRenderSidebar(uint32_t whichRows, uint8_t image[][kDisplayWidth + kSideBarWidth][3],
                                    uint8_t occupancyMask[][kDisplayWidth + kSideBarWidth]) {
...
bool PerformanceSessionView::gridRenderMainPads(uint32_t whichRows, uint8_t image[][kDisplayWidth + kSideBarWidth][3],
                                     uint8_t occupancyMask[][kDisplayWidth + kSideBarWidth], bool drawUndefinedArea) {
...
void PerformanceSessionView::gridRenderClipColor(Clip* clip, uint8_t resultColour[]) {
...
Clip* PerformanceSessionView::gridCloneClip(Clip* sourceClip) {
	char modelStackMemory[MODEL_STACK_MAX_SIZE];
	ModelStackWithTimelineCounter* modelStack =
...
Clip* PerformanceSessionView::gridCreateClipInTrack(Output* targetOutput) {
...
bool PerformanceSessionView::gridCreateNewTrackForClip(InstrumentType type, InstrumentClip* clip, bool copyDrumsFromClip) {
...
InstrumentClip* PerformanceSessionView::gridCreateClipWithNewTrack(InstrumentType type) {
	// Allocate new clip
	void* memory = GeneralMemoryAllocator::get().allocMaxSpeed(sizeof(InstrumentClip));
	if (!memory) {
		display->displayError(ERROR_INSUFFICIENT_RAM);
...
Clip* PerformanceSessionView::gridCreateClip(uint32_t targetSection, Output* targetOutput, Clip* sourceClip) {
	actionLogger.deleteAllLogs();
...
void PerformanceSessionView::gridClonePad(uint32_t sourceX, uint32_t sourceY, uint32_t targetX, uint32_t targetY) {
...
void PerformanceSessionView::gridStartSection(uint32_t section, bool instant) {
	if (instant) {
...
void PerformanceSessionView::gridToggleClipPlay(Clip* clip, bool instant) {
	session.toggleClipStatus(clip, nullptr, instant, kInternalButtonPressLatency);
}

ActionResult PerformanceSessionView::gridHandlePads(int32_t x, int32_t y, int32_t on) {
...
ActionResult PerformanceSessionView::gridHandlePadsEdit(int32_t x, int32_t y, int32_t on, Clip* clip) {
...
ActionResult PerformanceSessionView::gridHandlePadsLaunch(int32_t x, int32_t y, int32_t on, Clip* clip) {
...
ActionResult PerformanceSessionView::gridHandlePadsLaunchImmediate(int32_t x, int32_t y, int32_t on, Clip* clip) {
...
ActionResult PerformanceSessionView::gridHandlePadsLaunchWithSelection(int32_t x, int32_t y, int32_t on, Clip* clip) {
	if (on) {
...
void PerformanceSessionView::gridHandlePadsLaunchToggleArming(Clip* clip, bool immediate) {
	if (immediate) {
...
ActionResult PerformanceSessionView::gridHandleScroll(int32_t offsetX, int32_t offsetY) {
...
void PerformanceSessionView::gridTransitionToSessionView() {
	Sample* sample;
...
void PerformanceSessionView::gridTransitionToViewForClip(Clip* clip) {
	if (clip->type == CLIP_TYPE_AUDIO) {
		// If no sample, just skip directly there
...
const uint32_t PerformanceSessionView::gridTrackCount() {
	uint32_t count = 0;
...
uint32_t PerformanceSessionView::gridClipCountForTrack(Output* track) {
	uint32_t count = 0;
	for (int32_t idxClip = 0; idxClip < currentSong->sessionClips.getNumElements(); ++idxClip) {
		Clip* clip = currentSong->sessionClips.getClipAtIndex(idxClip);
...
uint32_t PerformanceSessionView::gridTrackIndexFromTrack(Output* track, uint32_t maxTrack) {
	if (maxTrack <= 0) {
		return -1;
	}
...
Output* PerformanceSessionView::gridTrackFromIndex(uint32_t trackIndex, uint32_t maxTrack) {
	uint32_t count = 0;
	Output* currentTrack = currentSong->firstOutput;
	while (currentTrack != nullptr) {
		if (currentTrack->activeClip != nullptr) {
...
int32_t PerformanceSessionView::gridYFromSection(uint32_t section) {
...
int32_t PerformanceSessionView::gridSectionFromY(uint32_t y) {
...
int32_t PerformanceSessionView::gridXFromTrack(uint32_t trackIndex) {
...
int32_t PerformanceSessionView::gridTrackIndexFromX(uint32_t x, uint32_t maxTrack) {
	if (maxTrack <= 0) {
		return 0;
	}
...
Output* PerformanceSessionView::gridTrackFromX(uint32_t x, uint32_t maxTrack) {
...
Clip* PerformanceSessionView::gridClipFromCoords(uint32_t x, uint32_t y) {
	auto maxTrack = gridTrackCount();
	Output* track = gridTrackFromX(x, maxTrack);
	if (track == nullptr) {
		return nullptr;

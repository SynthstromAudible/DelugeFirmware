...
bool SongView::getGreyoutColsAndRows(uint32_t* cols, uint32_t* rows) {
...
ActionResult SongView::timerCallback() {
...
bool SongView::renderMainPads(uint32_t whichRows, RGB image[][kDisplayWidth + kSideBarWidth],
...
bool SongView::renderSidebar(uint32_t whichRows, RGB image[][kDisplayWidth + kSideBarWidth],
...
void SongView::renderOLED(deluge::hid::display::oled_canvas::Canvas& canvas) {
...
ActionResult SongView::padAction(int32_t xDisplay, int32_t yDisplay, int32_t on) {
...
ActionResult SongView::buttonAction(deluge::hid::Button b, bool on, bool inCardRoutine) {
	...
	// Clip-view button
	if (b == CLIP_VIEW) {
		bool isGridView = currentSong->songViewLayout == SongViewLayout::Grid;
		if (on
...
moveAfterClipInstance:
	...
	// Overwrite to allow not showing zoom level in grid
		...
		if (on) {
			...
			if (isNoUIModeActive() && (currentSong->songViewLayout != SongViewLayout::Grid)) {
				displayZoomLevel();
			}
...
ActionResult SongView::horizontalEncoderAction(int32_t offset) {
	if (currentSong->songViewLayout == SongViewLayout::Grid) {
...
ActionResult SongView::verticalEncoderAction(int32_t offset, bool inCardRoutine) {
...
void SongView::selectEncoderAction(int8_t offset) {
	switch (currentUIMode) {
	case UI_MODE_HOLDING_SECTION_PAD:
...
void SongView::modEncoderButtonAction(uint8_t whichModEncoder, bool on) {
	UI::modEncoderButtonAction(whichModEncoder, on);
	performActionOnPadRelease = false;
}

void SongView::modButtonAction(uint8_t whichButton, bool on) {
	UI::modButtonAction(whichButton, on);
	performActionOnPadRelease = false;
}

void SongView::modEncoderAction(int32_t whichModEncoder, int32_t offset) {
	performActionOnPadRelease = false;
...
// === COMMAND XXX STUFF === //
void SongView::commandChangeSectionRepeats(int8_t offset) {
	if (performActionOnSectionPadRelease) {
		beginEditingSectionRepeatsNum();
	}
	else {
...
void SongView::commandChangeClipPreset(int8_t offset) {
	performActionOnPadRelease = false;
...
void SongView::commandChangeCurrentSectionRepeats(int8_t offset) {
	if (session.hasPlaybackActive()) {
		if (session.launchEventAtSwungTickCount) {
...
// === UI INPUT === //
void SongView::sectionPadAction(uint8_t y, bool on) {
...
ActionResult SongView::clipCreationButtonPressed(hid::Button i, bool on, bool routine) {
	using namespace deluge::hid::button;
...
Clip* SongView::getClipOnScreen(int32_t yDisplay) {
	if (currentSong->songViewLayout == SongViewLayout::Grid) {
...
uint32_t SongView::getMaxZoom() {
	return currentSong->getLongestClip(true, false)->getMaxZoom();
}

bool SongView::calculateZoomPinSquares(uint32_t oldScroll, uint32_t newScroll, uint32_t newZoom, uint32_t oldZoom) {
...
int32_t SongView::getClipPlaceOnScreen(Clip* clip) {
	return currentSong->sessionClips.getIndexForClip(clip) - currentSong->songViewYScroll;
}

uint32_t SongView::getMaxLength() {
	return currentSong->getLongestClip(true, false)->loopLength;
}

uint32_t SongView::getClipLocalScroll(Clip* clip, uint32_t overviewScroll, uint32_t xZoom) {
	return std::min((clip->loopLength - 1) / (xZoom * kDisplayWidth) * xZoom * kDisplayWidth, overviewScroll);
}

void SongView::noteRowChanged(InstrumentClip* instrumentClip, NoteRow* noteRow) {
...
uint32_t SongView::getGreyedOutRowsNotRepresentingOutput(Output* output) {
...
void SongView::finishedTransitioningHere() {
...
void SongView::clipNeedsReRendering(Clip* clip) {
	if (currentSong->songViewLayout == SongViewLayout::Grid) {
		requestRendering(this, 0xFFFFFFFF, 0xFFFFFFFF);
		return;
	}
...
void SongView::sampleNeedsReRendering(Sample* sample) {
	if (currentSong->songViewLayout == SongViewLayout::Grid) {
		requestRendering(this, 0xFFFFFFFF, 0xFFFFFFFF);
		return;
	}
...
Output* SongView::getOutputFromPad(int32_t x, int32_t y) {
	if (currentSong->songViewLayout == SongViewLayout::Grid) {
...
// === DO STUFF IN THE UI === //
void SongView::goToArrangementEditor() {
...
void SongView::beginEditingSectionRepeatsNum() {
	performActionOnSectionPadRelease = false;
	drawSectionRepeatNumber();
	uiTimerManager.unsetTimer(TimerName::UI_SPECIFIC);
}

void SongView::editNumRepeatsTilLaunch(int32_t offset) {
...
ActionResult SongView::verticalScrollOneSquare(int32_t direction) {
...
Clip* SongView::createNewClip(OutputType outputType, int32_t yDisplay) {
	Clip* clip = nullptr;
	switch (outputType) {
...
Clip* SongView::createNewAudioClip(int32_t yDisplay) {
	actionLogger.deleteAllLogs();
...
Clip* SongView::createNewInstrumentClip(OutputType outputType, int32_t yDisplay) {
	actionLogger.deleteAllLogs();
...
bool SongView::insertAndResyncNewClip(Clip* newClip, int32_t yDisplay) {
...
void SongView::resyncNewClip(Clip* newClip, ModelStackWithTimelineCounter* modelStackWithTimelineCounter) {
...
void SongView::replaceInstrumentClipWithAudioClip(Clip* clip) {
...
void SongView::removeClip(Clip* clip) {
...
void SongView::cloneClip(uint8_t yDisplayFrom, uint8_t yDisplayTo) {
...
bool SongView::setupScroll(uint32_t oldScroll) {
	if (currentSong->songViewLayout == SongViewLayout::Grid) {
		return false;
	}
...
void SongView::transitionToViewForClip(Clip* clip) {
...
void SongView::transitionToSongView() {
	if (currentSong->songViewLayout == SongViewLayout::Grid) {
...
bool SongView::createNewTrackForAudioClip(AudioClip* newClip) {
	// Suss output
...
bool SongView::createNewTrackForInstrumentClip(OutputType type, InstrumentClip* clip, bool copyDrumsFromClip) {
...
void SongView::setupTrackCreation() const { // start clip creation, blink LED corresponding to last type created
	context_menu::clip_settings::newClipType.setupAndCheckAvailability();
	openUI(&context_menu::clip_settings::newClipType);
}

void SongView::exitTrackCreation() {
	...
	requestRendering(&songView);
}

void SongView::setupNewClip(Clip* newClip) {
...
// === UI RENDER === //
void SongView::drawStatusSquare(uint8_t yDisplay, RGB thisImage[]) {
...
void SongView::drawSectionSquare(uint8_t yDisplay, RGB thisImage[]) {
...
void SongView::drawSectionRepeatNumber() {
...
void SongView::redrawClipsOnScreen(bool doRender) {
	if (doRender) {
...
void SongView::redrawNumericDisplay() {
	...
	bool isSessionView =
	    ((currentUI == &songView) || (isPerformanceView && currentSong->lastClipInstanceEnteredStartPos == -1));
...
void SongView::clearNumericDisplay() {
...
void SongView::displayTempoBPM(deluge::hid::display::oled_canvas::Canvas& canvas, StringBuf& tempoBPM, bool clearArea) {
	int32_t yPos = OLED_MAIN_TOPMOST_PIXEL + 3;
...
void SongView::displayCurrentRootNoteAndScaleName(deluge::hid::display::oled_canvas::Canvas& canvas,
...
void SongView::setCentralLEDStates() {
...
void SongView::displayPotentialTempoChange(UI* ui) {
...
int32_t SongView::displayLoopsRemainingPopup(bool ephemeral) {
...
void SongView::potentiallyRenderClipLaunchPlayhead(bool reallyNoTickSquare, int32_t sixteenthNotesRemaining) {
...
void SongView::requestRendering(UI* ui, uint32_t whichMainRows, uint32_t whichSideRows) {
...
void SongView::rowNeedsRenderingDependingOnSubMode(int32_t yDisplay) {
...
void SongView::flashPlayRoutine() {
	switch (currentSong->songViewLayout) {
	case SongViewLayout::Rows: {
...
bool SongView::renderRow(ModelStack* modelStack, uint8_t yDisplay, RGB thisImage[kDisplayWidth + kSideBarWidth],
...
void SongView::gridRenderActionModes(int32_t y, RGB image[][kDisplayWidth + kSideBarWidth],
                                     uint8_t occupancyMask[][kDisplayWidth + kSideBarWidth]) {
	// Action modes column
	...
	case static_cast<uint8_t>(SongViewGridLayoutModifierPad::Loop): {
		if (enableLoopPads) {
			modeActive =
			    matrixDriver.isPadPressed(kDisplayWidth + 1, static_cast<uint8_t>(SongViewGridLayoutModifierPad::Loop));
			modeColour = colours::red; // Red
		}
		break;
	}
	case static_cast<uint8_t>(SongViewGridLayoutModifierPad::LayeringLoop): {
		if (enableLoopPads) {
			modeActive = matrixDriver.isPadPressed(kDisplayWidth + 1,
			                                       static_cast<uint8_t>(SongViewGridLayoutModifierPad::LayeringLoop));
...
bool SongView::gridRenderMainPads(uint32_t whichRows, RGB image[][kDisplayWidth + kSideBarWidth],
                                  uint8_t occupancyMask[][kDisplayWidth + kSideBarWidth], bool drawUndefinedArea) {
...
RGB SongView::gridRenderClipColor(Clip* clip) {
...
Clip* SongView::gridCreateClipInTrack(Output* targetOutput) {
...
AudioClip* SongView::gridCreateAudioClipWithNewTrack() {
	// Allocate new clip
...
InstrumentClip* SongView::gridCreateInstrumentClipWithNewTrack(OutputType type) {
	// Allocate new clip
...
Clip* SongView::gridCreateClip(uint32_t targetSection, Output* targetOutput, Clip* sourceClip) {
	actionLogger.deleteAllLogs();
...
void SongView::gridClonePad(uint32_t sourceX, uint32_t sourceY, uint32_t targetX, uint32_t targetY) {
...
void SongView::gridStartSection(uint32_t section, bool instant) {
	if (instant) {
...
void SongView::gridToggleClipPlay(Clip* clip, bool instant) {
	session.toggleClipStatus(clip, nullptr, instant, kInternalButtonPressLatency);
}

ActionResult SongView::gridHandlePads(int32_t x, int32_t y, int32_t on) {
...
ActionResult SongView::gridHandlePadsEdit(int32_t x, int32_t y, int32_t on, Clip* clip) {
...
ActionResult SongView::gridHandlePadsLaunch(int32_t x, int32_t y, int32_t on, Clip* clip) {
...
ActionResult SongView::gridHandlePadsLaunchImmediate(int32_t x, int32_t y, int32_t on, Clip* clip) {
...
ActionResult SongView::gridHandlePadsLaunchWithSelection(int32_t x, int32_t y, int32_t on, Clip* clip) {
	if (on) {
...
void SongView::gridHandlePadsLaunchToggleArming(Clip* clip, bool immediate) {
	if (immediate) {
...
void SongView::gridHandlePadsWithMidiLearnPressed(int32_t x, int32_t on, Clip* clip) {
	if (clip != nullptr) {
		if (clip->type != ClipType::AUDIO) {
...
void SongView::gridTransitionToSessionView() {
	Sample* sample;
...
void SongView::gridTransitionToViewForClip(Clip* clip) {
...
const uint32_t SongView::gridTrackCount() {
	uint32_t count = 0;
...
uint32_t SongView::gridClipCountForTrack(Output* track) {
	uint32_t count = 0;
	for (int32_t idxClip = 0; idxClip < currentSong->sessionClips.getNumElements(); ++idxClip) {
		Clip* clip = currentSong->sessionClips.getClipAtIndex(idxClip);
...
uint32_t SongView::gridTrackIndexFromTrack(Output* track, uint32_t maxTrack) {
	if (maxTrack <= 0) {
		return -1;
	}
...
Output* SongView::gridTrackFromIndex(uint32_t trackIndex, uint32_t maxTrack) {
	uint32_t count = 0;
	Output* currentTrack = currentSong->firstOutput;
	while (currentTrack != nullptr) {
		if (currentTrack->getActiveClip() != nullptr) {
...
int32_t SongView::gridYFromSection(uint32_t section) {
...
int32_t SongView::gridSectionFromY(uint32_t y) {
...
int32_t SongView::gridXFromTrack(uint32_t trackIndex) {
...
int32_t SongView::gridTrackIndexFromX(uint32_t x, uint32_t maxTrack) {
	if (maxTrack <= 0) {
		return 0;
	}
...
Output* SongView::gridTrackFromX(uint32_t x, uint32_t maxTrack) {
...
Clip* SongView::gridClipFromCoords(uint32_t x, uint32_t y) {
	auto maxTrack = gridTrackCount();
	Output* track = gridTrackFromX(x, maxTrack);
	if (track == nullptr) {
		return nullptr;

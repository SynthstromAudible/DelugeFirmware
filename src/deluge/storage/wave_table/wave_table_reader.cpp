...
Error WaveTableReader::readBytesPassedErrorChecking(char* outputBuffer, int32_t num) {
	...
	while (num--) {
		...
		*outputBuffer = storageManager.mDeserializer.fileClusterBuffer[byteIndexWithinCluster];
		outputBuffer++;
...
Error WaveTableReader::readNewCluster() {
	...
	UINT bytesRead;
	FRESULT result = f_read(&fileSystemStuff.currentFile, storageManager.mDeserializer.fileClusterBuffer,

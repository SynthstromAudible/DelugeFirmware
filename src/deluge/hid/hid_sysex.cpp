	...
	if (data[4] == 0 or data[4] == 1) {
		...
		if (display->haveOLED()) {
			...
			if (oledDeltaImage == nullptr) {
				oledDeltaImage = (uint8_t*)GeneralMemoryAllocator::get().allocMaxSpeed(sizeof(
				    uint8_t
				        [OLED_MAIN_HEIGHT_PIXELS >> 3]
				        [OLED_MAIN_WIDTH_PIXELS])); //@TODO: Check if there is a speed difference putting this into nonaudio
			}
		}

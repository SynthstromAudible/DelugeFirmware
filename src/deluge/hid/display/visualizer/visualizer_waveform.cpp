...
// Helper function for amplitude scaling with 2:1 compression
int32_t scaleWaveformAmplitude(int32_t sample, int32_t graph_height) {
	if (sample == 0) {
		return 0;
	}
	...
	float compressed_amplitude = std::pow(normalized_amplitude, 0.5f);
...
void renderVisualizerWaveform(oled_canvas::Canvas& canvas) {
	...
	// Use reference amplitude for consistent waveform scaling
	// Ensures waveform peaks align with VU meter clipping indication
	...
	for (uint32_t i = 0; i < num_samples_to_display; i++) {
		...
		int32_t scaled_height = scaleWaveformAmplitude(sample, k_graph_height);

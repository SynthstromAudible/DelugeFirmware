...
pad_s MatrixDriver::charToPad(uint8_t value) {
#if DELUGE_MODEL == DELUGE_MODEL_40_PAD
	return {
	    (unsigned int)value % 10,
	    ((unsigned int)value % 70) / 10,
	};
#else
...
uint8_t MatrixDriver::padToChar(pad_s p) {
#if DELUGE_MODEL == DELUGE_MODEL_40_PAD
	return p.y * 10 + p.x;
#else
	uint8_t value = 9 * p.y + (p.x / 2);
	if ((p.x % 2) == 1) {
...
bool MatrixDriver::isPad(uint8_t value) {
#if DELUGE_MODEL == DELUGE_MODEL_40_PAD

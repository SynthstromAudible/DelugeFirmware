...
#define MIDI_DEVICES_FOLDER "MIDI_DEVICES"
#define MIDI_DEVICES_XML "MIDI_DEVICES/MIDIDevices.XML"
...
void readDevicesFromFile() {
	...
	if (!success) {
		...
		// if so, let's move it to the new MIDI_DEVICES folder (but first make sure folder exists)
		FRESULT result = f_mkdir(MIDI_DEVICES_FOLDER);

...
bool Clip::opportunityToBeginSessionLinearRecording(ModelStackWithTimelineCounter* modelStack, bool* newOutputCreated,
	...
	*newOutputCreated = false;
	...
	if (playbackHandler.recording != RecordingMode::OFF && wantsToBeginLinearRecording(modelStack->song)) {
		...
		D_TRY_CATCH(beginLinearRecording(modelStack, buttonPressLatency), error, {
...
trimFoundParamManager:
		...
		if (!paramManager.containsAnyMainParamCollections()) {
			...
			if (otherClip) {
				...
				D_TRY_CATCH(paramManager.cloneParamCollectionsFrom(&otherClip->paramManager, false, true), error, {
		...
		if (output->activeClip && output->activeClip->beingRecordedFromClip == this) {
			...
			if (type == ClipType::AUDIO) {
				...
				if (repeatCount >= 1) {
					...
					D_TRY_CATCH(output->clipInstances.insertAtIndex(clipInstanceI), error, { return false; });

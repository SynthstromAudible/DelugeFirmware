...
void MelodicInstrument::receivedCC(ModelStackWithTimelineCounter* modelStackWithTimelineCounter, MIDICable& cable,
	...
	case MIDIMatchType::CHANNEL:
		...
		if (ccNumber == CC_EXTERNAL_SOSTENUTO_PEDAL && type != OutputType::MIDI_OUT
		    && runtimeFeatureSettings.isOn(RuntimeFeatureSettingType::EnableSostenutoPedal)) {
			int32_t paramValue = (value >= 64) ? 2147483647 : -2147483648;
		...
		if (ccNumber == CC_EXTERNAL_SOFT_PEDAL && type != OutputType::MIDI_OUT
		    && runtimeFeatureSettings.isOn(RuntimeFeatureSettingType::EnableSoftPedal)) {
			int32_t paramValue = (value >= 64) ? 2147483647 : -2147483648;
...
void MelodicInstrument::releaseSostenutoVoices(ModelStackWithTimelineCounter* modelStack) {
	...
	for (const auto& voice : soundInstrument->voices()) {
		if ((voice->pedalState & Voice::PedalState::SostenutoDeferred) != Voice::PedalState::None) {
			voice->pedalState =
			    voice->pedalState & ~(Voice::PedalState::SostenutoCapture | Voice::PedalState::SostenutoDeferred);
			voice->noteOff(modelStackWithSoundFlags, true, true); // ignoreSustain — force release
		}
	}
}
...
void MelodicInstrument::stopAnyAuditioning(ModelStack* modelStack) {
	...
	if (type != OutputType::MIDI_OUT && runtimeFeatureSettings.isOn(RuntimeFeatureSettingType::EnableSustainPedal)) {

...
public:
	...
	void writeParamAsAttribute(StorageManager &bdsm, char const* name, int32_t p, bool writeAutomation, bool onlyIfContainsSomething = false,
	                           int32_t* valuesForOverride = NULL);
	void readParam(StorageManager &bdsm, ParamCollectionSummary* summary, int32_t p, int32_t readAutomationUpToPos);
...
public:
	...
	int32_t knobPosToParamValue(int32_t knobPos, ModelStackWithAutoParam* modelStack) override;
	int32_t paramValueToKnobPos(int32_t paramValue, ModelStackWithAutoParam* modelStack) override;
	bool writeToFile(StorageManager &bdsm, bool mustWriteOpeningTagEndFirst = false);
	void readFromFile(StorageManager &bdsm, ParamCollectionSummary* summary, int32_t readAutomationUpToPos);

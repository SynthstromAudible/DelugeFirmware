...
q31_t SidLpFilter::setConfig(q31_t lpfFrequency, q31_t lpfResonance, FilterMode lpfmode, q31_t lpfMorph,
                             q31_t filterGain) {
	lpfMode = lpfmode;
	morph = lpfMorph;

	// Determine if we should use 12DB mode or 24DB mode for SID_LOWPASS
	// We'll use 12DB for lower morph values, 24DB for higher ones
	use12DB = (lpfMode == FilterMode::SID_LOWPASS && morph < 536870912); // Threshold at 50%

	// Hot sid ladder - needs oversampling and stuff
	...
	if (lpfMode != FilterMode::SID_LOWPASS_DRIVE) { // Cold sid ladder only
...
[[gnu::hot]] void SidLpFilter::doFilter(q31_t* startSample, q31_t* endSample, int32_t sampleIncrement) {
	...
	// Full ladder (drive)
	else if (lpfMode == FilterMode::SID_LOWPASS_DRIVE) {
...
[[gnu::always_inline]] inline q31_t SidLpFilter::do12dBLPFOnSample(q31_t input, LpLadderState& state) {
...
[[gnu::always_inline]] inline q31_t SidLpFilter::do24dBLPFOnSample(q31_t input, LpLadderState& state) {
...
[[gnu::always_inline]] inline q31_t SidLpFilter::doDriveLPFOnSample(q31_t input, LpLadderState& state) {

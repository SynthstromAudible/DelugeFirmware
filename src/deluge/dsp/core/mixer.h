...
/// @tparam T The type of the samples to mix.
template <typename T>
struct SIMDMixer : SampleMixer<Argon<T>>, BlockMixer<T> {
	using value_type = T; ///< The type of the samples to mix.
	...
	void renderBlock(Signal<T> input_a, Signal<T> input_b, Buffer<T> output) final {
		auto input_a_view = argon::vectorize_load(input_a);
		auto input_b_view = argon::vectorize_load(input_b);
		auto output_view = argon::vectorize_store(output);

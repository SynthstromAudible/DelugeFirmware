...
// --- USB drain, end to end ---
//
// The DIN counterparts live in midi_din_drain_tests.cpp. USB had none, and both transports now share
// one CC-lane policy, so these pin the USB half of it: coalescing, debt reordering, and the fact that
// only the value byte of a packed event is ever rewritten.

namespace {
/// Packs a channel CC the way ConnectedUSBMIDIDevice does: byte0 cable/CIN, then status, CC, value.
uint32_t packUsbCC(uint8_t channel, uint8_t cc, uint8_t value) {
...
TEST(MIDIUsbDrain, ContinuousCCsAreCoalescedThenOrderedByDebt) {
	...
	for (uint8_t v = 0; v < 5; v++) {
		(void)queue.enqueue_message(packUsbCC(0, 40, v), MIDIIntent::Continuous);
	}
	(void)queue.enqueue_message(packUsbCC(0, 10, 7), MIDIIntent::Continuous);
	(void)queue.enqueue_message(packUsbCC(0, 25, 9), MIDIIntent::Continuous);
...
TEST(MIDIUsbDrain, EventCCsKeepTheirOrderAndTheirDuplicateValues) {
	...
	for (auto const& c : ccs) {
		(void)queue.enqueue_message(packUsbCC(0, c[0], c[1]), MIDIIntent::Event);
	}

...
TEST_GROUP(MatricealPropertyTest) {
	std::mt19937 rng;
	MusicalKey cMajor;
	...
	uint32_t computeLCM(const MatricealEngine& engine) {
	...
	void randomizeLane(MatricealLane & lane, int16_t minVal, int16_t maxVal) {
...
TEST(MatricealPropertyTest, noteAlwaysInMidiRange) {
...
TEST(MatricealPropertyTest, allTriggersZeroMeansAllRests) {
...
TEST(MatricealPropertyTest, deterministicLockReproducesOutput) {
	for (int trial = 0; trial < 20; trial++) {
		MatricealEngine engine;
		engine.trigger.length = std::uniform_int_distribution<int>(1, 8)(rng);
		for (int i = 0; i < engine.trigger.length; i++) {
		...
		randomizeLane(engine.pitch, 48, 84);
...
TEST(MatricealPropertyTest, pingpongFullCycleLength) {
...
TEST(MatricealPropertyTest, disabledLanesNeutral) {
	for (int trial = 0; trial < 20; trial++) {
		MatricealEngine engine1;
		MatricealEngine engine2;
		...
		for (int i = 0; i < 3; i++) {
			int16_t v = std::uniform_int_distribution<int16_t>(48, 84)(rng);
		...
		for (int step = 0; step < 12; step++) {
			MatricealNote n1 = engine1.step(cMajor);
			MatricealNote n2 = engine2.step(cMajor);
...
TEST(MatricealPropertyTest, intervalReversibility) {
	for (int x = 1; x <= 7; x++) {
		MatricealEngine engine;
...
TEST(MatricealPropertyTest, patternRepeatsAtLCMBoundary) {

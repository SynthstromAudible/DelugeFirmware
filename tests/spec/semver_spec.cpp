...
describe semver(
    "SemVer", $ {
	    ...
	    it(
	        "parses a semver with a pre-release tag", _ {
	    ...
	    context(
	        "with an alpha pre-release version number", _ {
		        ...
		        it(
		            "is less than its full release", _ {
			            expect(pre_release).to_be_less_than(SemVer{1, 5, 2});
		            });

		        it(
		            "is less than its beta release", _ {
			            expect(pre_release).to_be_less_than(SemVer{1, 5, 2, "beta"});
		            });

		        it(
		            "is greater than the previous core version", _ {
			            expect(pre_release).to_be_greater_than(SemVer{1, 5, 1});
		            });
	        });
    });

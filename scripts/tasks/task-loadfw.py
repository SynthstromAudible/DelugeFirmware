...
def argparser():
	...
	parser.add_argument("port_number",
						help="MIDI port number (example: 0)",type=int
						)
	parser.add_argument("hex_key",
						help="8-digit Deluge Hex Key (example 1234abcd)"
						)
...
def load_fw(port,handshake,file,delay_ms=1):
	...
	with midiout:
		data = bytearray(598)
		for i, segment in util.progressbar( list(enumerate((binary[pos:pos + 512] for pos in range(0, len(binary), 512))) ) ,"Firmware Upload "):
		...
		data[4:18] = pack87(handshake.to_bytes(4,byteorder='little') +
							len(binary).to_bytes(4, byteorder='little') +
							checksum.to_bytes(4, byteorder='little'), 14)
		data[18] = 0xf7
		midiout.send_message(data)

...
```
app MIDI engine (parse/produce MIDI; owns cables, MPE zones)
   │  deluge_midi_read(port, dst, max) / deluge_midi_write(port, src, len)
   ▼
BSP usb_midi (owns per-device RX/TX buffers + ring, USB transfer descriptors,
   the receive/send state machine; drives Renesas pipes; drains HAL completions)
...
1. **Foundation:** create `src/bsp/rza1/usb_midi.{c,h}` + the boundary additions
   above (declarations + stubs). No behaviour change.
2. **Move transport state down:** relocate the `g_usb_midi_*_utr` descriptor
...
3. **Send path:** move `flushUSBMIDIOutput`, `flushUSBMIDIToHostedDevice`, the
   `usbSendComplete*` functions into the BSP; expose via `deluge_midi_write`/flush;
   convert send-complete to a polled count.
4. **Receive path:** move `setupUSBHostReceiveTransfer`, `usbReceiveComplete*`,
   `brdyOccurred` into the BSP; the app pulls bytes via `deluge_midi_read`; keep the
   USB-MIDI-event-packet decode + sysex in the app.
5. **HAL `rohan_midi` functions:** reduce to generic pipe-complete event recording;
   stop including app MIDI headers; the BSP drains the events.
6. **Strip + rewire:** remove transport from `midi_engine.cpp`; app uses only the
   boundary; delete the `deluge/drivers/usb/userdef/r_usb_pmidi_config.h` app-tree
   include from the HAL (relocate the USB-MIDI *config* to the BSP).

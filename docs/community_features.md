...
- In case of issue loading a song, a failsafe mode will be triggered :
    - If a crash occurs at startup loading a song, it will automatically deactivate STARTUPSONG feature and will show you at startup which song is having issues.
    - STARTUPSONG feature will be deactivated until either:
...
- In case of the crash still persists, you can always factory reset by holding `SELECT` while powering on the Deluge, which will force to STARTUPSONG to boot up on `NEW SONG`.

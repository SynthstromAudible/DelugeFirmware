...
When a note plays, the Deluge first sets the pitch voltage on the clip’s CV output and then raises the matching gate output. The gate is only raised once the new voltage has been sent, so a connected synthesizer will not sound the previous pitch for a moment at the start of each note.

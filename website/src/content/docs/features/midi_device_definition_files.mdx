...
Updated MIDI CC name's will be displayed when you press :key[GOLD ENCODER] / :key[TURN GOLD ENCODER] the CC is assigned to and also when automating that CC in `Automation View`

## Rename a MIDI CC

To rename a `MIDI CC`, enter `Automation View` and select the CC you wish to rename using the :key[TURN SELECT] or using the :key[SHIFT] + :key[SHORTCUT PAD].
...
The new `MIDI CC` name will be immediately visible in `Automation View` and you can assign that CC to a `Gold Knob` in the MIDI clip and it will show that name when you press :key[GOLD ENCODER] and when you :key[TURN GOLD ENCODER] to change the CC value.
...
Simply save the song by pressing :key[SAVE]
...
While in the MIDI clip, press :key[SAVE] + :key[MIDI]
...
## Hiding / Unhiding Unlabeled CC'S

You can hide CC's that are unlabeled by toggling `Hide Unlabeled CC (HIDE)` via the `MIDI > Device Definition (DEVI)` menu.

With unlabeled CC"s hidden, when you scroll through assignable CC's (e.g. :key[Gold] + :key[Turn Select] or :key[Turn Select] in Automation View), you will only see labeled CC's.

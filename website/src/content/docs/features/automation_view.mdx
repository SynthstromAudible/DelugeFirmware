...
<Badge text="c1.0 Feature" variant="tip" />
...
<LinkPreviewCard
  href="https://www.youtube.com/watch?v=KV5hWgcanxI"
  title="Automation Mode - Part 1: Experimentation"
  description="Ron Cavagnaro"
/>
<LinkPreviewCard
  youtubeId="RGRlln8OW1s"
  title="Deluge Is Now an Advanced Glitch Effects Pedal - Automation Mode Part 2"
  description="Ron Cavagnaro"
/>
<LinkPreviewCard
  youtubeId="KR3xTCeQj0Y"
  title="MIDI CC and Line In FX - Automation Mode Part 3"
  description="Ron Cavagnaro"
/>
<LinkPreviewCard youtubeId="J8hibZYJhoY" description="Red Means Recording" />
...
- The Automation Arranger View is accessed from within Arranger View by pressing :key[Clip]
  - Note: the automation arranger view editor for a specific parameter can be accessed directly from the arranger song menu for that parameter (e.g., the one you would access by pressing :key[Select] or by using the :key[Shift + Pad] shortcut) by pressing the :key[Clip] button while in the parameter menu.
- The Automation Clip View is accessed from within the Clip View by pressing the Clip button
  - Note: the automation clip view editor for a specific parameter can be accessed directly from the clip sound menu for that parameter (e.g., the one you would access by pressing on select encoder or by using the :key[Shift + Pad] shortcut) by pressing the :key[Clip] button while in the parameter menu.
...
> You can automate MIDI CC's 0-119
> You can automate Monophonic (Channel) Expression parameters (X - Pitch Bend, Y - Mod Wheel, Z - Channel Pressure / Aftertouch). Note: Mod Wheel is CC 1.
...
6. Automation for CV Clips

> In CV Clips, you can automate Monophonic (Channel) Expression parameters (X - Pitch Bend, Y - Mod Wheel, Z - Channel Pressure / Aftertouch)
...
For Non-Audio Instrument Clip Types (Synths, Kits, MIDI and CV), the sidebar from these instrument clip types is available in the Automation Clip View so that you can audition sounds while viewing/editing parameter automations.
...
Automation View can be accessed from the Arranger View and Clip Views by pressing the :key[Clip] button.

Automation View can also be accessed from the Sound menu for any automatable parameter, patch cable, modulation depth. While in the menu, press :key[Clip] to toggle the Automation Editor for that parameter.
...
2. By turning select
3. From the menu by selecting an automatable parameter for editing and then pressing clip
...
- For Synths, MIDI, CV and Audio clips, automations are recorded/edited on a clip basis (e.g. not on a per note row basis).
...
- enable you to quickly change parameters in focus for editing by turning select or using :key[Shift + Shortcut Pad] or going back to automation overview using :key[AffectEntire] or :key[Shift] + :key[Clip] or :key[Audition Pad] + :key[Clip]
...
<img
  width="297"
  alt="Screenshot 2024-03-16 at 5 13 50 PM"
  src="https://github.com/seangoodvibes/DelugeFirmware/assets/138174805/3d5dded1-efc2-4cb6-ad07-df2942fdc66e"
/>
...
<img
  width="293"
  alt="Screenshot 2024-03-16 at 5 07 36 PM"
  src="https://github.com/seangoodvibes/DelugeFirmware/assets/138174805/e6f853f7-56bc-4102-8cc3-9b5a86bd4bfb"
/>
...
- extend/shorten the arranagement / clip length using the current combo (:key[Shift] + :key[Turn Horizontal])

> **Note:** For Arranger View, Synth, MIDI, CV and Kit Clips with :key[AffectEntire] enabled, and Audio Clips, the Automation Editor grid will display the shortened arranagement/clip size (e.g. you cannot edit automation past the arrangement/clip length boundary).

- in automation clip view, extend/shorten a note row length using the current combo (:key[Audition Pad] + :key[Turn Horizontal])

> **Note 1:** For Synth, MIDI and CV clips, the automation editor will not display the shortened note row, but you will see that it has been shortened when playing the clip as the cursor will move independently of other rows).
...
- in automation clip view, double clip length using the current combo (:key[Shift] + Press :key[Horizontal])
- enable you to quickly turn interpolation on/off by using the :key[Shift] + :key[Interpolation Pad] shortcut (the interpolation shortcut pad is the interpolation pad in the first column of the grid, second from the top). The interpolation shortcut pad will blink every few seconds to remind you that interpolation is on.
...
- access the sample browser
- use regular grid shortcuts (except for :key[Shift] + Param :key[Pad])
...
Interpolation itself can be toggled on/off by using the :key[Shift] + :key[Interpolation Pad] shortcut (the interpolation shortcut pad is the interpolation pad in the first column of the grid, second from the top). The interpolation shortcut pad will blink every few seconds to remind you that interpolation is on.

Interpolation is only applied when you are physically entering a long automation pad press (e.g. when you hold one :key[pad] and press a second :key[pad] to connect the pads together). If you turn interpolation on and do not edit any parameter values, it will not have any effect.
...
Currently in the Arranger View and Instrument Clip View if you press :key[Horizontal] and press the :key[Back] button at the same time, all **Clips/Notes** AND **Automations** are cleared.
...
Currently in the Clip View if you press :key[Vertical] and :key[Turn Horizontal] or hold :key[Shift] + :key[Press Turn Horizontal], it will shift Notes / Audio File AND the ALL Automations of the entire clip horizontally.
...
Currently in the Instrument Clip View if you press down on a :key[note pad] and :key[Turn Horizontal], it will nudge the Note AND the Automations for that same step.
...
Currently in the Instrument Clip View if you hold down an :key[audition pad] and press a :key[shortcut pad] pad on the grid, it will open the menu corresponding to that shortcut pad.

By default in the Automation Clip View that same behaviour of holding an :key[audition pad] and pressing a :key[shortcut pad] pad is disabled in favour of you being able to hold down an audition pad and adjust the automation lane values so that you can audible hear the changes to the sound while adjusting automation settings.
...
- While in the Arranger Song / Clip Sound Menu for a specific automatable parameter, press `CLIP` to enter the Automation Editor for that parameter

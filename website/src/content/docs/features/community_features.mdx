...
- Horizontal Menu controls.
  ...
  - :key[PRESS TURN SELECT] to change the selected menu item
  ...
  - You can switch between the different pages of the horizontal menu using the :key[SCALE] and :key[CROSSSCREEN]
    - When in a Horizontal Menu, the :key[SCALE] and :key[CROSSSCREEN] buttons LED will light up if the Horizontal Menu has more than one page.
    - Use :key[CROSSSCREEN] button to switch to the next page
    - Use :key[SCALE] button to switch to the previous page
  - If the selected menu item is a submenu, you can enter the submenu by pressing the :key[SELECT] encoder or the lit :key[SYNTH], :key[KIT], :key[MIDI], or :key[CV] buttons under the submenu item selected

...
- <Badge text="c1.3" variant="tip" /> Added Community Feature toggle `Send MIDI CC on Clip / Section Launch (CCLN)` in `SETTINGS > COMMUNITY FEATURES`.
- Default is `OFF`.
- When `OFF`, MIDI clips do not send automated MIDI CC values when a clip or section is launched.
...
- `Send MIDI CC on Clip / Section Launch (CCLN)` <Badge text="c1.3" variant="tip" />
  - When On, MIDI clips send their current automated MIDI CC values when a clip or section is launched.
  - When Off (default), this launch-time MIDI CC send is disabled.

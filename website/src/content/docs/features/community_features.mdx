...
- (#837) <Badge text="c1.1" variant="tip" /> `MIDI control over transpose / scale.`
  ...
  - Choice of octave is determined by the first note received by each song (this is reset on each song load). The
    first transpostion will move the root note by at most a fifth, to the closest matching note. Subsequent transpose
...
- (#3277) <Badge text="c1.3" variant="tip" /> Added new Digital reverb model.
...
### 4.5.11 - Round-Robin Sample Variants

- Each oscillator sample zone in Synth and Kit sound sources can now hold up to 4 sample variants: the primary
  sample plus up to 3 alternates, played in rotation each time the zone is triggered - the classic
  multi-sample-per-pad workflow known from MPC-style samplers, useful for removing the "machine gun" effect on
  repeated drum hits.
- For a detailed description of this feature, please refer to the feature documentation:
  [Round-Robin Sample Variants Documentation](/features/round_robin_sample_variants)
...
- `Horizontal Menus` <Badge text="c1.3" variant="tip" />
  - When On, on OLED, some menu items render in horizontal menus, with multiple items visible and editable at the same time.
- `Rounded Corners` <Badge text="c1.3" variant="tip" />
  - When On, on OLED, corners are rounded in the menus.
...
- `Round-Robin Sample Variants (VARI)` <Badge text="c1.3" variant="tip" />
  - When On, enables the `VARIANTS` menu under `OSC1`/`OSC2` for sample-mode oscillators, letting you load up to 3
    alternate samples per zone that play in rotation. See [Round-Robin Sample Variants Documentation](/features/round_robin_sample_variants).
    When Off, the menu is hidden and playback always uses the primary sample; variant data already saved in songs is

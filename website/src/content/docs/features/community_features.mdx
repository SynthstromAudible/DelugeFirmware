...
- <Badge text="c1.3" variant="tip" /> A new submenu has been added to the `Sound` menu called `Randomizer`. This menu includes some of the Randomizer parameters present in the Arpeggiator submenu, which have been enabled to work also for non-arpeggiated notes. That is, regular notes from the sequencer when the arpeggiator is off, which are the following:

...
- (#889) <Badge text="c1.1" variant="tip" /> `Master MIDI Follow Mode` whereby after setting a master MIDI follow channel for Synth/MIDI/CV clips, Kit
  ...
  - (#4526) <Badge text="c1.3" variant="tip" /> Added specific track control to `MIDI FOLLOW` enabling you to have instant mixer control of the parameters of up to 16 tracks.
    - You can now configure up to 16 channels in the MIDI Follow menu.
    - CC's received on the specific track channels are directed to the active clip of the corresponding track.
    - Track 1 corresponds to bottom track in `ARRANGER VIEW` or left track in `GRID VIEW`.
    - You can also now configure the Channel `TRACK` for MIDI Follow Feedback. If a specific track is selected and has been configured
      for MIDI Follow control, the Deluge will send feedback of the CC values for that specific track.
    - MIDI Follow Feedback can now be sent to both the selected track channel and a regular follow channel by selecting `TRACK + A`, `TRACK + B`, or `TRACK + C`.
      - Combined feedback modes send to the configured track channel first, then to the configured regular channel.
      - If both targets resolve to the same MIDI channel, feedback is sent only once.
  - (#4526) <Badge text="c1.3" variant="tip" /> Added new default CC 89 to toggle the mute of the active clip in a specific track.
    - Toggle mute: CC Value > 0 (Send a CC value greater than 0 to toggle mute for the active clip in a specific track)
      - If used together with `SHIFT`, it will trigger instant mute.
  - (#4526) <Badge text="c1.3" variant="tip" /> Added new default CC 90 to toggle solo of the active clip in a specific track.
...
- (#395) <Badge text="c1.0" variant="tip" /> Load synth presets into kit rows by holding the row's :key[audition pad] + :key[synth]. Saving can be done by holding the
...
- (#2260) <Badge text="c1.2" variant="tip" /> Added `AUDIO EXPORT`, an automated process for exporting `CLIP's` while in `Session View` and `TRACK's` while in `ARRANGER VIEW`. Press `SAVE + RECORD` to start exporting audio. Press `BACK` to cancel the export and stop recording and playback.
...
- (#2315) <Badge text="c1.2" variant="tip" /> The display now shows the number of Bars (or Quarter Notes for the last bar) remaining until a clip or section launch event in all Session Views (Grid, Row, Performance).
...
### 3.27 - Updated UI for Creating New Clips in New Tracks in Session Grid View

- (#2429) <Badge text="c1.2" variant="tip" /> Added new mechanism for creating New Clips in New Tracks in `Session Grid View`
  - When you press a pad in a new track, a menu will appear asking you to confirm the type of clip you wish to create. The clip type selected to be created is shown on the display and is also indicated by the clip type button that is blinking.
    ...
    - If you press and hold a pad, you can choose a different type to create in a number of ways:
      ...
      - by pressing one of the clip type buttons (e.g. :key[synth], :key[kit], :key[midi], :key[cv]).
    ...
    - If you press :key[back] before releasing a pad or selecting a clip type, it will cancel the clip creation.
  - These changes only apply to `Session Grid View` and NOT `Session Row View`

### 3.28 - Add Clip Settings Menu in Session View to set Clip Mode and Clip Name and convert Instrument Clips to Audio Clips

- (#2299) <Badge text="c1.2" variant="tip" /> Holding a clip in `Session Grid View` or the status pad for a clip in `Session Row View` and pressing :key[Select] brings up a `CLIP SETTINGS` menu.
...
- (#2537) <Badge text="c1.2" variant="tip" /> Added `DEFAULTS (DEFA) > UI > ACCESSIBILITY (ACCE)` menu which contains accessibility changes to the Deluge UI to make the deluge more accessible to users with disabilities. These changes included:
  - `Shortcuts (SHOR)` to make specific shortcut combinations more accessible for users with mobility restrictions.
    - :key[horizontal + play] is changed to :key[crossscreen + play]
...
- (#2645) <Badge text="c1.3" variant="tip" /> Added new `MIDI LEARN` menu to the `SONG` menu. In `Session Grid View` this menu enables you to learn `Clip/Section Launch`. In `Session Row View` this menu enables you to learn the `Clip/Section Launch` and `Instrument`.
...
### 3.32 - Updated UI for Creating New Clips in Session Row View
...
- (#4494) <Badge text="c1.3" variant="tip" /> Added battery level display in the `SETTINGS` menu
...
- (#630) <Badge text="c1.0" variant="tip" /> In Session View, select `AFFECT ENTIRE` and the `SIDECHAIN`-related parameter button. Adjust the `UPPER` gold
...
- (#163) <Badge text="c1.0" variant="tip" /> In Session View, pressing a clip row pad + :key[Shift + Turn Vertical] changes the selected row color. This is
  similar to the shortcut when setting the color while in a clip view.

### 4.1.3 - Fill Clips and Once Clips

- (#2299) <Badge text="c1.2" variant="tip" /> Holding a clip in `Session Grid View` or the status pad for a clip in `Session Row View` and pressing :key[Select] brings up a `CLIP SETTINGS` menu. In this menu, you will find a submenu for `CLIP MODE`.
...
- (#251) <Badge text="c1.0" variant="tip" /> Add new grid session layout to Session View. All functionality from (classic) row layout applies except for the
  following:
  ...
  - In Session View turn :key[Song + Select] to switch between row layout and grid layout
...
- (#711) <Badge text="c1.1" variant="tip" /> Adds a new performance view, accessible from Song and Arranger View's using the Keyboard button. Each column on the grid represents a different "FX" and each pad/row in the column corresponds to a different FX value.
  ...
  - Specifications:
    ...
    - Save defaults to `SETTINGS/PerformanceView.XML` file
      ...
      - Adjustable default held :key[pad] settings for each FX column via Performance View or PerformanceView.xml (
        simply change a held :key[pad] in Performance View and save the layout to save the layout with the held pads).
...
- (#1545 and #2418) <Badge text="c1.2" variant="tip" /> Add song macros. Macros are a way to quickly switch playing clips without needing to go into Session View.
  ...
  - Inside a `CLIP TIMELINE VIEW`, hold :key[song] button and press the `LEFT SIDEBAR` to launch a macro.
  - In `KEYBOARD VIEW`, macros are available as a sidebar control.
    - :key[Shift] makes the launch immediate just like in Session View.
...
- (#241) <Badge text="c1.0" variant="tip" /> Automation Clip View is a new view that complements the existing Clip View.
  - It is accessed from within the Clip View by pressing :key[clip] (which will blink to indicate you are in the
    Automation View).
...
### 4.4.1.9 - Harmonic Brush (Chord Placement) <Badge text="c1.3" variant="tip" />

Place a whole chord into the piano roll the same way you place a note. The chord can come from any chord-producing mode (`CHORD`, `CHORD LIBRARY`) or even a chord played by hand on any keyboard layout. The brush captures whatever notes are currently sounding, so it is not tied to any one chord source. Requires the `Chord Brush (BRSH)` community feature (see the [Community Features Menu](#5-community-features-menu-aka-runtime-settings)).

- **Arm:** In a keyboard view, hold the notes of a chord (press a chord pad in `CHORD`/`CHORD LIBRARY`, or hold a chord by hand) and press :key[Select] to capture the sounding notes as a "harmonic brush". The display shows `PEND`. The brush stays armed until you replace or clear it.
- **Place:** Switch to the piano roll with :key[Keyboard] and tap a step to stamp the whole chord at that column. The chord keeps its own pitches; only the column (time position) matters. The display shows `PLCD`. Tap multiple steps to stamp the chord repeatedly, useful for syncopated rhythms.
- **Set length:** Exactly like a note, hold the start step and press a step further along to stretch the whole chord to that length.
- **Undo:** :key[Back] removes the most recently stamped chord, one stamp per press.
- **Replace:** Arm a new chord at any time to swap the brush.
- **Clear:** Press :key[Select] while the brush is armed (in the keyboard view or the piano roll) to clear it. The display shows `CLR` and normal single-note editing resumes.
...
- `Alternative Playback Start Behaviour (STAR)` <Badge text="c1.2" variant="tip" />
  - When On, the behaviour of playback start shortcuts changes as follows:
    - With playback off, pressing :key[play] will start playback from the current grid scroll position
    - With playback off, pressing :key[horizontal + play] will start playback from the start of the arrangement or clip
...
- `Alternative Tap Tempo Behaviour (TAPT)` <Badge text="c1.3" variant="tip" />
  - When On, the number of `TAP TEMPO` button presses to engage :key[taptempo] is changed to `FOUR (4)` to avoid mistakingly changing tempo.
...
- `Chord Brush (BRSH)` <Badge text="c1.3" variant="tip" />
  - When On, enables the Harmonic Brush: capture a sounding chord in a keyboard view with :key[Select] and stamp it into the piano roll like a note. See [Harmonic Brush (Chord Placement)](#441---keyboard-view) under Keyboard View features. On by default.

...
- `Step Record (STEP)` <Badge text="c1.4" variant="tip" />
  - When On, holding :key[Record] and pressing :key[Keyboard] in clip view enters Step Record mode, where notes are entered one step at a time while the transport is stopped: each note - played on the audition pads, the keyboard view, or a MIDI controller - lands at the step cursor rather than at the live playhead position, and the cursor advances one step when the note is released. Chords and stacked kit drums land on one step; :key[Record] while holding a note ties it (extends it by one step and advances) and :key[Record] with no notes held enters a rest - both work on melodic and kit clips. :key[Back] exits the mode. For a detailed description, please refer to the feature documentation: [Step Record Documentation](/features/step_record)

...
- `Visualizer (VISU)` <Badge text="c1.4" variant="tip" />
  - When On, enables real-time audio visualizations on the OLED display with multiple modes including waveform, spectrum, MIDI piano roll, and special effects. Works by default with the VU meter or can be toggled independently.

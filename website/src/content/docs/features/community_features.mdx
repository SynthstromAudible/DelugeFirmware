...
- (#3698) <Badge text="c1.3" variant="tip" /> `Learned MIDI input updates clip PGM/BANK/SUB` - MIDI tracks that are learned to a channel will listen for program and bank changes (CC 0 and 32), and assign incoming values into the active clip when RECORD is illuminated.This is useful for toggling between the following scenarios:

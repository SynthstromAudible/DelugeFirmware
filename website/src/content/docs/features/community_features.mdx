...
- Horizontal Menu controls.
  ...
  - Additionally, you can change the selected menu item by :key[PRESS TURN SELECT]
...
- (#XXXX) <Badge text="c1.3" variant="tip" /> Added battery level display in the System menu
  ...
  - On 7-segment displays: Shows voltage in format "X.XX" (volts)
...
- `Show Battery Level (BATT)` <Badge text="c1.3" variant="tip" />
  - When On, displays battery level information in the System menu. Shows voltage and percentage on OLED displays, voltage only on 7-segment displays.
## 6. Sysex Handling

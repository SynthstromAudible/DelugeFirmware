...
MIDI control mode which enables you to learn your external controller to the Deluge only once.

- Edit the active clip in the track you last interacted with
- <Badge text="c1.3" variant="tip" /> Edit the active clip of specific tracks.
- Provides a standard MIDI CC implementation to control Deluge parameters with ease.
...
- There are three regular MIDI follow channel's (A/B/C) which will control the instrument of the active context. These follow channel's allows you to learn different devices to MIDI follow, should you require device specific channel settings.
- <Badge text="c1.3" variant="tip" /> There are sixteen specific track MIDI
  follow channel's (Track1-16) which will always control the specific track
  configured.
...
- In the `MIDI-Follow > Channel` submenu, set the channel between 1 and 16 for Non-MPE or MPE Lower/Upper for MPE.
  - There are three regular channel's (A/B/C) available to accomodate learning multiple devices / channels to MIDI Follow Mode.
    - These channels will always control the selected / active clip context
  - <Badge text="c1.3" variant="tip" /> There are sixteen specific track
    channel's (1-16) available so that you can always control specific tracks
    (e.g. like a mixer) - These channels will always control the active clip of
    the specific track configured
...
- In the `MIDI-Follow > Feedback > Channel` submenu, Enable or Disable MIDI follow feedback by setting a MIDI Follow Feedback Channel.
  - You can choose MIDI Follow Channel A/B/C, Track, Track + A/B/C, or NONE.
    Thus, when you update MIDI Follow Channel A/B/C or a configured Track
    channel it will automatically update the channel used for MIDI Feedback.
  - Track + A/B/C feedback modes send Track feedback first when the
    selected clip belongs to a configured track, then send the selected A/B/C
    feedback channel if it is configured and is not the same MIDI channel.
...
In <Badge text="c1.3" variant="tip" /> the above functionality has been expanded to add specific track control enabling you to have instant mixer control of the parameters of up to 16 tracks.

- CC's received on those channels will control the specific track configured in the same fashion as the regular A/B/C channels.
  - You can now configure up to 16 track channels in the MIDI Follow menu.
  ...
  - You can choose `TRACK + A`, `TRACK + B`, or `TRACK + C` to send feedback
    to both the selected clip's configured Track channel and the configured
    A/B/C channel. If both resolve to the same MIDI channel, feedback is only
    sent once.
...
In <Badge text="c1.3" variant="tip" /> additional MIDI's CC's (89 and 90) have been reserved for Specific Track Mute and Solo commands:
...
- Set up to sixteen MIDI Follow Channels and Devices for controlling up to 16 Specific Tracks <Badge text="c1.3" variant="tip" />
- Set the MIDI Follow Root Note for Kits
- Enable or Disable MIDI Follow Param Pop-up's
- Enable or Disable MIDI Follow Feedback by setting/unsetting the MIDI Follow
  Feedback Channel, including paired Track + A/B/C feedback modes
...
### MIDI Follow is unresponsive to MIDI controller input

As of <Badge text="c1.3" variant="tip" /> MIDI Follow settings are no longer stored in Flash Memory. They are now saved in the MIDIFollow.XML file found in SD CARD/Settings/MIDIFollow.XML
...
<img
  width="560"
  alt="image"
  src="https://raw.githubusercontent.com/SynthstromAudible/DelugeFirmware/refs/heads/main/contrib/midi_follow/touch_osc/touch_osc.png"
/>
...
## Appendix F - Neuzeit Instruments' Drop Templates for Deluge MIDI Follow Mode

For users of Neuzeit Instruments' [Drop MIDI Control Center](https://www.neuzeit-instruments.com/Drop), you will find a MIDI Follow project file in this folder: [MIDI Follow Mode Neuzeit Instruments Drop Project Template]
...
[MIDI Follow Mode Neuzeit Instruments Drop Project Template]: https://github.com/SynthstromAudible/DelugeFirmware/tree/main/contrib/midi_follow/neuzeit_drop

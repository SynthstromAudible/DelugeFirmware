...
#### <ins>Scales/Key</ins>
- Added toggle in `SETTINGS > DEFAULTS > UI > USE FLATS(B)` to change notes displayed to use flats instead of sharps when displaying an accidental. When enabled in OLED mode, notes will be displayed as BB or EB instead of A# or D#. In 7SEG mode, a dot is used to indicade when the note has the configured accidental. 

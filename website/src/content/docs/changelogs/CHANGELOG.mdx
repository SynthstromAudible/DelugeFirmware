...
import RecentMainChanges from "../../../components/RecentMainChanges.astro"
...
<Aside type="tip">
  A new shortcuts page is available, check the [Deluge
  Companion](/reference/deluge_companion)!
</Aside>
...
## c1.3.0 Derbyshire <Badge text="Upcoming" variant="tip" />

### Major Workflow / UI Changes from Official or Previous Community Releases

#### <ins>Audio clips / Audio Clip View</ins>
...
- Added `Edit Name` menu to Actions > menu which allows you to rename the Audio Clips Track Name and Clip Name. :key[Shift] + `NAME` :key[pad] will now open the `Actions > Edit Name` menu.
...
- MIDI Follow settings are no longer stored in Flash Memory. They are now saved in the MIDIFollow.XML file found in SD CARD/Settings/MIDIFollow.XML
- If you are updating to <Badge text="c1.3" variant="tip" /> from an earlier firmware, please delete all existing MIDIFollow.XML files from your SD card so that the file can be re-generated with the new file format.

#### <ins>Instrument Clips / Instrument Clip View</ins>

##### General

- Added `Edit Name` menu to Actions > menu which allows you to rename the Track Name and Clip Name. :key[Shift] + `NAME` :key[pad] will now open the `Actions > Edit Name` menu.
  - Note for Kits, the `Edit Name` menu is in the `Kit FX > Actions` menu which you access by enabling :key[AffectEntire] and then pressing :key[select].
...
- Layered Shortcuts mechanism allows multiple shortcuts to be accessed under a single shortcut pad. Holding :key[Shift] and pressing the same :key[Shortcut Pad] again cycles between shortcuts. The items available under layered shortcuts can also be accessed from menu: layered shortcut is never the only access method.
...
- Master transpose menu has been updated to support decimal editing of the transpose "cents" for finer tuning. UI is the same as with Oscillator and Modulate transpose menus.

#### <ins>Mod FX</ins>

- Added a Warbler fx to synths/kits/kit rows/song/audio clips
...
- Added new Digital Reverb model

#### <ins>LFO's</ins>

- Added Warble LFO type to synths and kit rows
- Added two more LFOs (LFO 3 -global- and LFO 4 -per voice-), accessible in menu and with layered shortcuts

#### <ins>Envelopes</ins>

- Added two more envelopes (Envelope 3 and Envelope 4), accessible in menu and with layered shortcuts

#### <ins>Mod Matrix / Patch Cables</ins>

- Patch cables can be toggled to unipolar or bipolar mode
  - :key[press turn select] to change the polarity while in the patch cable menu
...
- Added round-robin sample variants. Each sample zone can hold up to 4 samples - the primary plus 3 alternates -
  played in turn each time the zone is triggered, removing the "machine gun" effect of repeated hits.
  - Set up in the new `VARIANTS` menu under `OSC1`/`OSC2`, with a `MODE` of `CYCLE`, `RANDOM` or `NO REPEAT`, and
    per-variant `START`/`END`, `TRANSPOSE` and `VOLUME`.
  - Importing a whole folder stacks samples that land on the same note as variants of one zone.
  - Can be turned off in `SETTINGS > COMMUNITY FEATURES > ROUND-ROBIN SAMPLE VARIANTS (VARI)`; variants already saved
    in a song are kept either way.
  - See the [Round-Robin Sample Variants](/features/round_robin_sample_variants) page for details.

### User Interface

#### <ins>General</ins>

- Added a new settings reset workflow under `SETTINGS > ACTIONS > RESET SETTINGS`, with confirmation prompts for:
  - `Reset Flash`
...
Added shortcut to return to Session / Arranger view from menu

- Press :key[SONG] while in the Menu to exit the menu and enter Arranger / Session view
...
- Added `DEFAULTS (DEFA) > UI > ACCESSIBILITY (ACCE)` menu which contains accessibility changes to the Deluge UI to make the deluge more accessible to users with disabilities. These changes include:
  - `Shortcuts (SHOR)` to make specific shortcut combinations more accessible for users with mobility restrictions.
    - :key[horizontal] + :key[play] is changed to :key[crossscreen] + :key[play]
...
- Added Community Feature toggle (`Settings > Community Features > Alternative Tap Tempo Behaviour (TAPT)`) to adjust number of :key[taptempo] button presses to engage `TAP TEMPO` to `FOUR (4)` to avoid mistakingly changing tempo.
...
- Easier loop recording in `Session View` (similar to Kit Row sampling working). Added ability to trigger a loop recording for a specific clip in `Session View` by holding a `CLIP` and pressing `RECORD`. By default this will trigger the Global MIDI Command `LAYERING LOOP`, however this can be changed in the `SETTINGS > RECORDING > LOOP COMMAND (LOOP)` submenu.
...
- Added ability to save / load Pattern-Files to Files. A Pattern represents all Notes of the actual Deluge Screen including Velocity, Probability, Lift, Iterance and Fill. The Patterns can be either of Type Melodic Instrument (Synth, Midi, CV) or Rhythmic Instrument (Kit, Drum). It's also possible to convert Midi-Files to patterns, which can then be used on the Deluge. See documentation at [Save / Load Patterns feature ](/features/save_load_patterns) for more info.
...
- Restored original behaviour which enabled you to insert / delete time for all automated parameters using :key[shift + horizontal]

#### <ins>Session View</ins>

- Doubled the number of Session View sections from 12 to 24.
...
- Added ability to enter clips in `Session Grid View Green Mode` by `Pressing a Clip Pad` + `Pressing the Clip button` if you have `Select in Green Mode` enabled in the `SETTINGS > DEFAULTS > UI > SONG > GRID` menu.

##### Selected Clip Pulsing

- The selected clip in `Session Grid View` (which is the last clip you interacted with), now shows a `pulsing / breathing` animation whereby the clip pad will fade in and out between the clip's normal colour and a blurred colour.
...
- Added the ability to set a CV instrument to use both 1 and 2 channels, which makes the cv2 source selectable between mod wheel, velocity, and aftertouch
  - :key[Turn Select] to choose `1 AND 2 (BOTH)` CV / Gate channel.
...
- Added MIDI CC numbers and labels to :key[gold] encoder popups.
...
#### <ins>Velocity View</ins>

##### General

- Fixed Quantize/Humanize bug which would freeze the velocity view UI
- Updated shortcut for `Note Velocity View`. You can now only enter `Note Velocity View` by holding :key[audition pad] + pressing the `Velocity` :key[pad].

#### <ins>Automation View</ins>

##### Defaults

- The `Settings/Defaults/Automation/Shift` toggle now only applies to Clip View. In Arranger View if you use :key[shift + horizontal] to insert / delete time at a specific point in the arrangement, all parameteter automation is shifted.

##### Automation Overview and Shortcuts

- Removed the `Note Velocity View` shortcut from the Automation Overview. The `Velocity` :key[pad] on the Automation Overview now represents the Automatable parameter `Velocity Spread`
- Updated shortcut for `Note Velocity View`. You can now only enter `Note Velocity View` by holding :key[audition pad] + pressing the `Velocity` :key[pad].
...
- Added same shortcut that is used for copy / pasting notes in Instrument Clip View to Automation View to copy / paste automation. While in the Automation Editor, Copy with :key[learn + horizontal]. Paste with :key[shift + learn + horizontal].

#### <ins>Keyboard View</ins>

##### Kits

- Added ability to change the pad size in the `KIT VELOCITY KEYBOARD VIEW` using the Zoom In/Out shortcut by :key[press turn horizontal]
- `KIT VELOCITY KEYBOARD VIEW` Changes:
  - Additional shortcut of :key[press turn horizontal] to change the pad size using the Zoom In/Out.
...
- Added battery level display in the `SETTINGS` menu showing current battery voltage and percentage on OLED and percentage on 7SEG.
...
- Added new `Use Relative CC` toggle within the configuration of each MIDI device in the `Settings > MIDI > Devices` menu.
...
- Added new `MIDI Thru` toggle within the configuration of each MIDI device in the `Settings > MIDI > Devices` menu.
  - If Devices have not already been set up, or if you Reset Device Settings, then this will inherit by default the Global MIDI Thru setting.
  - After the per Device MIDI Thru setting has been setup, the Global MIDI Thru setting will no longer be used to determine whether MIDI from a specific Device should be sent through.

#### <ins>Learn</ins>

- Fixed issue where MIDI CCs learned to global command's could not trigger those command's while in a menu
...
- Added new learnable global commands:
  - `LOAD NEXT SONG`, which when received while playing, it will queue to load the next song within the folder of the current song.
  - `SHIFT` to toggle the deluge's :key[shift] button. It can also be used to toggle sticky shift.
    - Shift down: Note On or CC Value > 0
    - Shift up: Note Off or CC Value = 0

#### <ins>Follow</ins>

- Add specific track control to `MIDI FOLLOW` enabling you to have instant mixer control of the parameters of up to 16 tracks.
...
- Added new default CC 89 to toggle the mute of the active clip in a specific track.
...
- Fixed a bug that was causing audio clicks when recording audio and when scrolling / zooming in Session Row View and the audio waveform renderer.
...
- Added `AUDIO EXPORT`, an automated process for exporting `CLIP's` while in `Session View` and `TRACK's` while in `ARRANGER VIEW` to individual files. Press :key[Save + Record] to start exporting clips/tracks. Press :key[Back] to cancel exporting and stop recording and playback.
...
- **(Shortcut changed in 1.3)** Added `VELOCITY VIEW`, accessible from `AUTOMATION VIEW OVERVIEW` by pressing the `VELOCITY` shortcut, from `AUTOMATION VIEW EDITOR` by pressing :key[shift] or :key[audition pad] + VELOCITY :key[pad] or from `INSTRUMENT CLIP VIEW` by pressing :key[audition pad] + VELOCITY :key[pad].
...
- Added learning a user specified scale.
  - Hold :key[learn] and press :key[scale] while in clip view. Notes from current clip & all scale mode clips are learned as the "USER" scale. This scale is part of the normal scale rotation, accessible with :key[shift + scale], and saved as part of the song. If another user scale is learned, the previous one is overwritten: currently each song can only have one user scale.
...
- Added `ACTIVE SCALES` menu.
  - `SONG > ACTIVE SCALES` toggles scales on and off from the :key[shift + scale] rotation for the current song. Active scales are saved as part of the song. On 7-segment display dot indicates that the named scale is active, lack of dot indicates it has been disabled.
...
- Added ability to create song macros to quickly switch playing clips from inside clip view without needing to go into Session View.
- From Session View, open the `SONG MENU` and enter the `CONFIGURE MACROS` menu to edit macros

##### Clip Settings Menu

- Holding a clip in `Session Grid View` or the status pad for a clip in `Session Row View` and pressing :key[Select] brings up a `CLIP SETTINGS` menu.
...
- Changed the shortcut to enter Performance View in `Session Grid View` from the `PINK GRID MODE` to the :key[Keyboard] button for consistency with `Session Row View` and `ARRANGER VIEW`.
...
- Added ability to scroll `KEYBOARD VIEW` horizontally using :key[horizontal] while editing Param values in the menu.
...
- `ARRANGER VIEW` and `Session View` now display the name of the current view on the screen.
...
- Added a menu for Song-level parameters, accessible in `Session View` and `ARRANGER VIEW` by pressing :key[Select].
...
- Added Master Chromatic Transpose of All Scale Mode Instrument Clips from any Session View (`Session View`, `ARRANGER VIEW`, `ARRANGER AUTOMATION VIEW` and `PERFORMANCE VIEW`). Uses the same shortcut as in a Synth/Midi/CV clip (:key[Press Turn Vertical] to transpose all clips by +/- 1 semitone). The number of semitones transposed is customizable (:key[Shift + Press Turn Vertical]). After transposing, the display shows the new Root Note (and Scale Name on OLED displays).

#### <ins>VU Meter</ins>

- Added a `VU METER` toggle that displays the VU Meter on the sidebar in `SONG`, `ARRANGER`, & `PERFORMANCE VIEW`. To activate it, on a Session View ensure :key[AffectEntire] is enabled, then select the `LEVEL/PAN` mod button and press it again to activate the meter.
...
- A `MASTER COMPRESSOR` has been added and is accessible in Session View.
...
- **Shortcut changed in 1.3** Pressing :key[Audition Pad] + Random :key[pad] on a drum kit row will load a random sample from the same folder as the current
  sample. \*

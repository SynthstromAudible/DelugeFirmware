...
- Main visualization modes that can be selected as defaults: Waveform, Spectrum, Equalizer, Tunnel, Starfield, Skyline, Pulse Grid
- Additional special visualizers available via hot keys: Cube, Stereo Spectrum, Stereo Equalizer
- Works by default with the VU meter or can be toggled independently
- Can be toggled independently using :key[Shift] + `LEVEL/PAN` mod button for quick access
- Hot keys allow switching between modes: :key[SYNTH] for Waveform, :key[KIT] for Spectrum, :key[MIDI] for MIDI Piano Roll, :key[CV] for special visualizers
- **Clip-specific visualization**: Visualizer can now display waveforms for individual instrument clips (Synth/Kit) when in clip view, keyboard screen, or holding clips in Session/Arranger views
- **Auto-hide on silence**: Visualizer automatically hides after 1 second of audio silence to prevent displaying noise or empty waveforms
- **Program name display**: Shows instrument/program name popup when entering clip-specific visualizer mode

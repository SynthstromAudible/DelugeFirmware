...
- Added Step Record mode (enable the `Step Record (STEP)` community feature). Hold :key[Record] and press :key[Keyboard] in clip view to enter; notes - on the audition pads, the keyboard view, or a MIDI controller - land at a step cursor instead of the live playhead, and the cursor advances one step when the note is released, so notes can be entered one step at a time while the transport is stopped. Chords and stacked kit drums land on one step; :key[Record] while holding a note ties it (extends it by one step and advances) and :key[Record] with no notes held enters a rest, both on melodic and kit clips. :key[Back] exits the mode. See [Step Record](/features/step_record).

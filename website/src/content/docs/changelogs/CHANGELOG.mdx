...
- Added a new settings reset workflow under `SETTINGS > ACTIONS > RESET SETTINGS`, with confirmation prompts for:
  ...
  - `Reset MIDI Devices`
  - `Factory Reset`
- Added a `SHORTCUT BLINK INTERVAL (BLNK)` setting under `SETTINGS > PADS` which controls how fast the shortcut pad blinks to indicate the currently selected menu item (e.g. when choosing a filter type like `EQ` / `HP` / `LP`). It is set in milliseconds; higher values blink slower. The default has been slowed down by 25% (from 180ms to 225ms), and the previous fast blink can be restored by setting it to 180ms.
